...
HTTP smoke tests that exercise the gepaw FastAPI app end-to-end via a
real subprocess. Each test file owns its own gepaw app subprocess on a
...
test file gets its own gepaw app subprocess on a random port, sharing
...
- Forces `gepaw_AUTH_ENABLED=false` and `NO_PROXY=*`
...
>  Always pass `--no-cov` when using this mode ?`pytest-cov` on the
...
- **Coverage mode is single-worker**: `gepaw_INTEGRATION_COVERAGE=1`
