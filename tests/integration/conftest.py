...
def _write_integration_subprocess_rc(root: Path, dest_ini: Path) -> None:
    ...
    """
    src_gepaw = (root / "src" / "gepaw").resolve()
    text = (
        "[run]\n"
        "parallel = true\n"
        "branch = false\n"
...
class AppServer:
    ...
    # Working directory of the subprocess (= gepaw_WORKING_DIR). Tests that
...
def app_server(  # pylint: disable=too-many-statements,too-many-branches
    ...
    """Start one isolated gepaw app process per test module.
    ...
    env["gepaw_WORKING_DIR"] = str(working_dir)
    env["gepaw_SECRET_DIR"] = str(secret_dir)
    env["gepaw_BACKUP_DIR"] = str(backups_dir)
    env["gepaw_AUTH_ENABLED"] = "false"
    env["gepaw_UPLOAD_MAX_SIZE_MB"] = "10"
    env["NO_PROXY"] = "*"
    env["PYTHONUNBUFFERED"] = "1"
    # Force UTF-8 stdio in the subprocess so non-ASCII log lines (e.g.
    # /emoji from skills, agentscope, etc.) don't crash the parent's
    ...
    if _integration_coverage_requested():
        if _INTEGRATION_COVERAGE_DIR is None:
            raise AssertionError(
                "gepaw_INTEGRATION_COVERAGE is set but coverage dir was not "
        ...
        try:
            ...
            while time.time() - start_at < max_wait_seconds:
                if process.poll() is not None:
                    raise AssertionError(
                        "gepaw app exited during startup.\n"
                ...
                raise AssertionError(
                    "gepaw app did not become ready in time.\n"
