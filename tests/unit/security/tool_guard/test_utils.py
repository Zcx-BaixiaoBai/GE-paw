# -*- coding: utf-8 -*-
"""Tests for gepaw.security.tool_guard.utils."""
# pylint: disable=redefined-outer-name
from __future__ import annotations

from unittest.mock import patch

from gepaw.security.tool_guard.models import (
    GuardFinding,
    GuardSeverity,
    GuardThreatCategory,
    ToolGuardResult,
)
from gepaw.security.tool_guard.utils import (
...
class TestResolveGuardedTools:
    ...
    def test_env_var_gepaw_tool_guard_tools(self, mock_env_loader):
        """Env var gepaw_TOOL_GUARD_TOOLS is consulted
    ...
    def test_config_load_failure_falls_to_default(
        ...
        mock_env_loader.return_value = ""
        with patch(
            "gepaw.security.tool_guard.utils._load_config_tool_guard",
            return_value=None,
        ):
            result = resolve_guarded_tools()
        assert result == set(_DEFAULT_GUARDED_TOOLS)
...
class TestResolveDeniedTools:
    ...
    def test_user_defined_none_pass_through(self):
        ...
        with patch(
            "gepaw.security.tool_guard.utils.EnvVarLoader.get_str",
            return_value="",
        ):
    ...
    def test_env_var_gepaw_tool_guard_denied_tools(self, mock_env_loader):
        """Env var gepaw_TOOL_GUARD_DENIED_TOOLS is consulted."""
...
class TestResolveAutoDeniedRules:
    ...
    def test_env_var_consulted_when_user_defined_is_none(
        ...
        """Env var gepaw_TOOL_GUARD_AUTO_DENIED_RULES is read next."""
...
class TestLogFindings:
    ...
    ``gepaw`` logger, so caplog cannot capture records.  Instead we
