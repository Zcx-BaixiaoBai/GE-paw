...
@pytest.fixture
def mock_config():
    """Patch ``gepaw.config.load_config`` to return a mock
    ...
    with patch("gepaw.config.load_config", return_value=app_cfg) as patched:
        yield patched
...
def mock_env_loader():
    ...
    with patch(
        "gepaw.constant.EnvVarLoader.get_str",
        return_value="",
    ) as patched:
        yield patched
...
def mock_workspace(tmp_path: Path):
    ...
    workspace.mkdir()
    with patch(
        "gepaw.config.context.get_current_workspace_dir",
