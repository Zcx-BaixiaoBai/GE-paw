# -*- coding: utf-8 -*-
"""Tests for gepaw.security.skill_scanner.__init__.
...
from gepaw.security.skill_scanner import (
    BlockedSkillRecord,
    SkillScanError,
    _finding_to_dict,
...
from gepaw.security.skill_scanner.models import (
    Finding,
    ScanResult,
    Severity,
    ThreatCategory,
...
class TestIsSkillWhitelisted:
    ...
    def test_loads_config_when_none(self):
        ...
        with patch(
            "gepaw.security.skill_scanner._load_scanner_config",
            return_value=None,
        ) as mock_load:
...
class TestBlockedHistoryPersistence:
    ...
    def test_get_blocked_history_empty(self, tmp_path):
        ...
        with patch(
            "gepaw.security.skill_scanner._get_blocked_history_path",
...
class TestGetScanMode:
    ...
    def test_env_var_block(self):
        """gepaw_SKILL_SCAN_MODE=block should return 'block'."""
        with patch(
            "gepaw.security.skill_scanner.EnvVarLoader.get_str",
            return_value="block",
        ):
            assert _get_scan_mode() == "block"

    def test_env_var_warn(self):
        """gepaw_SKILL_SCAN_MODE=warn should return 'warn'."""
        with patch(
            "gepaw.security.skill_scanner.EnvVarLoader.get_str",
            return_value="warn",
        ):
    ...
    def test_env_var_off(self):
        """gepaw_SKILL_SCAN_MODE=off should return 'off'."""
        with patch(
            "gepaw.security.skill_scanner.EnvVarLoader.get_str",
            return_value="off",
        ):
...
class TestScanSkillDirectory:
    ...
    def test_scan_mode_off(self, tmp_path):
        ...
        with patch(
            "gepaw.security.skill_scanner._get_scan_mode",
            return_value="off",
        ):
    ...
    def test_scan_whitelisted_skill(self, tmp_path):
        ...
        with patch(
            ...
            "gepaw.security.skill_scanner.is_skill_whitelisted",
            return_value=True,
        ):
            result = scan_skill_directory(str(tmp_path))
            assert result is None
    ...
    def test_scan_unsafe_skill_block_mode(self, tmp_path):
        ...
        with patch(
            ...
            "gepaw.security.skill_scanner._get_scanner",
        ) as mock_scanner_cls, patch(
            "gepaw.security.skill_scanner._get_cached_result",
            return_value=None,
        ), patch(
            "gepaw.security.skill_scanner._store_cached_result",
        ):
    ...
    def test_scan_unsafe_skill_warn_mode(self, tmp_path):
        ...
        with patch(
            ...
            "gepaw.security.skill_scanner._record_blocked_skill",
        ):
            mock_scanner = MagicMock()
            mock_scanner.scan_skill.return_value = unsafe_result
            mock_scanner_cls.return_value = mock_scanner
