...
def manager(tmp_path):
    ...
    from gepaw.agents.memory.agent_md_manager import AgentMdManager
...
class TestAgentMdManagerInit:
    ...
    def test_working_dir_created(self, tmp_path):
        ...
        from gepaw.agents.memory.agent_md_manager import (
            AgentMdManager,
        )
...
class TestAgentMdManagerReadWorkingMd:
    ...
    def test_reads_content_via_encoding_fallback(
        ...
        # patch path resolution issues with gepaw.agents.__getattr__
        # on Linux (where the package attr may not yet be set).
        mod = sys.modules["gepaw.agents.memory.agent_md_manager"]
        with patch.object(
            mod,
