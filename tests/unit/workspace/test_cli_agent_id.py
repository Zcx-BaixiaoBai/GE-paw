...
from gepaw.cli.channels_cmd import channels_group
from gepaw.cli.cron_cmd import cron_group
from gepaw.cli.daemon_cmd import daemon_group
from gepaw.cli.chats_cmd import chats_group
from gepaw.cli.skills_cmd import skills_group
from gepaw.config.config import AgentProfileConfig
...
def test_channels_list_default_agent(
    ...
    """Test gepaw channels list uses default agent by default."""
    ...
    with patch("gepaw.cli.channels_cmd.load_agent_config") as mock_load:
...
def test_channels_list_custom_agent(
    ...
    """Test gepaw channels list with custom agent_id."""
...
def test_cron_list_with_agent_id():
    """Test gepaw cron list with --agent-id."""
    runner = CliRunner()

    with patch("gepaw.cli.cron_cmd.client") as mock_client:
...
def test_daemon_status_default_agent():
    """Test gepaw daemon status defaults to 'default' agent."""
    runner = CliRunner()

    with patch("gepaw.cli.daemon_cmd.run_daemon_status") as mock_status:
        with patch("gepaw.cli.daemon_cmd._get_agent_workspace") as mock_ws:
...
def test_daemon_status_custom_agent():
    """Test gepaw daemon status with custom agent."""
    runner = CliRunner()
...
def test_skills_list_default_agent():
    """Test gepaw skills list defaults to 'default' agent."""
    runner = CliRunner()

    with patch(
        "gepaw.cli.skills_cmd._get_agent_workspace",
    ) as mock_ws:
        with patch("gepaw.cli.skills_cmd.SkillService") as mock_service:
            mock_ws.return_value = "/tmp/default"
...
def test_skills_list_custom_agent():
    """Test gepaw skills list with custom agent."""
    runner = CliRunner()
...
def test_chats_list_with_agent_id():
    """Test gepaw chats list with --agent-id."""
    runner = CliRunner()

    with patch("gepaw.cli.chats_cmd.client") as mock_client:
        mock_response = MagicMock()
