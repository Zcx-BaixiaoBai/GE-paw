...
``Workspace.task_tracker`` (gepaw's internal ``TaskTracker``).  Background
...
async def test_unregistered_task_causes_immediate_stop():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # gepaw TaskTracker is empty (no registered tasks)
...
async def test_internal_tracked_task_delays_shutdown():
    """Verify that tasks tracked by gepaw's ``TaskTracker`` (internal
