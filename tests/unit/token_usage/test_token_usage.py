...
from gepaw.token_usage.buffer import (
    TokenUsageBuffer,
    _UsageEvent,
    _apply_event,
)
from gepaw.token_usage.manager import (
...
from gepaw.token_usage.model_wrapper import TokenRecordingModelWrapper
from gepaw.token_usage.storage import load_data, save_data_sync
...
class TestTokenUsageManagerCore:
    ...
    def test_get_instance_returns_singleton(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "gepaw.token_usage.manager.WORKING_DIR",
            tmp_path,
        )
        monkeypatch.setattr(
            "gepaw.token_usage.manager.TOKEN_USAGE_FILE",
