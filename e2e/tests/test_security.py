# -*- coding: utf-8 -*-
"""
gepaw Security module P0-level end-to-end test cases.
    ...
    def test_tool_guard_toggle_and_tab_switch(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            breadcrumb_settings = page.locator(
                'span[class*="breadcrumbParent"]:has-text(""), '
        ...
        tool_guard_tab = page.locator('[data-node-key="toolGuard"] .gepaw-tabs-tab-btn').first
        file_guard_tab = page.locator('[data-node-key="fileGuard"] .gepaw-tabs-tab-btn').first
        ...
        if not active_panel.is_visible():
            ...
            active_panel = page.locator('.gepaw-tabs-tabpane-active').first
        ...
        tool_guard_switch = active_panel.locator('button.gepaw-switch[role="switch"]').first
        ...
        protected_tools_select = active_panel.locator('.gepaw-select').first
        ...
        file_guard_panel = page.locator('.gepaw-tabs-tabpane-active').first
        ...
        file_guard_switch = file_guard_panel.locator('button.gepaw-switch[role="switch"]').first
        ...
        path_input = file_guard_panel.locator('input[placeholder*="?], input[placeholder*="file or directory"], input[placeholder*="File or directory"], input[placeholder*="path"]').first
...
class TestSecurityFileGuardPathAndToolSelect:
    ...
    def test_file_guard_path_add_and_tool_select(self, page: Page, request: pytest.FixtureRequest):
        ...
        add_button = file_guard_panel.locator('button.gepaw-btn-primary').first
        ...
        tool_guard_panel = page.locator('.gepaw-tabs-tabpane-active').first
        ...
        protected_tools_select = tool_guard_panel.locator('.gepaw-select').first
        expect(protected_tools_select).to_be_visible(timeout=5000)
        # Click the selector inside the Select to trigger the dropdown
        select_selector = protected_tools_select.locator('.gepaw-select-selector').first
        ...
        dropdown = page.locator('.gepaw-select-dropdown:visible').first
        if dropdown.count() > 0 and dropdown.is_visible():
            options = dropdown.locator('.gepaw-select-item').all()
...
class TestSecurityConfigSaveAndPersist:
    ...
    def test_security_config_save_and_persist(self, page: Page, request: pytest.FixtureRequest):
        ...
        tool_guard_switch = tool_guard_panel.locator('button.gepaw-switch[role="switch"]').first
        expect(tool_guard_switch).to_be_visible(timeout=5000)
        ...
        try:
            ...
            save_btn = page.locator('button.gepaw-btn-primary:has-text(""), button:has-text("??)').first
            if not save_btn.is_visible():
                save_btn = page.locator('div[class*="footer"] button.gepaw-btn-primary').first
            ...
            success_msg = page.locator('.gepaw-message-success, .gepaw-message-notice-content:has-text("")').first
            ...
            tool_guard_tab_refreshed = page.locator('[data-node-key="toolGuard"] .gepaw-tabs-tab-btn').first
            expect(tool_guard_tab_refreshed).to_be_visible(timeout=5000)
            tool_guard_tab_refreshed.click()
            page.wait_for_timeout(1500)

            tool_guard_panel_refreshed = page.locator('.gepaw-tabs-tabpane-active').first
            expect(tool_guard_panel_refreshed).to_be_visible(timeout=5000)

            tool_guard_switch_refreshed = tool_guard_panel_refreshed.locator('button.gepaw-switch[role="switch"]').first
            ...
            try:
                tool_guard_switch_refreshed = page.locator('.gepaw-tabs-tabpane-active').first.locator('button.gepaw-switch[role="switch"]').first
                if tool_guard_switch_refreshed.is_visible():
                    ...
                    if current_state != initial_checked:
                        ...
                        # Save again
                        save_btn_refreshed = page.locator('button.gepaw-btn-primary:has-text(""), button:has-text("??)').first
                        if not save_btn_refreshed.is_visible():
                            save_btn_refreshed = page.locator('div[class*="footer"] button.gepaw-btn-primary').first
        ...
        skill_scanner_tab = page.locator('[data-node-key="skillScanner"] .gepaw-tabs-tab-btn').first
        ...
        if skill_scanner_tab.is_visible():
            ...
            skill_scanner_panel = page.locator('.gepaw-tabs-tabpane-active').first
...
class TestSecurityRuleCrud:
    ...
    def test_security_rule_crud(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            add_rule_btn = tool_guard_panel.locator('button:has-text(""), button:has-text("Add Rule")').first
            ...
            modal = page.locator('.gepaw-modal').first
            ...
            confirm_btn = modal.locator('button.gepaw-btn-primary').first
            ...
            severity_tag = rule_row.locator('.gepaw-tag:has-text("HIGH")').first
            if severity_tag.count() == 0:
                severity_tag = rule_row.locator('.gepaw-tag').first
            ...
            enable_switch = rule_row.locator('button.gepaw-switch[role="switch"]').last
            ...
            edit_modal = page.locator('.gepaw-modal').first
            ...
            cancel_btn = edit_modal.locator('button:has-text(""), button:has-text("Cancel")').first
            ...
            if rule_id:
                try:
                    ...
                    if cleanup_rule_row.count() > 0:
                        ...
                        delete_btn.click()
                        page.wait_for_timeout(1500)
                        confirm_delete_btn = page.locator('.gepaw-modal-confirm button.gepaw-btn-primary, .gepaw-modal button:has-text(""), .gepaw-modal button:has-text("Delete")').first
...
class TestSkillScannerModeSwitch:
    ...
    def test_skill_scanner_mode_switch(self, page: Page, request: pytest.FixtureRequest):
        ...
        mode_select = skill_scanner_panel.locator('.gepaw-select').first
        ...
        current_mode_selector = mode_select.locator('.gepaw-select-selector').first
        ...
        try:
            ...
            block_option = page.locator('.gepaw-select-item:has-text("block"), .gepaw-select-item:has-text("Block")').first
            ...
            warn_option = page.locator('.gepaw-select-item:has-text("warn"), .gepaw-select-item:has-text("Warn")').first
            ...
            off_option = page.locator('.gepaw-select-item:has-text("off"), .gepaw-select-item:has-text("Off")').first
            ...
            timeout_input = skill_scanner_panel.locator('input[type="number"], .gepaw-input-number input').first
            ...
            scan_alerts_tab = skill_scanner_panel.locator('[data-node-key="scanAlerts"] .gepaw-tabs-tab-btn, .gepaw-tabs-tab-btn:has-text(""), .gepaw-tabs-tab-btn:has-text("Scan Alerts")').first
            ...
            whitelist_tab = skill_scanner_panel.locator('[data-node-key="whitelist"] .gepaw-tabs-tab-btn, .gepaw-tabs-tab-btn:has-text("?), .gepaw-tabs-tab-btn:has-text("Whitelist")').first
            ...
            if original_mode_text and current_mode_selector:
                try:
                    current_mode_selector.click()
                    page.wait_for_timeout(1000)
                    restore_option = page.locator(f'.gepaw-select-item:has-text("{original_mode_text}")').first
...
class TestDeniedToolsConfig:
    ...
    def test_denied_tools_config(self, page: Page, request: pytest.FixtureRequest):
        ...
        tool_guard_tab = page.locator(
            '[data-node-key="toolGuard"] .gepaw-tabs-tab-btn, '
            '.gepaw-tabs-tab-btn:has-text("Tool Guard"), '
            '.gepaw-tabs-tab-btn:has-text("")'
        ).first
        ...
        active_panel = page.locator('.gepaw-tabs-tabpane-active').first
        guard_switch = active_panel.locator('.gepaw-switch').first
        ...
        denied_tools_select = page.locator(
            '#denied_tools, '
            '.gepaw-select:near(:text("Denied"), 200), '
            '.gepaw-select:near(:text(""), 200)'
        ).first

        if denied_tools_select.count() == 0:
            # Try locating via the Form.Item label
            denied_label = page.locator(':text("Denied Tools"), :text("")').first
        ...
        if denied_tools_select.count() > 0:
            ...
            options = page.locator('.gepaw-select-item-option').all()
            if len(options) > 0:
                ...
                # Verify selection
                selected_tags = page.locator('.gepaw-select-selection-item').all()
            ...
            form_items = active_panel.locator('.gepaw-form-item').all()
...
class TestRulePreview:
    ...
    def test_rule_preview(self, page: Page, request: pytest.FixtureRequest):
        ...
        active_panel = page.locator('.gepaw-tabs-tabpane-active').first
        rule_table = active_panel.locator('table, .gepaw-table').first
        ...
        if rule_table.count() > 0:
            ...
            # Count rule rows
            rule_rows = active_panel.locator('table tbody tr, .gepaw-table-row').all()
            ...
            preview_btns = active_panel.locator(
                'button:has-text("Preview"), button:has-text(""), '
            ...
            if len(preview_btns) > 0:
                ...
                preview_modal = page.locator('.gepaw-modal').first
                if preview_modal.count() > 0:
                    ...
                    # Close the modal
                    close_btn = preview_modal.locator('.gepaw-modal-close, button:has-text("Close"), button:has-text(""), button:has-text("OK")').first
            ...
            rule_content = active_panel.locator(':text("rule"), :text(""), :text("Rule")').all()
...
class TestSecurityBatchRuleToggle:
    ...
    def test_security_batch_rule_toggle(self, page: Page, request: pytest.FixtureRequest):
        ...
        log_test_step("Find the rules table")
        active_panel = page.locator('.gepaw-tabs-tabpane-active').first
        rule_switches = active_panel.locator('.gepaw-switch').all()
