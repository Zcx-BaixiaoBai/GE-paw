# -*- coding: utf-8 -*-
"""
gepaw Token Usage module P0 end-to-end tests
    ...
    def test_token_usage_overview(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            if breadcrumb_settings.is_visible(timeout=3000):
                breadcrumb_current = page.locator('span[class*="breadcrumbCurrent"]:has-text("Token ?), span[class*="breadcrumbCurrent"]:has-text("Token Usage")').first
        ...
        page_title = page.locator('h1:has-text("Token Usage"), h1:has-text("Token"), .gepaw-page-header:has-text("Token")').first
        ...
        overview_cards = page.locator('.gepaw-card, [class*=overviewCard], [class*=statCard]').all()
        ...
        table_area = page.locator('.gepaw-table, table, [class*=dataTable]').first
...
class TestTokenUsageByModel:
    ...
    def test_token_usage_by_model(self, page: Page, request: pytest.FixtureRequest):
        ...
        tables = page.locator('.gepaw-table, table').all()
        ...
        if len(tables) > 0:
            ...
            headers = first_table.locator('th, .gepaw-table-thead th').all()
            ...
            data_rows = first_table.locator('tbody tr, .gepaw-table-row').all()
            empty_state = first_table.locator('.gepaw-empty, :text(""), :text("No data")').first
            ...
            stat_content = page.locator(':text("Model"), :text(""), :text("Token")').all()
...
class TestTokenUsageByDate:
    ...
    def test_token_usage_by_date(self, page: Page, request: pytest.FixtureRequest):
        ...
        if len(tables) >= 2:
            ...
            headers = date_table.locator('th, .gepaw-table-thead th').all()
            header_texts = [h.inner_text().strip() for h in headers if h.is_visible()]
            logger.info(f"Date table column headers: {header_texts}")

            has_date_column = any(
                "date" in h.lower() or "" in h or "Date" in h
            ...
            data_rows = date_table.locator('tbody tr, .gepaw-table-row').all()
            ...
            tabs = page.locator('.gepaw-tabs-tab, .gepaw-segmented-item').all()
            if len(tabs) > 0:
...
class TestTokenUsageDateFilter:
    ...
    def test_token_usage_date_filter(self, page: Page, request: pytest.FixtureRequest):
        ...
        range_picker = page.locator(
            '.gepaw-picker-range, .ant-picker-range, '
        ...
        if range_picker.count() > 0:
            ...
            date_panel = page.locator(
                '.gepaw-picker-dropdown, .ant-picker-dropdown, '
                '.gepaw-picker-panel, .ant-picker-panel'
            ).first
            if date_panel.count() > 0:
                ...
                today_cell = date_panel.locator(
                    '.gepaw-picker-cell-today, .ant-picker-cell-today, '
                    'td.gepaw-picker-cell-in-view'
                ).first
                if today_cell.count() > 0 and today_cell.is_visible(timeout=2000):
                    ...
                    if is_range:
                        ...
                        end_cells = date_panel.locator('td.gepaw-picker-cell-in-view, td.ant-picker-cell-in-view').all()
                        if len(end_cells) > 1:
                            end_cells[-1].click()
                            page.wait_for_timeout(500)
...
class TestTokenUsageEmptyState:
    ...
    def test_token_usage_empty_state(self, page: Page, request: pytest.FixtureRequest):
        ...
        loading = page.locator('.gepaw-spin, .ant-spin, [class*="loading"]').first
        ...
        empty_state = page.locator('.gepaw-empty, :text(""), :text("No data"), :text("Empty")').first
        tables = page.locator('.gepaw-table, table').all()
        cards = page.locator('.gepaw-card').all()
