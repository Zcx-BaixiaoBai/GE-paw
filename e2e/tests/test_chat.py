# -*- coding: utf-8 -*-
"""
gepaw Chat module P0-level end-to-end test cases.
...
class TestChatMessageSearch:
    ...
    def test_chat_message_search(
        ...
        search_selectors = [
            ...
            'button[title*=""], button[title*="Search"]',
            'button[aria-label*=""], button[aria-label*="Search"]',
        ...
        search_input_selectors = [
            '.gepaw-drawer input.gepaw-input',
            '.gepaw-drawer input[type="text"]',
            '.gepaw-drawer-body input',
        ...
        if len(search_results) == 0:
            ...
            drawer_items = clean_chat_page.page.locator(
                '.gepaw-drawer-body .gepaw-list-item, '
                '.gepaw-drawer-body li, '
                '.gepaw-drawer-body mark, '
                '.gepaw-drawer-body [class*="highlight"]'
            ).all()
            if len(drawer_items) > 0:
                ...
                clean_chat_page.wait(3000)
                search_results = clean_chat_page.page.locator(
                    '.gepaw-drawer-body [class*="Item"], '
                    '.gepaw-drawer-body [class*="result"]'
                ).all()
        ...
        if has_zero_results:
            ...
            search_input_retry = clean_chat_page.page.locator(
                '.gepaw-drawer input.gepaw-input, .gepaw-drawer input[type="text"]'
            ).first
...
class TestChatStopGeneration:
    ...
    def test_chat_stop_generation(self, page: Page, request: pytest.FixtureRequest):
        ...
        stop_btn = page.locator(
            ...
            'button[aria-label*="stop"], button[aria-label*=""], '
            'button:has-text("Stop"), button:has-text(""), '
...
class TestChatIMEInput:
    ...
    def test_chat_ime_input(self, page: Page, request: pytest.FixtureRequest):
        ...
        filled_value = input_area.input_value() if input_area.evaluate('el => el.tagName') == 'TEXTAREA' else input_area.inner_text()
        assert "" in filled_value, f"Chinese input failed: {filled_value}"
