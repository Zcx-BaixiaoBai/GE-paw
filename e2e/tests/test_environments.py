# -*- coding: utf-8 -*-
"""
gepaw Environments module P0 end-to-end test cases.
...
ADD_BTN_SELECTOR = 'button[class*="addBtn"], button:has-text("")'
DELETE_ROW_BTN_SELECTOR = 'button[title="?], button[title="Delete Row"], button[title="Delete row"], button[title="delete"]'
KEY_INPUT_SELECTOR = 'input[placeholder="Variable Name"], input[placeholder*="Key"], input[placeholder*="?]'
VALUE_INPUT_SELECTOR = 'input[placeholder="Value"], input[placeholder*="?]'
CHECKBOX_SELECTOR = '.gepaw-checkbox-input'
COUNT_SELECTOR = 'span[class*="toolbarCount"]'
SAVE_BTN_SELECTOR = 'button.gepaw-btn-primary:has-text(""), button:has-text("??), button:has-text("Save")'
...
def click_save_button(page: Page):
    ...
    if not save_btn.is_visible(timeout=3000):
        ...
        fallback_selectors = [
            'div[class*="toolbar"] button.gepaw-btn-primary',
            'button.gepaw-btn-primary:visible',
            'button:has-text("")',
    ...
    def test_environment_list_display(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            breadcrumb_current = page.locator(
                'span[class*="breadcrumbCurrent"]:has-text(""), '
        ...
        if row_count > 0:
            ...
            empty_css = page.locator('.gepaw-empty, [class*=empty]').first
            empty_text = page.locator('text=').or_(page.locator('text=No environment variables')).first
...
class TestAddEnvironment:
    ...
    def test_add_environment_key_required(self, page: Page, request: pytest.FixtureRequest):
        ...
        error_css = page.locator(
            '.gepaw-form-item-validate-error, .gepaw-message-error, '
            '.gepaw-form-item-explain-error, .gepaw-message-notice-content'
        ).first
        error_text = page.locator('text=Key ').or_(page.locator('text=Key is required')).first
...
class TestEnvVarMultiRowAndCheckbox:
    ...
    def test_env_var_multi_row_and_checkbox(self, page: Page, request: pytest.FixtureRequest):
        ...
        insert_btn = first_new_row.locator('button[title=""], button[title="Insert Row Below"], button[title="Insert row below"], button[title="insert"]').first
    ...
    def test_env_var_save_and_persist(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            success_msg = page.locator(
                ...
                '.gepaw-message-notice-content:has-text("")'
            ).first
...
class TestEnvVarKeyValidation:
    ...
    def test_env_var_key_format_validation(self, page: Page, request: pytest.FixtureRequest):
        ...
        error_selector = '.gepaw-form-item-explain-error, [class*="error"]:has-text("")'
...
class TestEnvKeyDuplicateDetection:
    ...
    def test_env_key_duplicate_detection(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            add_btn = page.locator('button:has-text("Add"), button:has-text("")').first
            expect(add_btn).to_be_visible(timeout=5000)
            add_btn.click()
            page.wait_for_timeout(1000)
            ...
            if len(key_inputs) == 0:
                ...
                row_inputs = page.locator('tr:last-child input, .gepaw-form-item input').all()
            ...
            save_btn = page.locator('button:has-text("Save"), button:has-text("")').first
            ...
            error_indicators = page.locator(
                '.gepaw-form-item-has-error, '
                '[style*="border-color: red"], '
                '[style*="color: red"], '
                '.gepaw-form-item-explain-error, '
                ':text(""), :text("duplicate"), :text("Duplicate")'
            ).all()
