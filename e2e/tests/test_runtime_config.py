# -*- coding: utf-8 -*-
"""
gepaw Runtime Config (Agent Config) module P0 end-to-end test cases.
...
class TestReActAgentConfig:
    ...
    def test_react_agent_language_and_timezone(self, page: Page, request: pytest.FixtureRequest):
        ...
        react_tab = page.locator('[data-node-key="reactAgent"] .gepaw-tabs-tab-btn').first
        ...
        card_title = active_panel.locator('.gepaw-spark-title').first
        ...
        language_select = active_panel.locator('.gepaw-select').first
        expect(language_select).to_be_visible(timeout=5000)

        # Read the currently selected value
        current_value = language_select.locator('.gepaw-select-selection-item').first.inner_text()
        ...
        if dropdown.is_visible():
            options = dropdown.locator('.gepaw-select-item-option').all()
        ...
        selects = active_panel.locator('.gepaw-select').all()
        assert len(selects) >= 2, f"Insufficient dropdowns (expected >= 2): {len(selects)}"
        timezone_select = selects[1]
        timezone_value = timezone_select.locator('.gepaw-select-selection-item').first.inner_text()
...
class TestAgentConfigTabSwitch:
    ...
    def test_agent_config_tab_switch(self, page: Page, request: pytest.FixtureRequest):
        ...
        for key in tab_keys:
            tab_btn = page.locator(f'[data-node-key="{key}"] .gepaw-tabs-tab-btn').first
        ...
        llm_retry_tab = page.locator('[data-node-key="llmRetry"] .gepaw-tabs-tab-btn').first
        llm_retry_tab.click()
        page.wait_for_timeout(1500)

        retry_panel = page.locator('.gepaw-tabs-tabpane-active').first
        expect(retry_panel).to_be_visible(timeout=5000)

        retry_switches = retry_panel.locator('button.gepaw-switch[role="switch"]').all()
        retry_inputs = retry_panel.locator('.gepaw-input, .gepaw-input-number, .gepaw-select').all()
        ...
        rate_limiter_tab = page.locator('[data-node-key="llmRateLimiter"] .gepaw-tabs-tab-btn').first
        rate_limiter_tab.click()
        page.wait_for_timeout(1500)

        rate_panel = page.locator('.gepaw-tabs-tabpane-active').first
        expect(rate_panel).to_be_visible(timeout=5000)

        rate_switches = rate_panel.locator('button.gepaw-switch[role="switch"]').all()
        rate_inputs = rate_panel.locator('.gepaw-input, .gepaw-input-number, .gepaw-select').all()
        ...
        context_tab = page.locator('[data-node-key="lightContext"] .gepaw-tabs-tab-btn').first
        context_tab.click()
        page.wait_for_timeout(1500)

        context_panel = page.locator('.gepaw-tabs-tabpane-active').first
        expect(context_panel).to_be_visible(timeout=5000)

        context_inputs = context_panel.locator('.gepaw-input, .gepaw-input-number, .gepaw-select').all()
        ...
        react_panel = page.locator('.gepaw-tabs-tabpane-active').first
...
class TestAgentConfigSaveAndReset:
    ...
    def test_config_save_and_reset(self, page: Page, request: pytest.FixtureRequest):
        ...
        context_tab = page.locator('[data-node-key="remeLightMemory"] .gepaw-tabs-tab-btn').first
        ...
        enable_switch = context_panel.locator('.gepaw-switch').first
        ...
        save_btn = page.locator('button.gepaw-btn-primary:has-text("Save")').first
        ...
        success_msg = page.locator('.gepaw-message-success, .gepaw-message-notice-content:has-text("Save")').first
        try:
        ...
        context_tab_refreshed = page.locator('[data-node-key="remeLightMemory"] .gepaw-tabs-tab-btn').first
        expect(context_tab_refreshed).to_be_visible(timeout=5000)
        context_tab_refreshed.click()
        page.wait_for_timeout(1500)

        context_panel_refreshed = page.locator('.gepaw-tabs-tabpane-active').first
        ...
        enable_switch_refreshed = context_panel_refreshed.locator('.gepaw-switch').first
        ...
        # Save again
        save_btn_refreshed = page.locator('button.gepaw-btn-primary:has-text("Save")').first
...
class TestToolResultCompactConfig:
    ...
    def test_tool_result_compact_config(self, page: Page):
        ...
        inputs = active_panel.locator('.gepaw-input, .gepaw-input-number, .gepaw-select').all()
...
class TestEmbeddingConfig:
    ...
    def test_embedding_config(self, page: Page):
        ...
        switches = active_panel.locator('.gepaw-switch').all()
...
class TestContextCompactConfig:
    ...
    def test_context_compact_config(self, page: Page, request: pytest.FixtureRequest):
        ...
        context_tab = page.locator(
            '[data-node-key="lightContext"] .gepaw-tabs-tab-btn, '
            '.gepaw-tabs-tab-btn:has-text("Context")'
        ).first
        expect(context_tab).to_be_visible(timeout=5000)
        context_tab.click()
        page.wait_for_timeout(1500)
        ...
        switches = active_panel.locator('.gepaw-switch').all()
        sliders = active_panel.locator('.gepaw-slider').all()
        inputs = active_panel.locator('.gepaw-input, .gepaw-input-number, .gepaw-select').all()
...
class TestConfigDynamicLinkage:
    ...
    def test_config_dynamic_linkage(self, page: Page, request: pytest.FixtureRequest):
        ...
        tabs = page.locator('.gepaw-tabs-tab').all()
...
class TestMemorySummaryConfig:
    ...
    def test_memory_summary_config(self, page: Page, request: pytest.FixtureRequest):
        ...
        memory_tab = page.locator(
            '[data-node-key="memorySummary"] .gepaw-tabs-tab-btn, '
            '.gepaw-tabs-tab-btn:has-text("Memory")'
        ).first
        ...
        number_inputs = active_panel.locator('.gepaw-input-number').all()
