# -*- coding: utf-8 -*-
"""
gepaw Skills module P0 end-to-end test cases.
...
SKILL_CARD_SELECTOR = ".gepaw-card"
SWITCH_SELECTOR = '.gepaw-switch'
...
class TestSkillListAndFilter:
    ...
    def test_skill_list_filter_and_search(self, page: Page, request: pytest.FixtureRequest):
        ...
        if search_container.is_visible():
            ...
            search_select = search_container.locator('.gepaw-select').first
            search_select.click()
            ...
            dropdown = page.locator('.gepaw-select-dropdown').first
            if dropdown.is_visible():
                ...
                if len(options) > 0:
                    ...
                    # Clear filter
                    clear_btn = search_container.locator('.gepaw-select-clear').first
...
class TestSkillImportToggleDeleteBatch:
    ...
    def test_import_toggle_delete_and_batch(self, page: Page, request: pytest.FixtureRequest):
        ...
        create_btn = page.locator('button:has-text("?), button:has-text("Create Skill"), button:has-text("Create")').first
        ...
        batch_btn = page.locator('button:has-text(""), button:has-text("Batch"), button:has-text("Bulk")').first
        if batch_btn.is_visible():
            batch_btn.click()
            page.wait_for_timeout(1000)

            checkboxes = page.locator(
                '.gepaw-card input[type="checkbox"], '
                '.gepaw-card .gepaw-checkbox'
            ).all()
            if len(checkboxes) >= 2:
                checkboxes[0].check()
                checkboxes[1].check()
            ...
            exit_btn = page.locator(
                'button:has-text("?), button:has-text("Exit"), '
...
class TestSkillCRUDLifecycle:
    ...
    def test_skill_create_edit_delete(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            create_btn = page.locator('button:has-text("?), button:has-text("Create")').first
            ...
            drawer = page.locator('.gepaw-drawer-open').first
...
"""
            ...
            content_area = drawer.locator('.gepaw-form-item').filter(has_text="Content")
            preview_switch = content_area.locator('button.gepaw-switch[role="switch"]').first
            ...
            submit_btn = drawer.locator('button.gepaw-btn-primary').last
            ...
            edit_drawer = page.locator('.gepaw-drawer-open').first
            ...
            # Disable preview
            edit_content_area = edit_drawer.locator('.gepaw-form-item').filter(has_text="Content")
            edit_preview_switch = edit_content_area.locator('button.gepaw-switch[role="switch"]').first
...
"""
            ...
            save_btn = edit_drawer.locator('button.gepaw-btn-primary').last
            ...
            delete_btn = target_card.locator('button.gepaw-btn-dangerous, button[class*="deleteButton"]').first
            if not delete_btn.is_visible():
                delete_btn = target_card.locator('button:has-text(""), button:has-text("Delete")').first
            ...
            confirm_btn = page.locator('.gepaw-modal-confirm-btns button.gepaw-btn-dangerous').first
            if not confirm_btn.is_visible():
                # Fallback: any danger or primary button in a modal
                confirm_btn = page.locator('.gepaw-modal button.gepaw-btn-dangerous, .gepaw-modal button.gepaw-btn-primary').first
            if not confirm_btn.is_visible():
                confirm_btn = page.locator('button:has-text(""), button:has-text("Delete"), button:has-text(""), button:has-text("OK")').first
                ...
                try:
                    ...
                    if target_card.is_visible():
                        ...
                        if delete_btn.is_visible():
                            delete_btn.click()
                            page.wait_for_timeout(1000)
                            confirm_btn = page.locator('.gepaw-modal-confirm-btns button.gepaw-btn-dangerous, .gepaw-modal button.gepaw-btn-dangerous, .gepaw-modal button.gepaw-btn-primary').first
...
class TestSkillTagManagementAndFilter:
    ...
    def test_skill_tag_management_and_filter(self, page: Page):
        ...
        skill_cards = page.locator(".gepaw-card, .ant-card, [class*='skill-card'], [class*='skill-item']").all()
        ...
        edit_btn = first_skill.locator("button:has-text('Edit'), button:has-text(''), .anticon-edit, [class*='edit-btn']").first
        ...
        if edit_btn.count() > 0:
            ...
            page.wait_for_timeout(500)
            edit_modal = page.locator(".ant-modal:visible, .gepaw-modal:visible, .ant-drawer:visible, .gepaw-drawer:visible").first
            if edit_modal.count() == 0:
                edit_modal = page.locator(".ant-modal-visible, .gepaw-modal-visible, .ant-drawer-visible, .gepaw-modal, .gepaw-drawer").last
            assert edit_modal.count() > 0, "Edit modal did not open"
            logger.info("Edit modal opened")

            log_test_step("Verify form fields in modal")
            form_fields = edit_modal.locator("input, textarea, .gepaw-select, .ant-select, .gepaw-switch").all()
            ...
            tag_input = edit_modal.locator("input[placeholder*='tag'], input[placeholder*=''], [class*='tag-input'] input").first
            existing_tags = edit_modal.locator(".ant-tag, .gepaw-tag, [class*='tag']").all()
            ...
            if tag_input.count() > 0 and tag_input.is_visible():
                ...
                updated_tags = edit_modal.locator(".ant-tag, .gepaw-tag, [class*='tag']").all()
                ...
                if test_tag in tag_texts:
                    ...
                    test_tag_el = edit_modal.locator(f".ant-tag:has-text('{test_tag}'), .gepaw-tag:has-text('{test_tag}')").first
                    close_icon = test_tag_el.locator(".anticon-close, .gepaw-tag-close-icon, [class*='close']").first
            ...
            close_btn = edit_modal.locator("button:has-text('Cancel'), button:has-text(''), .ant-modal-close, .gepaw-modal-close").first
            ...
            detail_area = page.locator(".ant-modal, .gepaw-modal, .ant-drawer, .gepaw-drawer, [class*='detail']").first
        ...
        final_skill_cards = page.locator(".gepaw-card, .ant-card, [class*='skill-card'], [class*='skill-item']").all()
...
class TestSkillImportFromHub:
    ...
    def test_skill_import_from_hub(self, page: Page, request: pytest.FixtureRequest):
        ...
        import_btn = page.locator(
            'button:has-text("Import"), button:has-text(""), '
            'button:has-text("Hub"), '
        ...
        page.wait_for_timeout(2000)
        import_modal = page.locator('.gepaw-modal, .ant-modal, .gepaw-drawer, .ant-drawer, [role="dialog"]').last
        try:
        ...
        close_btn = import_modal.locator(
            '.gepaw-modal-close, button:has-text("Cancel"), button:has-text("")'
        ).first
        if close_btn.count() > 0:
            close_btn.click()
        else:
...
class TestSkillPoolSync:
    ...
    def test_skill_pool_sync(self, page: Page, request: pytest.FixtureRequest):
        ...
        upload_btn = page.locator(
            'button:has-text("Upload"), button:has-text(""), '
            'button:has-text("Pool"), button:has-text(""), '
        ...
        page.wait_for_timeout(500)
        visible_modals = page.locator('.gepaw-modal:visible, .ant-modal:visible, [role="dialog"]:visible')
        sync_modal = visible_modals.last if visible_modals.count() > 0 else page.locator('.gepaw-modal, .ant-modal').last
        ...
        list_items = sync_modal.locator(
            '.gepaw-checkbox, .ant-checkbox, '
            '.gepaw-list-item, .ant-list-item, '
            'tr, [class*="skill"]'
        ).all()
...
"""
            ...
            success_message = page.locator(
                '.gepaw-message-success, '
                '.gepaw-message-notice:has-text(""), '
                '.gepaw-message-notice:has-text("success"), '
                '.gepaw-notification-notice:has-text("")'
            ).first
                    ...
                    if target_card.is_visible():
                        ...
                        if delete_btn.is_visible():
                            ...
                            confirm_btn = page.locator(
                                '.gepaw-modal-confirm-btns button.gepaw-btn-dangerous, '
                                '.gepaw-modal button.gepaw-btn-dangerous, '
                                '.gepaw-modal button.gepaw-btn-primary'
                            ).first
                            if confirm_btn.is_visible():
                                confirm_btn.click()
                                page.wait_for_timeout(2000)
