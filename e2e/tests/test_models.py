# -*- coding: utf-8 -*-
"""
gepaw Local Models module P0 end-to-end test cases.
...
class TestModelListDisplay:
    ...
    def test_model_list_display(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            breadcrumb_settings = page.locator('span[class*="breadcrumbParent"]:has-text(""), span[class*="breadcrumbParent"]:has-text("Settings")').first
            expect(breadcrumb_settings).to_be_visible(timeout=5000)

            breadcrumb_current = page.locator('span[class*="breadcrumbCurrent"]:has-text(""), span[class*="breadcrumbCurrent"]:has-text("Models")').first
        ...
        server_status = page.locator('[class*="serverStatus"], .gepaw-card:has-text("llama.cpp"), .gepaw-card:has-text("Server")').first
        ...
        model_list = page.locator('[class*="modelList"], .gepaw-list, .gepaw-card').all()
        ...
        provider_cards = page.locator('[class*="providerCard"], .gepaw-card').all()
        ...
        modal = page.locator('.gepaw-modal, .gepaw-drawer').first
        ...
        empty_state = page.locator('.gepaw-empty, [class*=empty]').first
        data_items = page.locator('[class*="modelItem"], .gepaw-list-item, .gepaw-table-row').all()
...
class TestModelDownload:
    ...
    def test_model_download_flow(self, page: Page, request: pytest.FixtureRequest):
        ...
        local_entry_selectors = [
            ...
            '[class*="providerCard"]:has-text("")',
            '[class*="providerCard"]:has-text("llama")',
            '[class*="providerCard"]:has-text("Llama")',
            # Button approach
            'button:has-text(""), button:has-text("Manage Local")',
            'button:has-text(""), button:has-text("Download")',
        ...
        if local_entry is None:
            ...
            if len(all_cards) > 0:
                ...
                for card in all_cards:
                    ...
                    if any(kw in card_text.lower() for kw in ["local", "", "llama", "gguf"]):
                        local_entry = card
        ...
        if modal.count() > 0 and modal.is_visible(timeout=5000):
            ...
            download_elements = modal.locator(
                'button:has-text(""), button:has-text("Download"), '
                'button:has-text("Install"), button:has-text(""), '
                '.gepaw-progress, [class*="download" i]'
            ).all()
            logger.info(f"Found {len(download_elements)} download-related elements")

            # Step 6: Close the modal
            log_test_step("6. Close the modal")
            close_btn = modal.locator('.gepaw-modal-close, button[aria-label="Close"]').first
...
class TestModelServe:
    ...
    def test_model_serve_flow(self, page: Page, request: pytest.FixtureRequest):
        ...
        model_items = page.locator('[class*=modelItem], .gepaw-list-item, .gepaw-card').all()
        ...
        serve_btns = page.locator('button:has-text(""), button:has-text("Serve"), button:has-text(""), .gepaw-btn:has-text("")').or_(page.get_by_text("")).or_(page.get_by_text("Serve")).or_(page.get_by_text("")).all()
        ...
        if len(serve_btns) > 0:
            ...
            response_indicators = page.locator(
                '.gepaw-modal, .gepaw-drawer, '
                '.gepaw-message, .gepaw-notification, '
        ...
        port_display = page.locator('[class*=port]').or_(page.get_by_text("")).or_(page.get_by_text("Port")).first
        status_display = page.locator('[class*="status"], [class*="serving"], .gepaw-tag, .gepaw-badge').first
...
class TestModelManagement:
    ...
    def test_model_management_operations(self, page: Page, request: pytest.FixtureRequest):
        ...
        if len(more_btns) > 0:
            ...
            delete_option = page.locator('.gepaw-dropdown-menu-item:has-text(""), .gepaw-dropdown-menu-item:has-text("Delete")').or_(page.get_by_text("")).or_(page.get_by_text("Delete")).first
        ...
        if running_status.is_visible(timeout=3000):
            ...
            # Find stop button
            stop_btn = page.locator('button:has-text(""), button:has-text("Stop")').or_(page.get_by_text("")).or_(page.get_by_text("Stop")).first
...
class TestCustomProviderCreateAndDelete:
    ...
    def test_custom_provider_create_and_delete(self, page: Page):
        ...
        add_provider_btn = page.locator("button:has-text('Add Provider'), button:has-text('?)").first
        ...
        ok_button = modal.locator("button.gepaw-btn-primary").first
        ...
        provider_card = page.locator(
            f".gepaw-card:has-text('{provider_name}'), "
            f".gepaw-card:has-text('{provider_id}'), "
        ...
        delete_btn = provider_card.locator("button.gepaw-btn-dangerous, button[class*='dangerous']").first
        ...
        if delete_btn.count() == 0:
            delete_btn = provider_card.locator(
                "button:has-text('??), button:has-text('Delete'), "
                "button:has-text('')"
            ).first
        ...
        confirm_modal = page.locator(".gepaw-modal-confirm, .gepaw-modal, .gepaw-popconfirm").first
        if confirm_modal.count() > 0:
            ...
            if confirm_btn.count() > 0:
                ...
                footer_btn = confirm_modal.locator(".gepaw-modal-confirm-btns button, .gepaw-modal-footer button").last
...
class TestProviderConfigAndConnection:
    ...
    def test_provider_config_and_connection_test(self, page: Page):
        ...
        try:
            ...
            log_test_step("Submit the create form")
            create_button = modal.locator("button.gepaw-btn-primary").first
            ...
            try:
                ...
                if provider_card.count() > 0:
                    ...
                    delete_btn = provider_card.locator("button.gepaw-btn-dangerous, button[class*='dangerous']").first
                    if delete_btn.count() == 0:
                        delete_btn = provider_card.locator("button:has-text('??), button:has-text('Delete'), button:has-text('')").first
                    if delete_btn.count() > 0:
                        delete_btn.click()
                        page.wait_for_timeout(1000)
                        confirm_modal = page.locator(".gepaw-modal-confirm, .gepaw-modal, .gepaw-popconfirm").first
                        if confirm_modal.count() > 0:
                            confirm_btn = confirm_modal.locator("button.gepaw-btn-primary, button:has-text('??), button:has-text('OK')").first
                            if confirm_btn.count() > 0:
                                confirm_btn.click()
                                page.wait_for_timeout(2000)
...
class TestProviderSearchFilter:
    ...
    def test_provider_search_filter(self, page: Page, request: pytest.FixtureRequest):
        ...
        search_input = page.locator('input[type="search"], input[placeholder*="search"], input[placeholder*="Search"], input[placeholder*=""], .gepaw-input-search input').first
        expect(search_input).to_be_visible(timeout=5000)
        logger.info("Search box exists")

        log_test_step("Record Provider count before search")
        provider_cards = page.locator('.gepaw-card').all()
        ...
        # Search box is a gepaw-select component (readonly input); need to click the parent container to open dropdown
        ...
        if is_readonly:
            ...
            select_container = page.locator('.gepaw-select').first
        ...
        filtered_cards = page.locator('.gepaw-card').all()
        ...
        if is_readonly:
            ...
            clear_btn = page.locator('.gepaw-select-clear').first
        ...
        restored_cards = page.locator('.gepaw-card').all()
...
class TestModelActivation:
    ...
    def test_model_activation(self, page: Page, request: pytest.FixtureRequest):
        ...
        models_btn = page.locator('button:has-text("Models"), button:has-text("")').first
        ...
        if models_btn.count() > 0:
            ...
            if modal.count() > 0:
                ...
                # Close modal
                close_btn = modal.locator('.gepaw-modal-close, button:has-text("Cancel"), button:has-text("")').first
                if close_btn.count() > 0:
                    close_btn.click()
                    page.wait_for_timeout(1000)
            else:
                logger.info("No model management modal, may be a Drawer instead")
                drawer = page.locator('.gepaw-drawer').first
                if drawer.count() > 0:
            ...
            has_response = page.locator('.gepaw-modal, .gepaw-drawer').first.count() > 0
...
class TestOpenRouterFilter:
    ...
    def test_openrouter_filter(self, page: Page, request: pytest.FixtureRequest):
        ...
        settings_btn = page.locator(
            'button:has-text("Settings"), button:has-text(""), '
            'button:has-text("Configure"), button:has-text(""), '
        ...
        if settings_btn.count() > 0:
            ...
            modal_or_drawer = page.locator('.gepaw-modal, .ant-modal, .gepaw-drawer, .ant-drawer').first
