# -*- coding: utf-8 -*-
"""
gepaw Heartbeat module P0 end-to-end tests
...
class TestHeartbeatFullConfig:
    ...
    def test_full_heartbeat_configuration(self, heartbeat_page: HeartbeatPage, request: pytest.FixtureRequest):
        ...
        heartbeat_page.set_interval(15, "")
        ...
        heartbeat_page.assert_interval(15, "")
        ...
        heartbeat_page.configure_heartbeat(
            ...
            unit=original_interval.get("unit", "") or "",
...
class TestHeartbeatTargetAndActiveHours:
    ...
    def test_target_session_and_active_hours(self, heartbeat_page: HeartbeatPage, request: pytest.FixtureRequest):
        ...
        target_session_selector = heartbeat_page.page.locator(
            '.gepaw-radio-group, .gepaw-select, [class*="targetSession"], [class*="target"]'
        ).first
        ...
        main_option = heartbeat_page.page.locator(
            '.gepaw-radio-label:has-text("main"), .gepaw-radio-label:has-text("?), '
            '[class*="radio"]:has-text("main"), [class*="radio"]:has-text("?)'
        ).first
        last_option = heartbeat_page.page.locator(
            '.gepaw-radio-label:has-text("last"), .gepaw-radio-label:has-text("?), '
            '[class*="radio"]:has-text("last"), [class*="radio"]:has-text("?)'
        ).first
        ...
        active_hours_switch = heartbeat_page.page.locator(
            '.gepaw-switch, [class*="activeHours"], [class*="active"]'
        ).first
        ...
        start_time_picker = heartbeat_page.page.locator(
            '.gepaw-picker, .gepaw-time-picker, [class*="startTime"], [class*="start"]'
        ).first
        if start_time_picker.is_visible():
            start_time_picker.click()
            heartbeat_page.page.wait_for_timeout(500)
            # Select 09:00
            time_option = heartbeat_page.page.locator('.gepaw-picker-panel li, .ant-picker-panel li').filter(has_text="09").first
        ...
        end_time_picker = heartbeat_page.page.locator(
            '.gepaw-picker, .gepaw-time-picker, [class*="endTime"], [class*="end"]'
        ).first
        if end_time_picker.is_visible():
            end_time_picker.click()
            heartbeat_page.page.wait_for_timeout(500)
            # Select 18:00
            time_option = heartbeat_page.page.locator('.gepaw-picker-panel li, .ant-picker-panel li').filter(has_text="18").first
            if time_option.is_visible():
                time_option.click()
                heartbeat_page.page.wait_for_timeout(500)
...
class TestHeartbeatIntervalUnit:
    ...
    def test_heartbeat_interval_unit(self, page: Page, heartbeat_page: "HeartbeatPage", request: pytest.FixtureRequest):
        ...
        # Need to locate the .gepaw-select container that wraps this input
        unit_select = page.locator('.gepaw-select:has(#everyUnit)').first

        if unit_select.count() > 0:
            # Get currently selected unit text (use selection-item to avoid duplicate text)
            selection_item = unit_select.locator('.gepaw-select-selection-item')
            ...
            if target_option:
                ...
                unit_select.click()
                page.wait_for_timeout(500)
                restore_option = page.locator(f'.gepaw-select-item-option:has-text("{current_unit}")').first
