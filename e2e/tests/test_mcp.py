# -*- coding: utf-8 -*-
"""
gepaw MCP module P0 end-to-end tests
...
class TestCreateMCPClient:
    ...
    def test_create_mcp_client_stdio_and_http(self, page: Page, request: pytest.FixtureRequest):
        ...
        modal = page.locator('.gepaw-modal-content').first
                ...
                try:
                    ...
                    for card in cleanup_cards:
                        ...
                        if title_el.is_visible():
                            title_text = title_el.inner_text()
                            if client_name in title_text:
                                delete_btn = card.locator('button:has-text(""), button[title=""], button[class*="deleteBtn"]').first
                                if not delete_btn.is_visible():
                                    ...
                                    if card_footer.is_visible():
                                        delete_btn = card_footer.locator('button:has-text("")').first
...
class TestMcpClientEdit:
    ...
    def test_mcp_client_edit(self, page: Page, request: pytest.FixtureRequest):
        ...
        if edit_btn.count() > 0:
            ...
            json_editor = modal.locator('textarea, .gepaw-input-textarea, [class*="editor"]').first
        ...
        close_btn = modal.locator('.gepaw-modal-close, button:has-text("Cancel"), button:has-text(""), button:has-text("Close"), button:has-text("")').first
...
class TestMcpMultiProtocol:
    ...
    def test_mcp_multi_protocol(self, page: Page, request: pytest.FixtureRequest):
        ...
        create_btn = page.locator(
            ...
            'button:has-text("New"), button:has-text("")'
        ).first
        ...
        modal_or_drawer = page.locator('.gepaw-modal, .ant-modal, .gepaw-drawer, .ant-drawer').last
        if modal_or_drawer.count() > 0:
            json_input = modal_or_drawer.locator(
                'textarea:not([aria-hidden="true"]), '
                '.gepaw-input-textarea textarea, '
        ...
        if json_input.count() > 0:
            ...
            modal = page.locator('.gepaw-modal, .ant-modal').last
            if modal.count() > 0:
                modal_content = modal.inner_text()
