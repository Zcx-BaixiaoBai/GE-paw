# -*- coding: utf-8 -*-
"""
gepaw E2E tests - Skill Pool P0 cases
...
def navigate_to_skill_pool(page: Page):
    ...
    try:
        page.wait_for_selector('.gepaw-card', timeout=15000)
    except Exception:
...
class TestSkillPoolSearch:
    ...
    def test_skill_pool_search(self, page: Page, request: pytest.FixtureRequest):
        ...
        search_input = page.locator(
            ...
            '.gepaw-select-selection-search-input, '
            '.gepaw-input-search input'
        ).first
        ...
        try:
            page.wait_for_selector('.gepaw-card', timeout=10000)
            page.wait_for_timeout(500)
        except Exception:
            logger.warning("Did not see skill cards, page may have no data")
        skill_cards = page.locator('.gepaw-card').all()
        ...
        # Search input is a gepaw-select component (readonly input); click parent container to trigger dropdown
...
class TestSkillPoolInstall:
    ...
    def test_skill_pool_install(self, page: Page, request: pytest.FixtureRequest):
        ...
        broadcast_btn = page.locator(
            'button:has-text(""), button:has-text("Broadcast"), '
        ...
        modal = visible_modals.last if visible_modals.count() > 0 else page.locator('.gepaw-modal, .ant-modal').last
        ...
        checkboxes = modal.locator('.gepaw-checkbox, .ant-checkbox, .gepaw-checkbox-wrapper').all()
        selects = modal.locator('.gepaw-select, .ant-select').all()
        lists = modal.locator('.gepaw-list-item, .ant-list-item, tr').all()
        ...
        confirm_btn = modal.locator(
            ...
            'button:has-text("Broadcast"), button:has-text(""), '
...
class TestSkillPoolBroadcast:
    ...
    def test_skill_pool_broadcast(self, page: Page, request: pytest.FixtureRequest):
        ...
        modal_locator = page.locator('.gepaw-modal:visible, .ant-modal:visible, [role="dialog"]:visible')
        ...
        if len(workspace_items) == 0:
            ...
            workspace_items = modal.locator(
                '.gepaw-checkbox-wrapper, .ant-checkbox-wrapper, '
...
class TestSkillPoolBatchDelete:
    ...
    def test_skill_pool_batch_delete(self, page: Page, request: pytest.FixtureRequest):
        ...
        batch_btn = page.locator(
            'button:has-text(""), button:has-text("Batch"), '
            'button:has-text("Select"), button:has-text("")'
        ).first
        ...
        checkboxes = page.locator('.gepaw-checkbox, .ant-checkbox, .gepaw-checkbox-wrapper').all()
        ...
        if delete_btn.count() > 0 and delete_btn.is_visible(timeout=3000):
            ...
            select_all = page.locator(
                'button:has-text("?), button:has-text("Select All"), '
                '.gepaw-checkbox-wrapper:has-text("?)'
            ).first
    ...
    def test_skill_pool_zip_import(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            upload_btn = page.locator(
                ...
                'button:has-text(""), button:has-text("Upload"), '
            ...
            initial_cards = page.locator('.gepaw-card').all()
...
"""
            ...
            try:
                ...
                updated_cards = page.locator('.gepaw-card').all()
            ...
            if skill_uploaded:
                try:
                    target_card = page.locator(f'.gepaw-card:has-text("{skill_name}")').first
...
class TestSkillPoolBuiltinImport:
    ...
    def test_skill_pool_builtin_import(self, page: Page, request: pytest.FixtureRequest):
        ...
        builtin_btn = page.locator(
            'button:has-text(""), button:has-text("Built"), '
        ...
        modal_or_drawer = page.locator('.gepaw-modal, .ant-modal, .gepaw-drawer, .ant-drawer, [role="dialog"]').last
        if modal_or_drawer.count() > 0:
            ...
            success_msg = page.locator('.gepaw-message-success, .ant-message-success').first
