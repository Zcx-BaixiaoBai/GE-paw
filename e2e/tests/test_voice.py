# -*- coding: utf-8 -*-
"""
gepaw Voice Transcription module P0 end-to-end tests
...
class TestVoiceConfigDisplay:
    ...
    def test_voice_config_display(self, page: Page, request: pytest.FixtureRequest):
        ...
        page_title = page.locator('h1:has-text("Voice"), .gepaw-page-header:has-text("Voice")').first
        ...
        radio_group = page.locator('.gepaw-radio-group, .gepaw-radio-wrapper').first
        voice_switch = page.locator('.gepaw-switch').first
        ...
        all_controls = page.locator('.gepaw-radio-wrapper, .gepaw-switch, .gepaw-select, input').all()
        ...
        form_fields = page.locator('.gepaw-form-item, .gepaw-radio-wrapper, input, .gepaw-select, textarea').all()
        ...
        if has_radio:
            radio_items = page.locator('.gepaw-radio-wrapper').all()
        ...
        save_btn = page.locator('button:has-text("保存"), button:has-text("Save"), button.gepaw-btn-primary').first
...
class TestVoiceToggle:
    ...
    def test_voice_service_toggle(self, page: Page, request: pytest.FixtureRequest):
        ...
        voice_toggle = page.locator('.gepaw-switch').first
        ...
        all_controls = page.locator(
            '.gepaw-radio-group, .gepaw-radio-wrapper, .gepaw-switch, '
            '.gepaw-select, .ant-select, input, select, textarea, '
            '[class*="card"], .gepaw-card'
        ).all()
        ...
        if has_radio:
            ...
            checked_radio = page.locator('.gepaw-radio-wrapper-checked, .gepaw-radio-wrapper.gepaw-radio-wrapper-checked').first
            initial_text = ""
            ...
            new_checked = page.locator('.gepaw-radio-wrapper-checked, .gepaw-radio-wrapper.gepaw-radio-wrapper-checked').first
            ...
            voice_select = page.locator('.gepaw-select, .ant-select').first
            if voice_select.count() > 0 and voice_select.is_visible():
                voice_select.click()
                page.wait_for_timeout(1000)
                # Check dropdown options
                dropdown_items = page.locator('.gepaw-select-item, .ant-select-item, [class*=select-item]').all()
...
class TestVoiceServiceConfig:
    ...
    def test_twilio_config_form(self, page: Page, request: pytest.FixtureRequest):
        ...
        twilio_section = page.locator('[class*=twilio], .gepaw-card:has-text("Twilio"), .gepaw-collapse:has-text("Twilio")').first
        page_content = page.locator('body').inner_text()
        ...
        all_inputs = page.locator('input[type="text"], input[type="password"], .gepaw-input input, input').all()
        ...
        if len(visible_inputs) > 0:
            ...
            selects = page.locator('.gepaw-select, .ant-select').all()
        ...
        save_btn = page.locator('button:has-text("Save"), button:has-text("保存"), button.gepaw-btn-primary').first
        if save_btn.count() > 0 and save_btn.is_visible(timeout=3000):
            assert save_btn.is_enabled(), "Save button should be enabled"
            logger.info("Save button exists and is enabled")
        else:
        ...
        webhook_url = page.locator('[class*=webhook], .gepaw-paragraph:has-text("/voice/")').or_(page.get_by_text("Webhook", exact=False)).first
...
class TestVoiceModeSwitch:
    ...
    def test_voice_mode_switch(self, page: Page, request: pytest.FixtureRequest):
        ...
        mode_select = page.locator(
            '.gepaw-select, .ant-select, '
            '.gepaw-radio-group, .gepaw-segmented'
        ).first
        if mode_select.count() > 0:
            logger.info("Audio mode selector exists")
            if mode_select.locator('.gepaw-select-selector').count() > 0:
                mode_select.click()
                page.wait_for_timeout(500)
        ...
        switches = page.locator('.gepaw-switch').all()
        # Voice page should have at least some config controls
        all_controls = page.locator('.gepaw-switch, .gepaw-select, .ant-select, input, .gepaw-radio-group').all()
