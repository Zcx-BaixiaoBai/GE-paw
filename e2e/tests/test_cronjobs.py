# -*- coding: utf-8 -*-
"""
gepaw CronJobs module P0 end-to-end test cases.
        ...
        try:
            ...
            if exec_btn.count() > 0 and exec_btn.first.is_visible():
                ...
                confirm_or_msg = cronjobs_page.page.locator(
                    '.gepaw-modal, .gepaw-message, .gepaw-notification'
                ).first
                if confirm_or_msg.count() > 0 and confirm_or_msg.is_visible(timeout=3000):
                    ...
                    confirm_btn = cronjobs_page.page.locator(
                        '.gepaw-modal .gepaw-btn-primary, button:has-text("OK")'
                    ).first
...
class TestCronJobScheduleAndTaskType:
    ...
    def test_schedule_type_and_task_type(self, cronjobs_page: CronJobsPage, request: pytest.FixtureRequest):
        ...
        drawer = cronjobs_page.page.locator('.gepaw-drawer, .ant-drawer, [class*="drawer"]').first
        ...
        schedule_selector = cronjobs_page.page.locator(
            '.gepaw-radio-group, .gepaw-select, [class*="scheduleType"], [class*="schedule"]'
        ).first
        ...
        daily_option = cronjobs_page.page.locator(
            '.gepaw-radio-label:has-text("daily"), '
        ...
        if daily_option.is_visible():
            ...
            time_picker = cronjobs_page.page.locator(
                '.gepaw-picker, .gepaw-time-picker, [class*="timePicker"], [class*="time"]'
            ).first
        ...
        weekly_option = cronjobs_page.page.locator(
            '.gepaw-radio-label:has-text("weekly"), '
        ...
        if weekly_option.is_visible():
            ...
            weekday_selector = cronjobs_page.page.locator(
                '.gepaw-checkbox-group, .gepaw-select, [class*="weekday"], [class*="week"]'
            ).first
        ...
        custom_option = cronjobs_page.page.locator(
            '.gepaw-radio-label:has-text("custom"), '
        ...
        task_type_selector = cronjobs_page.page.locator(
            '.gepaw-radio-group, .gepaw-select, [class*="taskType"], [class*="task"]'
        ).nth(1)
        ...
        text_option = cronjobs_page.page.locator(
            '.gepaw-radio-label:has-text("text"), '
        ...
        agent_option = cronjobs_page.page.locator(
            '.gepaw-radio-label:has-text("agent"), '
...
class TestCronjobScheduleTypeSwitch:
    ...
    def test_cronjob_schedule_type_switch(self, page: Page):
        ...
        form_inputs = drawer.locator("input, textarea, .gepaw-select, .ant-select").all()
        ...
        schedule_type_select = drawer.locator(".ant-select, .gepaw-select").first
        if schedule_type_select.count() == 0:
            ...
            if schedule_type_label.count() > 0:
                ...
                schedule_type_select = parent_div.locator(".ant-select, .gepaw-select, select").first
        ...
        if schedule_type_select.count() > 0:
            ...
            options = page.locator(".ant-select-item-option, .gepaw-select-item").all()
            ...
            if len(options) > 1:
                ...
                schedule_type_select.click()
                page.wait_for_timeout(500)
                options_refreshed = page.locator(".ant-select-item-option, .gepaw-select-item").all()
        ...
        close_btn = drawer.locator("button:has-text('Cancel'), .ant-drawer-close, .ant-modal-close, .gepaw-modal-close").first
...
class TestCronjobEditAndUpdate:
    ...
    def test_cronjob_edit_and_update(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            submit_btn = drawer.locator('button:has-text("OK"), button:has-text("Submit"), button.gepaw-btn-primary').first
            ...
            task_switch = task_row.locator('.gepaw-switch').first
            ...
            edit_option = page.locator('.gepaw-dropdown-menu-item:has-text("Edit"), .ant-dropdown-menu-item:has-text("Edit")').first
            expect(edit_option).to_be_visible(timeout=3000)
            edit_option.click()
            page.wait_for_timeout(1500)

            log_test_step("Verify the edit drawer opens")
            edit_drawer = page.locator('.gepaw-drawer, .ant-drawer').first
            ...
            save_btn = edit_drawer.locator('button:has-text("OK"), button:has-text("Save"), button.gepaw-btn-primary').first
            if save_btn.count() > 0:
                save_btn.click()
                page.wait_for_timeout(2000)
            ...
            if current_name:
                try:
                    ...
                    if cleanup_row.count() > 0:
                        ...
                        delete_option = page.locator('.gepaw-dropdown-menu-item:has-text("Delete"), .ant-dropdown-menu-item:has-text("Delete")').first
                        if delete_option.count() > 0:
                            delete_option.click()
                            page.wait_for_timeout(1000)
                            confirm_btn = page.locator('.gepaw-modal-confirm .gepaw-btn-primary, .gepaw-popconfirm .gepaw-btn-primary, button:has-text("OK")').first
...
class TestCronjobWeeklySchedule:
    ...
    def test_cronjob_weekly_schedule(self, page: Page, request: pytest.FixtureRequest):
        ...
        drawer = page.locator('.gepaw-drawer, .ant-drawer, .gepaw-modal').first
        if drawer.count() == 0:
        ...
        schedule_select = drawer.locator('.gepaw-select, .ant-select').first
        if schedule_select.count() > 0:
            schedule_select.click()
            page.wait_for_timeout(500)
            weekly_option = page.locator('.gepaw-select-item:has-text("Weekly")').first
            if weekly_option.count() > 0:
                ...
                day_checkboxes = drawer.locator('.gepaw-checkbox, .ant-checkbox').all()
...
class TestCronjobTimezone:
    ...
    def test_cronjob_timezone(self, page: Page, request: pytest.FixtureRequest):
        ...
        timezone_select = drawer.locator(
            '.gepaw-select:near(:text("Timezone"), 200), '
