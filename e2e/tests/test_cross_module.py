# -*- coding: utf-8 -*-
"""
gepaw cross-module end-to-end test cases.
...
"""
            ...
            agent_table = page.locator('.gepaw-table').first
            expect(agent_table).to_be_visible(timeout=5000)
            agent_rows = page.locator('.gepaw-table-tbody tr.gepaw-table-row').all()
            ...
            for agent_row in agent_rows:
                edit_btn = agent_row.locator(
                    ...
                    '.gepaw-space-item:nth-child(1) button'
                ).first
            ...
            if editable_agent_found:
                ...
                modal = page.locator('.gepaw-modal, [role="dialog"]').first
                expect(modal).to_be_visible(timeout=5000)

                skills_section = modal.locator(
                    '.gepaw-form-item:has-text("Skills"), '
                    '[class*=skill]'
                ).first
                ...
                cancel_btn = modal.locator(
                    ...
                    '.gepaw-modal-footer button.gepaw-btn-default'
                ).first
                    ...
                    if skill_card.is_visible(timeout=3000):
                        ...
                        if delete_btn.is_visible(timeout=3000):
                            ...
                            confirm_btn = page.locator(
                                '.gepaw-popconfirm-buttons button.gepaw-btn-primary, '
                                'button:has-text("OK")'
                            ).first
...
class TestSecurityInterceptionInChat:
    ...
    def test_security_config_affects_chat(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            guard_switch = tool_guard_panel.locator('button.gepaw-switch[role="switch"]').first
            ...
            if file_guard_tab.is_visible(timeout=3000):
                ...
                file_switch = file_guard_panel.locator('button.gepaw-switch[role="switch"]').first
...
class TestEnvAndRuntimeConfigFlow:
    ...
    def test_env_and_runtime_config_consistency(self, page: Page, request: pytest.FixtureRequest):
        ...
        env_rows = page.locator(
            '.gepaw-table-tbody tr.gepaw-table-row, '
            '[class*=envRow], '
            '.gepaw-form-item'
        ).all()
