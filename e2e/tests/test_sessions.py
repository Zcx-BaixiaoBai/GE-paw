# -*- coding: utf-8 -*-
"""
gepaw Sessions module P0 end-to-end test cases.
...
class TestSessionListFilterAndDetail:
    ...
    def test_session_list_filter_and_detail(self, sessions_page: SessionsPage, request: pytest.FixtureRequest):
        ...
        if refreshed_count > 0:
            ...
            detail_panel = sessions_page.page.locator('.gepaw-drawer, .gepaw-modal, [class*="detail"]').first
...
class TestEditAndDeleteSession:
    ...
    def test_edit_and_delete_session(self, sessions_page: SessionsPage, ensure_session_data, request: pytest.FixtureRequest):
        ...
        batch_btns = sessions_page.page.locator(
            'button.gepaw-btn-dangerous:has-text("Delete"), '
...
class TestSessionEditAndSave:
    ...
    def test_session_edit_name_and_save(
        ...
        if not edit_btn.is_visible():
            ...
            fixed_row = sessions_page.page.locator('.gepaw-table-cell-fix-right button:has-text("Edit")').first
        ...
        if save_btn.count() == 0:
            save_btn = sessions_page.page.locator('button:has-text("Save"), button.gepaw-btn-primary').first
...
class TestSessionBatchDelete:
    ...
    def test_session_batch_delete(
        ...
        # Use broad selectors to cover both antd and gepaw prefixes
        row_checkboxes = sessions_page.page.locator(
            'tbody tr .gepaw-checkbox-input, '
            'tbody tr .ant-checkbox-input, '
            'tbody tr input[type="checkbox"]'
        ...
        # antd Button type="primary" danger may have classes gepaw-btn-primary + gepaw-btn-dangerous
        ...
        modal_visible = False
        try:
            modal = sessions_page.page.locator('.gepaw-modal, .ant-modal').first
        ...
        confirm_selectors = [
            ...
            '.gepaw-modal-confirm-btns .gepaw-btn-dangerous',
            '.gepaw-modal-confirm-btns .gepaw-btn-primary',
            '.gepaw-modal .gepaw-btn-dangerous',
            '.gepaw-modal .gepaw-btn-primary',
            ...
            '.gepaw-modal button:has-text("Delete")',
...
def ensure_session_data(page: Page):
    ...
    existing_count = page.locator(
        ...
        ":not(.gepaw-table-placeholder)"
        ":not(.gepaw-table-measure-row)"
    ).count()
...
class TestSessionFilterByUseridAndChannel:
    ...
    def test_session_filter_by_userid_and_channel(self, page: Page, ensure_session_data):
        ...
        channel_select = page.locator(".gepaw-select, .ant-select").first
        ...
        session_row_selector = "tbody tr:not(.gepaw-table-placeholder):not(.gepaw-table-measure-row)"
