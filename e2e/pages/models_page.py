...
class ModelsPage(BasePage):
    ...
    # Page load indicator
    PAGE_LOAD_INDICATOR = '.ant-breadcrumb, .gepaw-breadcrumb, h1, h2, [class*="breadcrumb"]'

    # Breadcrumb
    BREADCRUMB_SELECTOR = '.ant-breadcrumb, .gepaw-breadcrumb, nav[class*="breadcrumb"], [class*="Breadcrumb"]'

    # Download model button
    DOWNLOAD_MODEL_BTN = 'button:has-text("下载模型"), button:has-text("Download Model"), button:has-text("下载"), button:has-text("Download")'

    # Model list
    MODEL_LIST_SELECTOR = '.ant-list-item, .gepaw-list-item, [class*="modelItem"], [class*="model-item"], table tbody tr, .ant-table-row, .gepaw-table-row'
