...
class ChannelsPage(BasePage):
    ...
    CHANNEL_DRAWER = '.gepaw-drawer:visible, .ant-drawer:visible'
    DRAWER_TITLE = '.gepaw-drawer-title, .ant-drawer-title'
    DRAWER_CLOSE_BTN = '.gepaw-drawer-close, .ant-drawer-close'

    # Form fields
    FORM_ITEM = '.ant-form-item, .gepaw-form-item'
    FORM_LABEL = '.ant-form-item-label, .gepaw-form-item-label'
...
"": ["Console", "console", ""],
    ...
    def find_channel_card(self, channel_name: str) -> Optional[Locator]:
        ...
        Args:
            channel_name: Channel name (e.g. DingTalk/, Feishu/, Discord).
    ...
    def get_channel_bot_prefix(self, channel_name: str) -> str:
        ...
        try:
            card_text = card.inner_text()
            # Extract "Bot Prefix: xxx" / ": xxx" from the card text
    ...
    def is_builtin_channel(self, channel_name: str) -> bool:
        ...
        try:
            # Check whether the card text contains "" or "Built-in"
            card_text = card.inner_text()
            return "" in card_text or "Built-in" in card_text
        except Exception:
            try:
    ...
    def wait_for_drawer_open(self, timeout: Optional[int] = None) -> bool:
        ...
        try:
            self.page.locator('.gepaw-drawer, .ant-drawer').first.wait_for(state="visible", timeout=timeout)
            return True
        except Exception:
            return False
    ...
    def toggle_enable(self, enable: bool = True) -> "ChannelsPage":
        ...
        drawer = self.page.locator('.gepaw-drawer, .ant-drawer')
        switch = drawer.locator('.gepaw-switch, .ant-switch').first
    ...
    def has_form_validation_errors(self) -> bool:
        ...
        errors = self.page.locator(
            '.gepaw-form-item-explain-error, .ant-form-item-explain-error'
        )
        count = errors.count()
        if count > 0:
    ...
    def verify_filter_result(self, filter_type: str) -> bool:
        ...
        if filter_type == 'all':
            ...
            for card in cards:
                try:
                    card_text = card.inner_text()
                    if "" not in card_text and "Built-in" not in card_text:
                        return False
                except Exception:
                    return False
            return len(cards) > 0
