...
class ChatPage(BasePage):
    ...
    # Page components use the gepaw- CSS prefix
    ...
    # Input area
    CHAT_INPUT = 'textarea.gepaw-sender-input'
    SEND_BTN = 'button.gepaw-sender-actions-btn.gepaw-btn-primary'
    FILE_INPUT = 'input[type="file"]'
    UPLOAD_WRAPPER = 'span.gepaw-upload-wrapper'

    # Message area
    USER_MESSAGE = '.gepaw-bubble.gepaw-bubble-end'
    AI_MESSAGE = '.gepaw-bubble.gepaw-bubble-start'
    MESSAGE_CONTAINER = '.gepaw-bubble.gepaw-bubble-start, .gepaw-bubble.gepaw-bubble-end'
    MESSAGE_LIST = '.gepaw-bubble-list-scroll'

    # Welcome screen (check input visibility)
    WELCOME_TEXT = 'textarea.gepaw-sender-input'
    ...
    # Settings and model
    MODEL_SELECTOR = '.gepaw-dropdown-trigger'
    MODEL_OPTION = '.gepaw-dropdown-menu-item'
    AGENT_SELECTOR = '.gepaw-select-selector'

    # Action buttons
    COPY_BTN = 'span[title="复制"]'

    # Tool and skill details
    TOOL_TOGGLE = '.gepaw-operate-card-header-arrow'
    TOOL_DETAILS = '.gepaw-operate-card'

    # Errors and toasts (SUCCESS_MESSAGE / ERROR_MESSAGE inherited from BasePage)
    COPY_SUCCESS = '.gepaw-message-success'

    # Drawer and dialog
    DRAWER_CLOSE = '[class*=headerRight] button'
    CONFIRM_BTN = 'button:has-text("确认"), button:has-text("OK"), .gepaw-btn-primary:has-text("确定")'
    ...
    _JS_BTN_IS_DISABLED = """() => {
        const btn = document.querySelector(
            'button.gepaw-sender-actions-btn.gepaw-btn-primary'
        );
        if (!btn) return false;
        ...
        if (/gepaw-btn-disabled|gepaw-btn-loading|is-disabled|is-loading/.test(cls)) {
            return true;
        }
        return false;
    }"""
            ...
            try:
                self.page.wait_for_function(
                    """() => {
                        ...
                        if (btn) {
                            const cls = btn.className || '';
                            const disabledByCls = /gepaw-btn-disabled|gepaw-btn-loading|is-disabled|is-loading/.test(cls);
                ...
                try:
                    self.page.evaluate(
                        "() => { try { delete window.__gepaw_send_idle_cache__; } catch(e) {} }"
                    )
                except Exception:
                    pass
        ...
        try:
            self.page.wait_for_function(
                """(expectedCount) => {
                    ...
                    if (hasRealText) {
                        const key = '__gepaw_ai_stable_cache__';
                        const now = Date.now();
                        const cache = window[key] || {};
                        if (cache.text !== raw) {
                            window[key] = { text: raw, since: now };
                    ...
                    if (btnEnabled && hasRealText) {
                        window.__gepaw_wait_passed_via__ = 'btn_enabled';
                        return true;
                    }
                    // Path B fallback (release on content stability even if button is forever disabled)
                    if (contentStable) {
                        window.__gepaw_wait_passed_via__ = 'content_stable';
                        return true;
                    }
                    return false;
                }""",
            ...
            try:
                passed_via = self.page.evaluate(
                    "() => window.__gepaw_wait_passed_via__ || 'unknown'"
                )
            except Exception:
                passed_via = "unknown"
            logger.info(
            ...
            try:
                self.page.evaluate(
                    "() => { try { "
                    "delete window.__gepaw_ai_stable_cache__; "
                    "delete window.__gepaw_wait_passed_via__; "
                    "} catch(e) {} }"
                )
            except Exception:
                pass
    ...
    def verify_file_uploaded(self, timeout: int = 10000) -> bool:
        ...
        """
        file_preview_selector = '.gepaw-upload-list-item, .gepaw-sender-content [class*="file"], [class*="attachment"]'
    ...
    def close_session_list(self) -> "ChatPage":
        ...
        close_btn = self.page.locator('.gepaw-drawer ' + self.DRAWER_CLOSE)
    ...
    def rename_session(self, index: int, new_name: str) -> "ChatPage":
        ...
        if rename_input is None:
            for selector in input_selectors:
                locator = self.page.locator(f'.gepaw-modal input, .gepaw-drawer input, {self.SESSION_ITEM} {selector}')
                if locator.count() > 0 and locator.first.is_visible():
                    rename_input = locator.first
    ...
    def open_model_selector(self) -> "ChatPage":
        ...
        header = self.page.locator('.gepaw-chat-anywhere-layout-right-header')
    ...
    def dismiss_error(self) -> "ChatPage":
        ...
        error = self.find(self.ERROR_MESSAGE)
        if error.count() > 0:
            close_btn = error.locator('.gepaw-message-close, .gepaw-notification-close').first
            if close_btn.count() > 0:
                close_btn.click()
                self.wait(500)
        return self
    ...
    def scroll_to_top(self) -> "ChatPage":
        ...
        self.page.evaluate("""() => {
            const list = document.querySelector('.gepaw-bubble-list-scroll');
