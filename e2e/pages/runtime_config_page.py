# -*- coding: utf-8 -*-
"""
gepaw Runtime Config page object.
...
class RuntimeConfigPage(BasePage):
    ...
    PAGE_LOAD_INDICATOR = '.gepaw-tabs-tab-btn'

    # Tabs
    REACT_TAB = '[data-node-key="reactAgent"] .gepaw-tabs-tab-btn'
    LLM_RETRY_TAB = '[data-node-key="llmRetry"] .gepaw-tabs-tab-btn'
    LLM_RATE_LIMITER_TAB = '[data-node-key="llmRateLimiter"] .gepaw-tabs-tab-btn'
    CONTEXT_COMPACT_TAB = '[data-node-key="lightContext"] .gepaw-tabs-tab-btn'
    TOOL_RESULT_COMPACT_TAB = '[data-node-key="lightContext"] .gepaw-tabs-tab-btn'  # Merged into the Context Management tab
    MEMORY_SUMMARY_TAB = '[data-node-key="remeLightMemory"] .gepaw-tabs-tab-btn'
    EMBEDDING_CONFIG_TAB = '[data-node-key="remeLightMemory"] .gepaw-tabs-tab-btn'  # Embedding model config merged into the Long-term Memory tab
    TOOL_EXECUTION_LEVEL_TAB = '[data-node-key="toolExecutionLevel"] .gepaw-tabs-tab-btn'

    # Active panel
    ACTIVE_PANEL = '.gepaw-tabs-tabpane-active'

    # Language dropdown
    LANGUAGE_SELECT = '.gepaw-select'

    # Timezone display
    TIMEZONE_DISPLAY = '.gepaw-select-selection-item'
    ...
    # Save button
    SAVE_BTN = 'button.gepaw-btn-primary:has-text(""), button:has-text("??)'
    RESET_BTN = 'button:has-text(""), button:has-text("??)'

    # Card title
    CARD_TITLE = '.gepaw-spark-title'

    # Generic form elements
    SWITCH = '.gepaw-switch'
    INPUT_NUMBER = '.gepaw-input-number-input'
    SLIDER = '.gepaw-slider'
    ...
    def switch_to_tab(self, tab_key: str) -> "RuntimeConfigPage":
        ...
        """
        tab_selector = f'[data-node-key="{tab_key}"] .gepaw-tabs-tab-btn'
    ...
    def get_memory_backend(self) -> str:
        ...
        if select.is_visible():
            selection = select.locator('.gepaw-select-selection-item').first
    ...
    def select_language(self, language: str) -> "RuntimeConfigPage":
        ...
        # Pick the option
        dropdown = self.page.locator('.gepaw-select-dropdown:visible').first
        if dropdown.is_visible():
            option = dropdown.locator(f'.gepaw-select-item-option:has-text("{language}")').first
    ...
    def get_current_language(self) -> str:
        ...
        language_select = self.get_language_select()
        selection_item = language_select.locator('.gepaw-select-selection-item').first
    ...
    def get_timezone_display(self) -> Locator:
        ...
        if len(selects) >= 2:
            return selects[1]
        return selects[0] if len(selects) > 0 else self.page.locator('.gepaw-select').last
    ...
    def click_save(self) -> "RuntimeConfigPage":
        ...
        if not save_btn.is_visible():
            ...
            save_btn = self.page.locator('div[class*="footer"] button.gepaw-btn-primary').first
    ...
    def assert_config_saved(self) -> "RuntimeConfigPage":
        ...
        error_msg = self.page.locator('.gepaw-message-error')
