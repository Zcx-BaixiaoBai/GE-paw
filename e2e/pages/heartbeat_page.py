...
class HeartbeatPage(BasePage):
    ...
    PAGE_LOAD_INDICATOR = '.ant-switch, .gepaw-switch, input'

    # Configuration card
    CONFIG_CARD = ".ant-card, .gepaw-card, [class*=card]"
    CONFIG_FORM = ".ant-form, .gepaw-form"
    ...
    def is_heartbeat_enabled(self) -> bool:
        ...
        if switch.count() > 0:
            return switch.evaluate("el => el.classList.contains('ant-switch-checked') || el.classList.contains('gepaw-switch-checked') || el.getAttribute('aria-checked') === 'true'")
        return False
    ...
    def get_interval(self) -> Dict[str, Any]:
        ...
        if unit_select.count() > 0:
            ...
            selection_item = unit_select.first.locator('.gepaw-select-selection-item, .ant-select-selection-item')
            ...
            if unit_select.count() > 0:
                ...
                for alias in aliases:
                    option = self.page.locator(
                        f'.gepaw-select-item-option:has-text("{alias}"), '
                        f'.ant-select-item-option:has-text("{alias}"), '
                        f'.gepaw-select-item:has-text("{alias}"), '
...
"": ["", "Seconds", "seconds", "sec"],
    ...
    def assert_config_saved(self) -> "HeartbeatPage":
        ...
        error_msg = self.page.locator('.ant-message-error, .gepaw-message-error')
