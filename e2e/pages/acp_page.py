# -*- coding: utf-8 -*-
"""
gepaw ACP (Agent Communication Protocol) page object.
...
class ACPPage(BasePage):
    ...
    # Filter tabs
    FILTER_TABS = '.gepaw-tabs, .gepaw-segmented, [class*="filterTabs"]'
    TAB_ALL = '[class*="tab"]:has-text("All"), [class*="tab"]:has-text(""), .gepaw-segmented-item:has-text("All")'
    TAB_BUILTIN = '[class*="tab"]:has-text("Builtin"), [class*="tab"]:has-text(""), .gepaw-segmented-item:has-text("Builtin")'
    TAB_CUSTOM = '[class*="tab"]:has-text("Custom"), [class*="tab"]:has-text("?), .gepaw-segmented-item:has-text("Custom")'

    # Create button
    CREATE_BUTTON = 'button:has-text("Create"), button:has-text(""), button:has-text("Add"), button:has-text("")'

    # ACP card list
    ACP_CARD = '[class*="acpCard"], [class*="ACPCard"], .gepaw-card'
    ACP_CARD_TITLE = '[class*="agentKey"], [class*="title"], .gepaw-card-meta-title'
    ACP_CARD_TAG = '.gepaw-tag'
    ACP_CARD_SWITCH = '.gepaw-switch'

    # ACP drawer (create/edit)
    DRAWER = '.gepaw-drawer'
    DRAWER_TITLE = '.gepaw-drawer-title'
    DRAWER_CLOSE = '.gepaw-drawer-close'
    ...
    FORM_ENABLED_SWITCH = '[class*="enabled"] .gepaw-switch, #enabled'
    FORM_TRUSTED_SWITCH = '[class*="trusted"] .gepaw-switch, #trusted'
    FORM_TOOL_PARSE_MODE = '.gepaw-select, select[id*="tool_parse_mode"]'
    FORM_BUFFER_LIMIT = 'input[id*="buffer"], input[name*="buffer"], input[type="number"]'

    # Drawer action buttons
    SAVE_BUTTON = '.gepaw-drawer button:has-text("Save"), .gepaw-drawer button:has-text(""), .gepaw-drawer button.gepaw-btn-primary'
    CANCEL_BUTTON = '.gepaw-drawer button:has-text("Cancel"), .gepaw-drawer button:has-text("")'
    DELETE_BUTTON_DRAWER = '.gepaw-drawer button:has-text("Delete"), .gepaw-drawer button:has-text("")'
    DOC_LINK = '.gepaw-drawer a[href*="doc"], .gepaw-drawer a[href*="integration"]'

    # Confirmation popups
    POPCONFIRM = '.gepaw-popconfirm, .gepaw-modal-confirm'
    POPCONFIRM_OK = '.gepaw-popconfirm button:has-text("OK"), .gepaw-popconfirm button:has-text(""), .gepaw-popconfirm .gepaw-btn-primary'
    POPCONFIRM_CANCEL = '.gepaw-popconfirm button:has-text("Cancel"), .gepaw-popconfirm button:has-text("")'
    ...
    def is_card_builtin(self, card: Locator) -> bool:
        ...
        return "builtin" in card_text or "" in card_text
    ...
    def is_card_enabled(self, card: Locator) -> bool:
        ...
        if switch.count() > 0:
            return switch.evaluate(
                "el => el.classList.contains('gepaw-switch-checked') || "
