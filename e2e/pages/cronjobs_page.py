...
class CronJobsPage(BasePage):
    ...
    PAGE_LOAD_INDICATOR = 'button:has-text(""), button:has-text("Create Job"), .gepaw-table, .ant-table'

    # Create button (UI text is in Chinese)
    CREATE_JOB_BTN = 'button:has-text(""), button:has-text("+ "), button:has-text("Create Job"), button:has-text("+ Create Job")'

    # Table selectors
    JOB_TABLE = ".ant-table, .gepaw-table, table"
    JOB_TABLE_ROW = ".ant-table-tbody > tr, .gepaw-table-tbody > tr, table tbody tr"

    # Job action buttons
    EDIT_BTN = 'button:has-text("Edit"), button:has-text(""), .ant-btn:has(svg):not(:has-text("Delete")):not(:has-text(""))'
    DELETE_BTN = 'button:has-text("Delete"), button:has-text("")'
    ENABLE_TOGGLE = '.ant-switch, .gepaw-switch'
    EXECUTE_NOW_BTN = 'button:has-text("Execute Now"), button:has-text("Run"), button:has-text(""), button:has-text("")'

    # Drawer / dialog
    DRAWER = ".ant-drawer, .gepaw-drawer, [class*=drawer]"
    DRAWER_TITLE = ".ant-drawer-title, .gepaw-drawer-title"
    DRAWER_SAVE_BTN = '.ant-drawer .ant-btn-primary:has-text("Save"), .ant-drawer button:has-text("OK"), [class*=drawer] button:has-text("Save"), [class*=drawer] button:has-text("OK"), [class*=drawer] button:has-text(""), [class*=drawer] button:has-text("??), [class*=drawer] button:has-text(""), [class*=drawer] .gepaw-btn-primary'
    DRAWER_CANCEL_BTN = '.ant-drawer .ant-btn:has-text("Cancel"), [class*=drawer] button:has-text(""), [class*=drawer] button:has-text("??)'

    # Form fields
    JOB_NAME_INPUT = 'input#name, input[id*="jobName"], input[placeholder*="Job Name" i], input[placeholder*="" i], input[placeholder*="" i]'
    CRON_EXPRESSION_INPUT = '#schedule_cron'
    TIMEZONE_SELECT = '.ant-select[data-placeholder*="Timezone" i], .gepaw-select[data-placeholder*="" i]'
    TASK_TYPE_SELECT = '.ant-select[data-placeholder*="Task Type" i], .gepaw-select[data-placeholder*="" i]'
    DESCRIPTION_INPUT = 'textarea[id*="description"], textarea[placeholder*="Description" i], textarea[placeholder*="" i]'
    ENABLED_SWITCH = '.ant-switch, .gepaw-switch'

    # Filter and search
    SEARCH_INPUT = 'input[placeholder*="Search" i], input[placeholder*="" i]'
    ...
    def _select_option(self, field_id: str, value: str) -> None:
        ...
        current_value = select.first.locator('.gepaw-select-selection-item, .ant-select-selection-item')
        ...
        selector = select.first.locator('.gepaw-select-selector, .ant-select-selector')
        if selector.count() > 0:
        ...
        option = self.page.locator(f'.gepaw-select-item-option-content:has-text("{value}")').first
    ...
    def get_job_row(self, job_name: str) -> Locator:
        ...
        return self.page.locator(f"tr:not([aria-hidden='true']):not(.gepaw-table-placeholder):not(.gepaw-table-measure-row):has-text('{job_name}')")
    ...
    def job_exists(self, job_name: str) -> bool:
        ...
        pagination_items = self.page.locator('.gepaw-pagination-item:not(.gepaw-pagination-item-active)').all()
    ...
    def save_job(self) -> "CronJobsPage":
        ...
        drawer = self.page.locator('.gepaw-drawer:visible, .ant-drawer:visible')
        if drawer.count() > 0:
            close_btn = self.page.locator('.gepaw-drawer-close, .ant-drawer-close')
    ...
    def delete_job(self, job_name: str, confirm: bool = True) -> "CronJobsPage":
        ...
        if confirm:
            # Confirm deletion
            confirm_btn = self.page.locator('.ant-modal .ant-btn-danger:has-text("OK"), .gepaw-modal .gepaw-btn-danger:has-text("OK"), .ant-modal button:has-text(""), .gepaw-modal button:has-text(""), button:has-text("")')
    ...
    def enable_job(self, job_name: str) -> "CronJobsPage":
        ...
        row = self.get_job_row(job_name)
        toggle = row.locator(self.ENABLE_TOGGLE)
        is_enabled = toggle.first.evaluate("el => el.classList.contains('ant-switch-checked') || el.classList.contains('gepaw-switch-checked')")
        if not is_enabled:
            toggle.click()
        return self
    ...
    def assert_job_enabled(self, job_name: str) -> "CronJobsPage":
        ...
        toggle = row.locator(self.ENABLE_TOGGLE)
        if toggle.count() > 0:
            is_enabled = toggle.first.evaluate("el => el.classList.contains('ant-switch-checked') || el.classList.contains('gepaw-switch-checked') || el.getAttribute('aria-checked') === 'true'")
