...
def generate_backup_id() -> str:
    ...
    Format: ``gepaw-{version}-{YYYYMMDDTHHmmssZ}-{short8}``

    Example: ``gepaw-1.2.3-20260420T093000Z-ab12cd34``
    """
    ver = re.sub(r"[^a-zA-Z0-9._-]", "_", get_gepaw_version())
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    short = uuid4().hex[:8]
    return f"gepaw-{ver}-{ts}-{short}"
...
def finalize_backup_meta(meta, agent_count: int) -> None:
    ...
    meta.gepaw_version = get_gepaw_version()
