...
def install_sidecar_logging(log_path: Path) -> Path:
    ...
    _LOG_FILE.write("\n")
    _LOG_FILE.write(
        f"== gepaw tauri sidecar {datetime.now().isoformat()} ==\n",
    )
    ...
    logging.getLogger("gepaw.tauri").info(
...
def _add_project_file_handler(log_path: Path) -> None:
    # Import lazily so gepaw.constant is loaded only after desktop env setup.
    from gepaw.utils.logging import add_project_file_handler
