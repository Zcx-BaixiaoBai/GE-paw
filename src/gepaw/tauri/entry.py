...
from gepaw.tauri.env import (
    DESKTOP_APP_ENV,
    DESKTOP_CORS_ORIGINS_ENV,
    DESKTOP_READY_PREFIX,
    ensure_desktop_cors_origins,
)
from gepaw.tauri.sidecar_logging import install_sidecar_logging

logger = logging.getLogger(__name__)


def _ensure_gepaw_app_not_loaded() -> None:
    if "gepaw.app._app" in sys.modules:
        raise RuntimeError(
            "gepaw app imported before desktop CORS origins were set",
        )


def _sync_loaded_gepaw_constant_cors_origins() -> None:
    constant_module = sys.modules.get("gepaw.constant")
...
def _install_desktop_runtime() -> None:
    ...
    # from gepaw.constant.CORS_ORIGINS at import time.
    _ensure_gepaw_app_not_loaded()
    ensure_desktop_cors_origins()
    _sync_loaded_gepaw_constant_cors_origins()
...
def _run_backend_server(log_level: str) -> None:
    ...
    from gepaw.config.utils import write_last_api
    from gepaw.constant import LOG_LEVEL_ENV
    ...
    if normalized_log_level in ("debug", "trace"):
        from gepaw.cli.main import log_init_timings
...
def main() -> None:
    ...
    from gepaw.constant import LOG_LEVEL_ENV, WORKING_DIR
    ...
    if not config_path.exists():
        from gepaw.cli.init_cmd import init_cmd
