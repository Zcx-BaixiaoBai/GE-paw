...
Blocks tool calls that target files explicitly listed in a
sensitive-file set.  Works for both POSIX and Windows-style paths.
"""
...
from gepaw.config.context import get_current_workspace_dir
from gepaw.constant import SECRET_DIR, WORKING_DIR
...
_SECRET_DIR_CURRENT_NAME = ".gepaw.secret"
_SECRET_DIR_LEGACY_NAMES = (".qwenpaw.secret", ".copaw.secret")


def _with_platform_trailing_sep(path: Any) -> str:
...
_COMPAT_SECRET_DIRS: tuple = (
    _with_platform_trailing_sep(SECRET_DIR),
    _with_platform_trailing_sep(Path.home() / _SECRET_DIR_LEGACY_NAMES[0]),
    _with_platform_trailing_sep(Path.home() / _SECRET_DIR_LEGACY_NAMES[1]),
)


def ensure_file_guard_paths(paths: Iterable[str]) -> list:
...
def _is_windows_style_path(raw: str) -> bool:
    """Heuristic check: does *raw* look like a Windows-style path?"""
    if not raw:
        return False
...
def _canonicalize_windows_path(raw: str) -> str:
    """Normalize a Windows-style path to canonical lower/forward form."""
...
def _load_sensitive_files_from_config() -> list:
...
def _extract_attached_redirect_path(token: str) -> Optional[str]:
...
def _extract_paths_from_shell_command(command: str) -> list:
...
class FilePathToolGuardian(BaseToolGuardian):
    ...
    def __init__(self, *, sensitive_files: Optional[Iterable[str]] = None) -> None:
        super().__init__(name="file_path_tool_guardian", always_run=True)
        self._enabled: bool = _is_file_guard_enabled()
        self._sensitive_files: set = set()
        self._sensitive_dirs: set = set()
    ...
    def set_sensitive_files(self, paths: Iterable[str]) -> None:
        ...
        normalized_files: set = set()
        normalized_dirs: set = set()
        for path in paths:
            if not path:
                continue
    ...
    def add_sensitive_file(self, path: str) -> None:
        """Register an additional sensitive file or directory."""
        if not path:
            return
        normalized = _normalize_path(path)
        p = Path(normalized)
    ...
    def remove_sensitive_file(self, path: str) -> bool:
        """Remove a previously registered sensitive path. Returns True if removed."""
        if not path:
            return False
        normalized = _normalize_path(path)
        removed = False
        if normalized in self._sensitive_files:
            self._sensitive_files.discard(normalized)
            removed = True
        if normalized in self._sensitive_dirs:
            self._sensitive_dirs.discard(normalized)
            removed = True
        return removed

    def is_sensitive(self, abs_path: str) -> bool:
        """Public wrapper around :meth:`_is_sensitive`."""
        return self._is_sensitive(abs_path)
    ...
    def _make_finding(
        ...
        *,
        ...
        return GuardFinding(
            ...
            description=(
                f"Tool {tool_name!r} attempted to access sensitive "
                f"file via parameter {param_name!r}."
            ),
            tool_name=tool_name,
    ...
    def _check_value(
        ...
        *,
        ...
        if self._is_sensitive(abs_path):
            findings.append(
                self._make_finding(
                    tool_name, param_name, raw_value, abs_path, snippet=snippet,
                ),
            )
    ...
    def guard(self, tool_name: str, params: dict) -> List[GuardFinding]:
        ...
        if tool_name == "execute_shell_command":
            ...
            for raw_path in _extract_paths_from_shell_command(command):
                self._check_value(
                    tool_name, "command", raw_path, findings, snippet=command,
                )
            return findings
    ...
    def reload(self) -> None:
        """Reload enabled state and sensitive paths from config."""
