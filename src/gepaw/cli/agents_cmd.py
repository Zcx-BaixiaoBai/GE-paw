...
def _submit_background_task(
    ...
    try:
        ...
        click.echo(f"  gepaw agents chat --background --task-id {task_id}")
...
def _check_task_status(
    ...
    try:
        ...
        if status == "finished":
            ...
            click.echo(
                f"  gepaw agents chat --background --task-id {task_id}",
            )
...
def agents_group() -> None:
    ...
    \b
    Examples:
      gepaw agents chat --from-agent bot_a --to-agent bot_b --text "..."
      gepaw agents create --name "Research Bot" --agent-id research_bot
      gepaw agents delete research_bot
      gepaw agents list
    """
...
def list_agents(ctx: click.Context, base_url: Optional[str]) -> None:
    ...
    Examples:
      gepaw agents list
      gepaw agents list --base-url http://192.168.1.100:8088
...
def delete_cmd(
    ...
    \b
    AGENT_ID  Configured agent ID, obtainable via `gepaw agents list`.

    \b
    Examples:
      gepaw agents delete research
      gepaw agents delete research --remove-workspace
      gepaw agents delete research --yes
    """
...
def chat_cmd(
    ...
    Background Task Mode (NEW):
      ...
      gepaw agents chat --background --task-id <task_id>
    ...
    \b
    Prerequisites:
      1. Use 'gepaw agents list' to discover available agents
      2. Ensure target agent (--to-agent) is configured and running
      3. Use 'gepaw chats list' to find existing sessions (optional)
