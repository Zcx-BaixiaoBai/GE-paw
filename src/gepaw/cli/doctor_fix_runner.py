# -*- coding: utf-8 -*-
"""Conservative filesystem repairs for ``gepaw doctor fix``.
...
``src/gepaw/console/`` (needs network for npm).

``validate-all-jobs-json`` reuses
:func:`~gepaw.cli.doctor_checks.check_cron_jobs_files` (read-only); exits
...
from ..utils.console_static import find_gepaw_source_repo_root
    ...
    *,
        ...
        for fid in fix_ids:
            if fid in RISKY_FIX_IDS and not yes and not dry_run:
                raise ValueError(
                    ...
                    "Use `gepaw doctor fix --dry-run --only ...` to preview "
                    "the plan without -y.",
                )
    ...
    if (
        ...
        raise ValueError(
            ...
            "ensure-working-dir in --only or run `gepaw doctor fix` without "
        ...
        if repo is None:
            ...
            def _rebuild_console(
                ...
                if (
                    ...
                    bkp_root = r / ".gepaw-doctor-fix-backups"
                    sid = _utc_session_id()
                    bkp = bkp_root / sid
