# -*- coding: utf-8 -*-
"""Extensible hooks for `gepaw doctor`
(entry points + programmatic registration).

Plugins can expose a setuptools entry point in group ``gepaw.doctor``::

    [project.entry-points."gepaw.doctor"]
...
def _load_entry_point_functions() -> list[tuple[str, DoctorNotesFn]]:
    ...
    seen: set[str] = set()
    for group in ("gepaw.doctor", "copaw.doctor"):
