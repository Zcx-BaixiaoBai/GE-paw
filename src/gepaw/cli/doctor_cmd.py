# -*- coding: utf-8 -*-
"""`gepaw doctor` — read-only checks.

`gepaw doctor fix` — conservative repairs with backup.
"""
...
def _doctor_server_python_mismatch_note(
    ...
    if server_exe and doctor_exe:
        if not _same_python_executable(doctor_exe, server_exe):
            return (
                "This `gepaw doctor` is not using the same Python "
                "executable as the running `gepaw app` — diagnostics and "
    ...
    if doctor_env.strip() != server_env.strip():
        return (
            ...
            f"`gepaw app` (doctor: {doctor_env!r}; server: "
            f"{server_env!r}). "
...
def _check_web_auth(base: str) -> tuple[bool, str]:
    ...
    if not has_registered_users():
        return (
            ...
            f"        1) Start `gepaw app`, open {base}/ in a browser.\n"
...
async def _check_active_llm(
    ...
    if (
        ...
        return (
            False,
            "no active LLM slot — run `gepaw models list` and configure "
            "an active model",
            [],
        )
    ...
    if deep and pid in ("gepaw-local", "copaw-local"):
        deep_notes = gepaw_local_llm_deep_notes()
...
def run_doctor_checks(
    ...
    """Run read-only ``gepaw doctor`` checks (no disk mutations)."""
    ...
    if config_ok:
        ...
        _doctor_fix_hint(
            ...
            "`gepaw doctor fix --dry-run --help` and `--only`.",
        )
    ...
    if raw_cfg is not None:
        ...
        if unknown:
            ...
            _doctor_fix_hint(
                ...
                "(`gepaw doctor` and `doctor fix` do not strip unknown keys "
                "yet).",
            )
    ...
    if config_ok:
        ...
        if ws_ok:
            ...
            _doctor_fix_hint(
                "Preview the plan (no writes): `gepaw doctor fix --dry-run "
        ...
        if not ext_nonempty:
            click.echo(
                click.style("OK", fg="green")
                + " — no extension notes (register via gepaw.doctor entry "
        ...
        click.echo(
            ...
            + _skipped_when_cfg_invalid
            + ". Fix the config file, then re-run `gepaw doctor`.",
        )
        click.echo("\n=== Browser (browser_use / Playwright) ===")
    ...
    if llm_ok:
        ...
        _doctor_fix_hint(
            "`gepaw models list` / console model settings — not a "
            "filesystem fix.",
        )
    ...
    try:
        ...
        click.echo(
            f"Hint: start the server with `gepaw app` (default {base}).",
            err=True,
        )
    else:
...
@click.option(
    ...
    help=(
        ...
        "uses --timeout) and, when the active model is gepaw-local, "
