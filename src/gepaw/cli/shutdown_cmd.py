...
def _find_frontend_dev_pids() -> set[int]:
    ...
    for pid, command in _process_table():
        ...
        if "vite" in lowered and console_dir in lowered:
            matches.add(pid)
            continue
        if "gepaw-console" in lowered and (
            "npm" in lowered
            or "pnpm" in lowered
            or "yarn" in lowered
            or "node" in lowered
...
def _find_desktop_wrapper_pids() -> set[int]:
    """Find `gepaw desktop` wrapper processes for this project."""
    matches: set[int] = set()
    patterns = (
        " -m gepaw desktop",
        " gepaw desktop",
...
def _find_windows_wrapper_ancestor_pids(pids: set[int]) -> set[int]:
    ...
    for pid in pids:
        ...
        while True:
            ...
            if _is_gepaw_wrapper_process(parent_info[1], parent_info[2]):
