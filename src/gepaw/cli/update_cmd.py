...
from .process_utils import (
    ...
    _is_gepaw_service_command,
    _process_table,
)

_PYPI_JSON_URL = "https://pypi.org/pypi/gepaw/json"
...
def _detect_installation() -> InstallInfo:
    ...
    dist = metadata.distribution("gepaw")
...
def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `gepaw shutdown` should be used before updating."""
    click.echo("")
...
def _run_shutdown_for_update(
    ...
    """Run `gepaw shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "gepaw"]
    ...
    click.echo("")
    click.echo("Running `gepaw shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `gepaw shutdown`: " f"{exc}",
        ) from exc
...
def _build_upgrade_command(
    ...
    package_spec = f"gepaw=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from gepaw.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[gepaw] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    try:
        ...
        click.echo(f"[gepaw] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        ...
        click.echo(f"[gepaw] Update failed with exit code {return_code}.")
        click.echo(
            "[gepaw] Please fix the error above and run "
            "`gepaw update` again.",
        )
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `gepaw update`, or rerun "
        ...
        if running.is_running:
            raise click.ClickException(
                f"Detected {_running_service_display(running)} "
                "after `gepaw shutdown`. "
                "Please stop it manually before running `gepaw update`.",
            )
