# -*- coding: utf-8 -*-
"""Read-only diagnostics for `gepaw doctor` (no config or disk mutations)."""
...
# Log file opened on app startup (see ``gepaw.app._app`` lifespan).
APP_LOG_BASENAME = LOG_FILE_BASENAME

# Built-in local llama.cpp provider id; legacy configs may still use
# copaw-local.
_QWENPAW_LOCAL_PROVIDER_IDS = frozenset({"gepaw-local", "copaw-local"})
...
def check_app_log_writable() -> tuple[bool, str]:
    ...
    if log_path.exists():
        ...
        return (
            ...
            "(required when starting `gepaw app`)",
        )
...
def environment_summary_lines(
    ...
    *server_python_environment* describes the **HTTP API process** (running
    ``gepaw app``), when ``GET /api/doctor/runtime`` returned
    ...
    lines = [
        ...
        f"gepaw version: {__version__}",
        f"platform: {platform.system()} {platform.machine()}",
        f"doctor_python_environment: {doctor_env}",
    ]
    if server_python_environment is not None:
        lines.append(
            f"gepaw_python_environment: {server_python_environment}",
        )
    else:
        lines.append(
            "gepaw_python_environment: "
...
def windows_environment_lines() -> list[str]:
    """Windows-specific read-only diagnostics for ``gepaw doctor``."""
...
def legacy_single_agent_workspace_note(cfg: Config) -> str | None:
    ...
    When this applies, ``gepaw app`` may run an automatic migration;
    ...
    return (
        ...
        "`gepaw app` uses to trigger legacy → multi-agent workspace "
        "migration. Start `gepaw app` once (or see docs / `gepaw init`). "
...
def check_enabled_agents_load_agent_config(cfg: Config) -> tuple[bool, str]:
    """Dry-run :func:`~gepaw.config.config.load_agent_config` for enabled.
    ...
    for agent_id, ref in cfg.agents.profiles.items():
        ...
        if not path.is_file():
            problems.append(
                ...
                "disk; ensure the file exists or use `gepaw doctor fix` "
                "where applicable.",
            )
            continue
        try:
...
def active_llm_local_failure_hint(provider: Provider, provider_id: str) -> str:
    ...
    if provider_id in _QWENPAW_LOCAL_PROVIDER_IDS:
        return (
            ...
            "binary from there. Run `gepaw doctor --deep` to see llama.cpp "
...
def gepaw_local_llm_deep_notes() -> list[str]:
    ...
    for agent_id, ref in cfg.agents.profiles.items():
        ...
        if deep and pid in _QWENPAW_LOCAL_PROVIDER_IDS:
            ...
            if not any("llama.cpp" in n for n in notes):
                notes.extend(gepaw_local_llm_deep_notes())
...
def console_static_diagnostic_notes() -> list[str]:
    ...
    if repo is not None:
        notes.append(
            ...
            "`gepaw doctor fix -y --only rebuild-console-npm`.",
        )
    else:
        notes.append(
