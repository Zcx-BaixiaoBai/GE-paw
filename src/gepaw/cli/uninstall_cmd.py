# -*- coding: utf-8 -*-
"""gepaw uninstall — remove the gepaw environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove gepaw PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# gepaw" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# gepaw\nexport PATH=\"\$HOME/\.gepaw/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove gepaw environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL gepaw data in {wd}")
    else:
        click.echo(
            "This will remove the gepaw Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("gepaw uninstalled. Please restart your terminal.")
