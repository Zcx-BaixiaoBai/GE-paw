...
# fallback via :class:`~gepaw.constant.EnvVarLoader`).
...
def resolve_console_static_dir() -> str:
    ...
    Resolution order matches :mod:`gepaw.app._app`: env override, package
    ``gepaw/console``, repo ``console/dist``, then cwd fallbacks.
    """
...
def find_gepaw_source_repo_root() -> Path | None:
    """Return the git checkout root if this Python
    is running from QwenPaw source.

    Looks upward from :mod:`gepaw` for ``console/package.json``,
    ``console/package-lock.json``, and ``src/gepaw/``
    (bundled static target).
    Returns ``None`` for a normal pip/wheel install.
    """
    try:
        import gepaw  # noqa: PLC0415 — avoid import cycle at module load
    except Exception:  # pylint: disable=broad-exception-caught
        return None
    cur = Path(gepaw.__file__).resolve().parent
    for _ in range(20):
        con = cur / "console"
        if (
            (con / "package.json").is_file()
            and (con / "package-lock.json").is_file()
            and (cur / "src" / "gepaw").is_dir()
        ):
            return cur
        if cur.parent == cur:
            break
