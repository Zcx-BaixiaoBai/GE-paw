...
TELEMETRY_ENDPOINT = (
    "https://gepawelemetry-sukzkbfzhc.ap-southeast-1.fcapp.run"
)
...
def has_telemetry_been_collected(working_dir: Path) -> bool:
    ...
    try:
        ...
        # v1.1 compat: single gepaw_version field
        return marker_data.get("gepaw_version", "") == current
    except Exception:
        return False
        ...
        if marker_file.exists():
            try:
                ...
                if not collected_versions:
                    old_ver = old_data.get("gepaw_version", "")
                    if old_ver:
