...
"""Handler for AgentScope message, token counting, and context compaction."""
...
class AsMsgHandler:
    """Handles token counting, formatting, and context compaction."""
    ...
    async def _format_tool_result_output(
        ...
        for block in output:
            try:
                ...
                logger.warning(
                    f"Failed to process block {block}: {exc}, skipped.",
                )
    ...
    async def count_msgs_token(self, messages) -> int:
    ...
    async def format_msgs_to_str(
        ...
        messages are skipped when the running token count would exceed
    ...
    @staticmethod
    def validate_tool_ids_alignment(messages) -> bool:
        ...
        Returns:
            True if all tool_use ids have corresponding tool_result ids
            and vice versa.
        """
        tool_use_ids = set()
        tool_result_ids = set()
