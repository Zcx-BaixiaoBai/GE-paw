...
_REQUIREMENTS_METADATA_NAMESPACES = ("openclaw", "gepaw", "clawdbot")
...
def _extract_emoji_from_metadata(metadata: Any) -> str:
    """Extract emoji from metadata.gepaw.emoji."""
    if not isinstance(metadata, dict):
        return ""
    gepaw = metadata.get("gepaw")
    if isinstance(gepaw, dict):
        return str(gepaw.get("emoji", "") or "")
    return ""
...
def extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="gepaw_skill_upload_"))
...
def staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"gepaw_skill_stage_{skill_name}_"),
    )
