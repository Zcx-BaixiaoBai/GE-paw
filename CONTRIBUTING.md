# Contributing to gepaw

## Welcome! 

Thank you for your interest in contributing to gepaw! gepaw is an open-source **personal AI assistant** that runs in your own environmenton your machine or in the cloud. It connects to DingTalk, Feishu, QQ, Discord, iMessage, and other chat apps, supports scheduled tasks and heartbeat, and extends its capabilities through **Skills**. We warmly welcome contributions that help make gepaw more useful for everyone: whether you add a new channel, a new model provider, a Skill, improve docs, or fix bugs.

**Quick links:** [GitHub](https://github.com/agentscope-ai/gepaw)  [Docs](https://gepaw.agentscope.io/)  [License: Apache 2.0](LICENSE)
...
- **Check [Open Issues](https://github.com/agentscope-ai/gepaw/issues)** and any [Projects](https://github.com/agentscope-ai/gepaw/projects) or roadmap labels.
...
gepaw is designed to be **extensible**: you can add models, channels, Skills, and more. Below are the main contribution areas we care about.

---

### Adding New Models / Model Providers

gepaw supports multiple providers, including cloud providers (such as DashScope, ModelScope) and local providers (such as Ollama, LM Studio). We also welcome new model providers to enrich user choices.
...
If the above conditions are met, you can create a new Provider instance in `src/gepaw/providers/provider_manager.py` and register it in the `ProviderManager` class to make it a built-in provider in gepaw.
...
Channels are how gepaw talks to **DingTalk, Feishu, QQ, Discord, iMessage**, etc. You can add a new channel so gepaw can work with your favorite IM or bot platform.

- **Protocol:** All channels use a unified in-process contract: **native payload ?`content_parts`** (e.g. `TextContent`, `ImageContent`, `FileContent`). The agent receives `AgentRequest` with these content parts; replies are sent back via the channels send path.
- **Implementation:** Implement a **subclass of `BaseChannel`** (in `src/gepaw/app/channels/base.py`):
  ...
  - Use the managers queue and consumer loop if the channel is long-lived (default).
- **Discovery:** Built-in channels are registered in `src/gepaw/app/channels/registry.py`. **Custom channels** are loaded from the working directory: place a module (e.g. `custom_channels/telegram.py` or a package `custom_channels/telegram/`) that defines a `BaseChannel` subclass with a `channel` attribute.
- **CLI:** Users install/add channels with:
  - `gepaw channels install <key>` ?create a template or copy from `--path` / `--url`
  - `gepaw channels add <key>` ?install and add to config
  - `gepaw channels remove <key>` ?remove custom channel from `custom_channels/`
  - `gepaw channels config` ?interactive config
...
**Skills** define what gepaw can do: cron, file reading, PDF/Office, news, browser, etc. We welcome **broadly useful** base skills (productivity, documents, communication, automation) that fit the majority of users.
...
- **Location:** Built-in skills live under `src/gepaw/agents/skills/<skill_name>/`. The app merges built-in and user **customized_skills** from the working dir into **active_skills**; no extra registration is needed beyond placing a valid `SKILL.md` in a directory.
...
- **Skills Hub:** gepaw supports importing skills from a community hub (e.g. ClawHub). If you want your skill to be installable via hub, follow the same `SKILL.md` + `references/`/`scripts/` layout and the hubs packaging format.
...
gepaw aims to run on **Windows**, **Linux**, and **macOS**. Contributions that improve support on a specific platform are welcome.
...
- **Install and run:** One-line install (`install.sh`), `pip` install, and `gepaw init` / `gepaw app` should work (or be clearly documented) on each supported platform. Fixes to install or startup on a given OS are valuable.
- **Platform-specific features:** Optional integrations (e.g. notifying only when supported) are fine as long as they dont break other platforms. Use runtime checks or optional dependencies where appropriate.
...
- **MCP (Model Context Protocol):** gepaw supports runtime **MCP tool** discovery and hot-plug. Contributing new MCP servers or tools (or docs on how to attach them) helps users extend the agent without changing core code.
- **Documentation:** Fixes and improvements to [the docs](https://gepaw.agentscope.io/) (under `website/public/docs/`) and README are always welcome.
...
- Dont open very large PRs without prior discussion.
- Dont ignore CI or pre-commit failures.
- Dont mix unrelated changes in one PR.
- Dont break existing APIs without a good reason and clear migration notes.
- Dont add heavy or optional dependencies to the core install without discussing in an issue.

---

## Getting Help

- **Discussions:** [GitHub Discussions](https://github.com/agentscope-ai/gepaw/discussions)
- **Bugs and features:** [GitHub Issues](https://github.com/agentscope-ai/gepaw/issues)
- **Community:** DingTalk group (see [README](README.md)) and [Discord](https://discord.gg/eYMpfnkG8h)

Thank you for contributing to gepaw. Your work helps make it a better assistant for everyone. 
