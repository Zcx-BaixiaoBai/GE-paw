...
A gepaw tool plugin that enables image generation and editing using OpenAI's GPT Image 2 model.
...
```bash
gepaw plugin install /path/to/gpt-image2
```

Or from ZIP:

```bash
gepaw plugin install gpt-image2-tool.zip
```

## Configuration

1. Start gepaw application
2. Navigate to Agent Settings ?Tools
3. Find the GPT Image 2 tools:
   - `generate_image_gpt` ( icon) - Generate images from text
   - `edit_image_gpt` (?icon) - Edit images with reference images
...
- Ensure the plugin is installed: `gepaw plugin list`
- Check gepaw logs: `~/.gepaw/logs/gepaw.log`
- Restart gepaw after installation
...
- Check file permissions in `~/.gepaw/plugins/`
...
For issues and feature requests, please use the gepaw issue tracker.
