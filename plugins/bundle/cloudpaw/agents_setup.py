...
def register_extra_tools(agent_id: str, extra_tools: dict[str, dict]) -> None:
    ...
    if not extra_tools:
        return
    try:
        from gepaw.config.config import (
            BuiltinToolConfig,
            ToolsConfig,
            load_agent_config,
            save_agent_config,
...
def _build_acp_config(spec: dict[str, Any]) -> Any:
    ...
    """
    try:
        from gepaw.config.config import ACPAgentConfig, ACPConfig
    except ImportError:
...
def _inject_llm_env(env: dict[str, str]) -> None:
    ...
    For iac-code >= 0.1.2: write llm_source: gepaw to settings.yml
    so iac-code reads config directly from gepaw.
    ...
    _write_gepaw_mode_to_settings()
    return


def _write_gepaw_mode_to_settings() -> None:
    """Write llm_source: gepaw to ~/.iac-code/settings.yml."""
    import yaml
    ...
    # Write llm_source: gepaw
    if settings.get("llm_source") != "gepaw":
        settings["llm_source"] = "gepaw"
        try:
...
def ensure_builtin_agents() -> None:
    ...
    try:
        ...
        from gepaw.config.utils import load_config, save_config
        from gepaw.constant import WORKING_DIR
    except ImportError:
        logger.error(
...
def _initialize_agent_workspace(
    ...
    from gepaw.agents.skill_system import get_workspace_skills_dir
...
def _seed_persona_md_files(
    ...
    try:
        from gepaw.agents.prompt import _get_agent_md_dir
...
def _uninstall_plugin_skills() -> None:
    ...
    try:
        from gepaw.agents.skill_system import (
            get_skill_pool_dir,
...
def _uninstall_cloudpaw_env_vars() -> None:
    ...
    try:
        from gepaw.envs import load_envs, save_envs
    except ImportError:
        logger.warning("Cannot import gepaw.envs; env uninstall skipped")
        return
...
def _install_workspace_skills(
    ...
    try:
        ...
        from gepaw.agents.skill_system.store import (
