...
def _check_iac_model_configured() -> bool:
    ...
    try:
        ...
        # If llm_source is gepaw, iac-code uses gepaw's model config
        if "llm_source: gepaw" in content:
            return True
...
def _check_environment_ready() -> (  # pylint: disable=too-many-branches
    ...
    if not ak or not sk:
        issues.append(
            ...
            "     gepaw env set ALIBABA_CLOUD_ACCESS_KEY_SECRET <your-sk>\n"
    ...
    if not _check_iac_model_configured():
        issues.append(
            ...
            "     3.  IAC_CODE_PROVIDER / IAC_CODE_MODEL / "
            "IAC_CODE_API_KEY",
        )
 ...
 **prd.json schema validation FAILED**. Problems found:
{problems}
...
# ---------------------------------------------------------------------------
#
# gepaw v1.1.7b1 `ACPAgentConfig.trusted` is *not* actually honoured by
...
def setup_acp_auto_approve() -> None:
    ...
    """
    try:
        from gepaw.agents.acp.client import ACPHostedClient
...
def setup_tool_and_prompt_hooks() -> (  # pylint: disable=too-many-statements
    ...
    """Monkey-patch gepawAgent to add cloudpaw tools and prompt sections."""
    # IaC operations are delegated to iac-code via the built-in async
    # `delegate_external_agent` tool (gepaw >= v1.1.7b1).  No CloudPaw-side
    # ACP wrapper is required ?the plugin only enables the built-in tool
    # with `async_execution=True` via constants.py.
    try:
        from gepaw.agents.react_agent import gepawAgent
    except ImportError as exc:
        logger.error(
            "Cannot import gepawAgent; tool/prompt hooks skipped: %s",
            exc,
        )
        return

    _original_create_toolkit = gepawAgent._create_toolkit
    _original_build_sys_prompt = gepawAgent._build_sys_prompt
    ...
    _original_interrupt = gepawAgent.interrupt
    ...
    gepawAgent._create_toolkit = _patched_create_toolkit
    gepawAgent._build_sys_prompt = _patched_build_sys_prompt
    gepawAgent.interrupt = _patched_interrupt
    logger.info(
        "Patched gepawAgent with cloudpaw tools, prompt hooks, "
        "and interrupt",
    )
...
def _setup_a2a_query_rewrite() -> None:
    ...
    try:
        ...
        from gepaw.app.runner.command_dispatch import _get_last_user_text
    except ImportError as exc:
        logger.warning(
...
def _patch_stream_task_timeout() -> None:
    ...
    """
    try:
        from gepaw.app._app import agent_app
...
def _patch_mission_master_prompt() -> None:
    ...
    """
    try:
        from gepaw.agents.mission import prompts as mission_prompts
        from gepaw.agents.mission.prompts import (
    ...
    try:
        from gepaw.agents.mission import handler as mission_handler
    ...
    try:
        from gepaw.agents.mission import mission_runner
