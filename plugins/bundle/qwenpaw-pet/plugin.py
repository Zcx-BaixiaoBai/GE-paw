# -*- coding: utf-8 -*-
"""gepaw Pet backend plugin entry point."""
...
# ``gepaw plugin install`` execs this file as a plain module (no
...
from gepaw.plugins.api import PluginApi  # noqa: E402
...
# Logger uses ``gepaw.*`` so messages appear in the project logger
# (``~/.gepaw/gepaw.log``).
...
class gepawPetPlugin:
    ...
    def register(self, api: PluginApi):
        ...
        logger.info("gepaw Pet plugin registered")
    ...
    def _startup(self):
        ...
        the desktop autostart and ``gepaw.startup`` emit going so the
        ...
        try:
            ...
            logger.exception(
                "gepaw Pet: failed to patch AgentRunner; "
        ...
        try:
            ...
            logger.exception(
                "gepaw Pet: failed to patch ApprovalService; "
        ...
        try:
            ...
            logger.info("gepaw Pet startup hook complete")
        except Exception:
            logger.exception("gepaw Pet startup hook failed")

    def _shutdown(self):
        """Notify the desktop, terminate it, and restore the runner patch.

        The pet desktop is treated as a child of gepaw: when gepaw
        exits, the floating window goes with it. ``stop_desktop`` only
        kills a process that this plugin instance spawned (so a desktop
        started independently via ``python -m gepaw_pet_desktop start``
        is left alone), and the whole behaviour can be opted out of via
        ``gepaw_PET_STOP_ON_SHUTDOWN=0``.
        """
        try:
            emit_pet_event("gepaw.shutdown", text="", duration_ms=500)
        except Exception:
            logger.warning(
                "gepaw Pet: shutdown event emit failed",
                exc_info=True,
            )

        try:
            result = stop_desktop(force=True, aggressive=True, grace=5.0)
            logger.info("gepaw Pet: stop_desktop result=%s", result)
        except Exception:
            logger.exception("gepaw Pet: failed to stop desktop process")

        try:
            restore_approval_service()
            restore_agent_runner()
        except Exception:
            logger.exception("gepaw Pet: failed to restore class methods")

        logger.info("gepaw Pet shutdown hook complete")


plugin = gepawPetPlugin()
