...
def _install_from_source(source: Path, *, replace: bool) -> dict[str, object]:
    ...
    from gepaw_pet_desktop import pet_package
...
def build_router() -> APIRouter:
    ...
    def import_pet(payload: ImportPetRequest):
        ...
        try:
            if src.is_dir():
                ...
                tmp_root = Path(
                    tempfile.mkdtemp(prefix="gepaw-pet-import-"),
                )
    ...
    def import_pet_upload(
        ...
        * **Folder upload** ?when multiple files are uploaded, each
        ...
        tmp_root = Path(tempfile.mkdtemp(prefix="gepaw-pet-upload-"))
        ...
        try:
            ...
            if single_zip:
                extract_root = Path(
                    tempfile.mkdtemp(prefix="gepaw-pet-upload-zip-"),
                )
