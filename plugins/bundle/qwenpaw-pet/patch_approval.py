...
def patch_approval_service() -> None:
    ...
    from gepaw.app.approvals.service import ApprovalService
    from gepaw.security.tool_guard.approval import ApprovalDecision
    ...
    async def create_pending_wrapped(self, **kwargs: Any):
        ...
        try:
            ...
            logger.info(
                "gepaw Pet: scheduled approval.pending tool=%s",
                pending.tool_name,
            )
        except Exception:
            logger.warning(
                "gepaw Pet: schedule approval.pending failed",
                exc_info=True,
            )
        return pending
    ...
    async def resolve_request_wrapped(self, request_id: str, decision: Any):
        ...
        try:
            ...
            logger.info(
                "gepaw Pet: scheduled approval decision=%s tool=%s",
                getattr(decision, "value", decision),
                resolved.tool_name,
            )
        except Exception:
            logger.warning(
                "gepaw Pet: schedule approval.resolved failed",
                exc_info=True,
            )
        return resolved
    ...
    async def cancel_all_wrapped(self, root_session_id: str) -> int:
        ...
        if n > 0:
            try:
                ...
                logger.warning(
                    "gepaw Pet: schedule approval.bulk_cancel failed",
                    exc_info=True,
                )
        return n
    ...
    _PATCHED = True
    logger.info(
        "gepaw Pet: patched ApprovalService "
...
def restore_approval_service() -> None:
    ...
    _PATCHED = False
    logger.info("gepaw Pet: restored ApprovalService")
