...
logger = logging.getLogger("gepaw.pet_desktop")
...
# where a previous gepaw run left the pet behind. A user who never
# lets gepaw see the pet (e.g. ``gepaw_PET_AUTOSTART=0`` and the pet
# is not running at startup) will not have their standalone desktop
# killed on gepaw exit. Hard opt-out: ``gepaw_PET_STOP_ON_SHUTDOWN=0``.
...
def _mark_desktop_owned() -> None:
    """Mark the desktop as managed by this gepaw process."""
...
TOKEN_PATH = Path(
    os.environ.get(
        "gepaw_PET_TOKEN_PATH",
        str(Path.home() / ".gepaw-pet/runtime/update-token"),
    ),
)
...
def _headers() -> dict[str, str]:
    ...
    if not token:
        return {}
    return {"X-gepaw-Pet-Token": token}
...
def _spawn_host_port_from_env() -> tuple[str, int]:
    """Host + preferred TCP port for ``gepaw_pet_desktop.app``."""
    url = (os.environ.get("gepaw_PET_DESKTOP_URL") or "").strip()
    if url:
        u = urlparse(url)
        ...
        if (u.scheme or "http").lower() == "https":
            return host, 443
        return host, 8765
    host = (os.environ.get("gepaw_PET_DESKTOP_HOST") or "127.0.0.1").strip()
    host = host or "127.0.0.1"
    port = int(os.environ.get("gepaw_PET_DESKTOP_PORT", "8765"))
    return host, port
...
def _pick_listen_port(host: str, preferred: int) -> int:
    ...
    Set ``gepaw_PET_DESKTOP_STRICT_PORT=1`` to disable fallback (spawn
    may still fail with EADDRINUSE if the port is taken between the probe
    and the child bind ?rare on localhost).

    If ``gepaw_PET_DESKTOP_URL`` pins an explicit ``host:port``, we never
    pick another port ?otherwise the running bridge would not match the
    URL the user configured.
    """
    if os.environ.get("gepaw_PET_DESKTOP_STRICT_PORT", "0") == "1":
        return preferred
    if (os.environ.get("gepaw_PET_DESKTOP_URL") or "").strip():
        return preferred
    ...
    for p in range(preferred + 1, preferred + 128):
        if _tcp_bind_test(host, p):
            logger.info(
                "gepaw Pet: preferred desktop port %s busy on %s; using %s",
                preferred,
                host,
                p,
            )
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        logger.warning(
            "gepaw Pet: desktop using ephemeral port %s on %s",
            ephem,
            host,
        )
        return ephem
...
def _desktop_url_candidates() -> list[str]:
    ...
    explicit = (os.environ.get("gepaw_PET_DESKTOP_URL") or "").strip()
    if explicit:
        return [explicit.rstrip("/")]

    out: list[str] = []
    try:
        from gepaw_pet_desktop import runtime as pet_rt
...
_MISSING_DEPS_HINT = (
    ...
    "gepaw's interpreter). Install into the same environment: "
...
def _spawn_desktop_background() -> tuple[bool, str | None]:
    ...
    Runs ``sys.executable -m gepaw_pet_desktop.app``. The package lives
    next to this plugin and is on the *parent's* ``sys.path`` because
    ``plugin.py`` injects the plugin directory; the child process gets
    that location via ``PYTHONPATH`` so ``python -m gepaw_pet_desktop.app``
...
def _spawn_desktop_background_impl() -> tuple[bool, str | None]:
    ...
    try:
        ...
        # event the plugin emits already carries ``X-gepaw-Pet-Token``
        ...
        cmd: list[str] = [
            sys.executable,
            "-m",
            "gepaw_pet_desktop.app",
            "--host",
            host,
            "--port",
            str(port),
        ]
        scale = os.environ.get("gepaw_PET_DESKTOP_SCALE")
        if scale:
            cmd.extend(["--scale", str(scale)])
        pet_dir = os.environ.get("gepaw_PET_DESKTOP_PET_DIR")
        if pet_dir:
        ...
        # embedded ``gepaw_pet_desktop`` package is importable here).
        # Propagate that path via PYTHONPATH so ``python -m
        # gepaw_pet_desktop.app`` can find the package.
        env = os.environ.copy()
...
def _stop_desktop_skip_reason(*, force: bool) -> str | None:
    if os.environ.get("gepaw_PET_STOP_ON_SHUTDOWN", "1") == "0":
        return "opted out"
...
def stop_desktop(
    *,
    ...
    """Stop the pet desktop process that this gepaw process manages.

    Called from ``_shutdown`` so the floating pet exits together with
    gepaw ?the default mental model is that the desktop is a child
    of gepaw, not a long-running independent service. Users who want
    to keep the pet alive across gepaw restarts can opt out with
    ``gepaw_PET_STOP_ON_SHUTDOWN=0``.

    By default this only acts on a desktop that gepaw has *adopted*
    (``_DESKTOP_OWNED``): either we spawned it, or ``/health`` was
    responding at startup / explicit ``desktop/start`` time. Pass
    ``force=True`` to stop a desktop that gepaw never observed (e.g.
    started just now by another tool while gepaw was already up).
    ...
    if pid == os.getpid():
        logger.warning(
            "gepaw Pet: refusing to stop desktop pid=%s (matches gepaw)",
            pid,
        )
        _clear_desktop_base_url_cache()
        return {"ok": True, "stopped": False, "reason": "pid is gepaw"}
...
def ensure_desktop_available() -> None:
    ...
    this stays quiet. gepaw should never fail because the pet is absent.

    Adoption rule: if a desktop is *already* responding to ``/health`` at
    startup (e.g. left over from a previous gepaw run that crashed
    ...
    processes that the next gepaw run merely "skips spawning".
    ...
    if desktop_health():
        ...
        _reset_desktop_reachability_probe()
        return
    if os.environ.get("gepaw_PET_AUTOSTART", "0") == "0":
        return
...
def start_desktop_interactive() -> dict[str, Any]:
    ...
    Always tries to spawn (ignores ``gepaw_PET_AUTOSTART``). Returns
    ...
    with _SPAWN_LOCK:
        ...
        return _desktop_start_response(
            ...
            hint=(
                "See log file under gepaw pet runtime "
                "(often ~/.gepaw-pet/runtime/pet-desktop.log)."
            ),
        )


def schedule_emit_pet_event(event: str, **payload: Any) -> None:
    """Notify desktop from async gepaw code without blocking the event loop.
...
def emit_pet_event(event: str, **payload: Any) -> None:
    """Send a lifecycle event to gepaw Pet Desktop.

    No-op when the desktop pet is not running. Otherwise fire-and-forget:
    short timeout, no exception escapes into gepaw's main request path.
    """
    if not _desktop_is_reachable():
        logger.debug(
            "gepaw Pet Desktop not running; skip event=%s",
            event,
        )
        return
    ...
    try:
        ...
        if response.status_code >= 400:
            logger.warning(
                "gepaw Pet Desktop POST /event HTTP %s "
                "event=%s detail=%s",
        ...
        _mark_desktop_unreachable()
        logger.warning(
            "gepaw Pet Desktop POST /event failed (url=%s event=%s)",
            base,
            event,
            exc_info=True,
        )
...
def switch_pet_desktop(
    *,
    ...
    try:
        ...
        if response.status_code >= 400:
            logger.warning(
                "gepaw Pet Desktop POST /pet HTTP %s detail=%s",
                response.status_code,
                (response.text or "")[:300],
            )
        return data
    except Exception as exc:
        logger.warning("gepaw Pet Desktop POST /pet failed: %s", exc)
