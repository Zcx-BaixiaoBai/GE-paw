// React and antd are injected by the gepaw console host at runtime;
...
//
// ``gepaw-host.d.ts`` declares the ``window.gepaw`` contract so the
 ...
 * blob keyed ``gepaw-agent-storage`` (sessionStorage takes precedence
 * over localStorage so each tab can pin its own agent). The gepaw API
...
function getSelectedAgentId(): string | null {
  try {
    const raw =
      window.sessionStorage?.getItem("gepaw-agent-storage") ??
      window.localStorage?.getItem("gepaw-agent-storage");
    if (!raw) return null;
...
function PetThumb({ folder }: { folder: string }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const url = getApiUrl(
          `/gepaw-pet/pets/${encodeURIComponent(folder)}/spritesheet`,
        );
...
function PetControlPage() {
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      const [petData, st] = await Promise.all([
        apiGet("/gepaw-pet/pets"),
        apiGet("/gepaw-pet/status"),
      ]);
  ...
  const startDesktop = async () => {
    ...
    try {
      const r = await apiPost("/gepaw-pet/desktop/start", {});
      const h = r?.desktop;
  ...
  const submitImport = async () => {
    ...
    try {
      ...
      const res = await fetch(getApiUrl("/gepaw-pet/import-pet-upload"), {
        method: "POST",
        headers: authHeaders(),
        body: form,
      });
  ...
  const switchTo = async (row: PetRow) => {
    ...
    try {
      const r = await apiPost("/gepaw-pet/switch-pet", { pet_id });
      if (r && r.ok === false) {
...
class gepawPetPlugin {
  readonly id = "gepaw-pet";

  setup(): void {
    const locale = resolvePetLocale();
    window.gepaw.registerRoutes?.(this.id, [
      {
        path: "/plugin/gepaw-pet/pets",
