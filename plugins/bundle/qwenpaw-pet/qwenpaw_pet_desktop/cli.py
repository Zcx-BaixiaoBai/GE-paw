# -*- coding: utf-8 -*-
"""Command-line management for gepaw Pet Desktop."""
...
def command_init(_args: argparse.Namespace) -> int:
    ...
    print(f"gepaw working dir: {runtime.gepaw_working_dir()}")
    ...
    print("")
    print("Install the gepaw backend plugin with:")
    print("  gepaw plugin install ./plugins/gepaw-pet")
    return 0
...
def command_start(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if health.get("ok"):
            print(f"gepaw Pet already running on port {args.port}")
            return 0
    except Exception:
        pass
    ...
    print(f"Started gepaw Pet Desktop (pid {process.pid})")
...
def command_switch(args: argparse.Namespace) -> int:
    ...
    if token:
        headers["X-gepaw-Pet-Token"] = token
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="gepaw-pet")
    ...
    g.add_argument(
        "--pet-id",
        help="Folder name under gepaw pets/ (e.g. snowpaw)",
    )
    p.add_argument("--port", type=int, default=8765)
