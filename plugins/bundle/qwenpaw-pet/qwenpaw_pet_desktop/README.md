# `gepaw_pet_desktop` (package internals)

Importable Python package embedded inside the **gepaw Pet plugin**.
It ships the desktop runtime: a transparent always-on-top Qt pet
window + a local FastAPI bridge on `127.0.0.1:8765` that gepaw (via
...
the package is importable as `gepaw_pet_desktop` from gepaw's
...
| `__main__.py` | `python -m gepaw_pet_desktop` ?delegates to `cli.main`. |
| `cli.py` | CLI subcommands (`python -m gepaw_pet_desktop ...`): `init`, `start`, `stop`, `status`, `install-pet`, `install-default-pet`, `token`, `switch`, `send`. |
...
| `server.py` | FastAPI app factory (`build_app`). Defines `/health`, `/event`, `/state`, `/bubble`, `/pet`. Optional bearer-style token guard via `gepaw_PET_REQUIRE_TOKEN=1` + `X-gepaw-Pet-Token`. |
...
| `sprites.py` | Atlas constants (192208 cells, 89 grid), per-state animation timing, and eventstate mapping table. |
...
| `assets/default-pet/snowpaw/` | The bundled default pet (`pet.json` + `spritesheet.webp`). Loaded via `importlib.resources.files("gepaw_pet_desktop")`, which resolves relative to this folder on disk (no setuptools install required). |

## Runtime layout

`runtime.home_dir()` defaults to `~/.gepaw-pet/` (override with
`gepaw_PET_HOME`) and contains:
...
~/.gepaw-pet/runtime/
  ...
  pet-desktop.log    # stdout/stderr of detached `gepaw-pet start`
```

Pet **packages** live in a separate location resolved by
`runtime.gepaw_working_dir()`, with this precedence:

1. `gepaw_WORKING_DIR`
2. `COPAW_WORKING_DIR`
3. `from gepaw.constant import WORKING_DIR` (if gepaw is installed)
...
send `X-gepaw-Pet-Token: <token from runtime/update-token>` on
mutating endpoints (the bundled gepaw plugin reads the token file
automatically). To disable the check (e.g. for local development), set
`gepaw_PET_REQUIRE_TOKEN=0`.
...
These two are equivalent and bypass the `gepaw-pet start` daemon
wrapper (no PID file, no log redirection ?useful while iterating):

```bash
python -m gepaw_pet_desktop.app --port 8765 --scale 0.58
python -m gepaw_pet_desktop                 # ?cli.main, takes subcommands
```
