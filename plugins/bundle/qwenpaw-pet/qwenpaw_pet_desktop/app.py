...
def run_http_server(
    ...
    config = uvicorn.Config(
        ...
        host=host,
        port=port,
        log_level=os.environ.get("gepaw_PET_LOG_LEVEL", "warning"),
    )
...
def run_desktop(pet_dir: Path, host: str, port: int, scale: float) -> int:
    ...
    # Local gepaw plugin always calls ``127.0.0.1`` even when the
    ...
    # SIGTERM is what the gepaw plugin sends on shutdown (see
    ...
    try:
        ...
        # immediately sees ``running=False`` and the next gepaw start
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run gepaw Pet Desktop")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not runtime.try_acquire_instance_lock():
        logger.info(
            "Another gepaw Pet Desktop instance is already running; exiting",
        )
        return 0
    try:
