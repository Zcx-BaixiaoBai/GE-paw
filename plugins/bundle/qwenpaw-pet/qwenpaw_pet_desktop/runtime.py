...
def home_dir() -> Path:
    return Path(
        os.environ.get(
            "gepaw_PET_HOME",
            str(Path.home() / ".gepaw-pet"),
        ),
    )


def gepaw_working_dir() -> Path:
    """Return gepaw's working directory using gepaw's precedence."""
    explicit = os.environ.get("gepaw_WORKING_DIR") or os.environ.get(
        "COPAW_WORKING_DIR",
    )
    if explicit:
        return Path(explicit).expanduser().resolve()
    try:
        from gepaw.constant import WORKING_DIR  # type: ignore
...
def current_process_status() -> dict[str, Any]:
    ...
    return {
        ...
        "home": str(home_dir()),
        "gepawWorkingDir": str(gepaw_working_dir()),
