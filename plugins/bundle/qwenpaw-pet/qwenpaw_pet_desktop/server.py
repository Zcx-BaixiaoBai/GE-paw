# -*- coding: utf-8 -*-
"""Local HTTP bridge for gepaw lifecycle events."""
...
def _token_required() -> bool:
    """Whether the local bridge enforces ``X-gepaw-Pet-Token``.
    ...
    setting ``gepaw_PET_REQUIRE_TOKEN=0``.
    """
    return os.environ.get("gepaw_PET_REQUIRE_TOKEN", "1") != "0"
...
def build_app(
    ...
    app = FastAPI(title="gepaw Pet Desktop")
    ...
    async def event(
        payload: PetEvent,
        request: Request,
        x_gepaw_pet_token: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _check_token(x_gepaw_pet_token)
