# gepaw Pet (plugin + desktop runtime)

Full-stack gepaw plugin: backend hooks, console UI sidebar, **and** the
desktop pet runtime itself (`gepaw_pet_desktop/`). Installing this
plugin gives you everything: the Qt floating pet window, the local HTTP
bridge on `127.0.0.1:8765`, and the gepaw side hooks that emit
...
plugins/gepaw-pet/
 ...
 router.py              # plugin HTTP routes under /api/gepaw-pet/...
 ...
 gepaw_pet_desktop/   # the Qt + FastAPI desktop runtime (embedded)
...
time, so `gepaw_pet_desktop` is importable from gepaw's Python
process without any `pip install` step. See
[`gepaw_pet_desktop/README.md`](gepaw_pet_desktop/README.md) for
...
- **From the gepaw console** ?open the plugin management page and
  install `gepaw-pet` like any other gepaw plugin. Three input
  shapes are accepted:
...
- **From the shell** ?run:
  ...
  ```bash
  gepaw plugin install ./plugins/gepaw-pet
  ```

> [!IMPORTANT]
> **Restart gepaw after install** ?the backend only picks up new
...
gepaw's interpreter needs these packages on its `sys.path` (declared
...
If your gepaw install does not auto-resolve plugin dependencies,
install them manually into the same environment:

```bash
pip install -r plugins/gepaw-pet/requirements.txt
```

PySide6-Essentials wheels exist only for **Python 3.10?.13**. On 3.14
the pet window cannot start; gepaw itself will still run, the pet
...
```python
subprocess.Popen(
    [sys.executable, "-m", "gepaw_pet_desktop.app",
     "--port", "8765"],
...
python -m gepaw_pet_desktop.app --port 8765 --scale 0.58

# CLI subcommands (daemonized: PID file + log redirect)
python -m gepaw_pet_desktop start
python -m gepaw_pet_desktop status
python -m gepaw_pet_desktop stop
python -m gepaw_pet_desktop switch --pet-id snowpaw
...
Autostart can be disabled with `gepaw_PET_AUTOSTART=0`.
...
`~/.copaw/plugins/gepaw-pet/dist/index.js` and refresh the console.
...
host at runtime (`window.gepaw.host.React`, `host.antd`,
`host.getApiUrl`, `host.getApiToken`). The shape of this contract is
declared in `frontend/src/gepaw-host.d.ts`. If the console host
...
The UI adds a sidebar page **Pet** (`/plugin/gepaw-pet/pets`): lists
pets under `<WORKING_DIR>/pets`, **Start desktop pet** (calls `POST
/api/gepaw-pet/desktop/start`), **Switch** (hot-switch via `POST
...
`POST /api/gepaw-pet/import-pet-upload`, then validated and copied
into `<WORKING_DIR>/pets/<id>/`. The source (or the unzipped archive)
must contain `pet.json` and the spritesheet referenced by it
(15361872 webp); a single top-level subfolder is also accepted, which
is what macOS Finder's "Compress" produces.

## gepaw plugin HTTP routes

```text
GET  /api/gepaw-pet/status
GET  /api/gepaw-pet/pets
GET  /api/gepaw-pet/pets/{folder}/spritesheet
POST /api/gepaw-pet/desktop/start
POST /api/gepaw-pet/switch-pet
POST /api/gepaw-pet/import-pet          # JSON body ?server-side path
POST /api/gepaw-pet/import-pet-upload   # multipart/form-data ?browser
POST /api/gepaw-pet/emit-test
```
...
`spritesheet.webp`, 15361872). The manifest `id` is checked against
...
`X-gepaw-Pet-Token: <runtime/update-token>` by default ?the bundled
gepaw plugin reads the token file automatically; standalone clients
must do the same. Set `gepaw_PET_REQUIRE_TOKEN=0` to disable the check
...
Row layout (driven by `gepaw_pet_desktop/sprites.py`):
...
(`gepaw_pet_desktop/assets/default-pet/snowpaw/`).
...
installed and cached under `~/.gepaw-pet/cache/snowpaw-spritesheet.webp`
for subsequent runs. Override the source with `gepaw_PET_SNOWPAW_URL`
...
`gepaw_pet_desktop/assets/default-pet/snowpaw/` and the bundled copy
...
- `register_startup_hook` ?patches `AgentRunner.query_handler` and the
  ...
  `gepaw.startup`. Patch failures (e.g. an upstream rename of
  `AgentRunner` / `ApprovalService`) are reported via
  `logger.exception` so a broken plugin install does not stay silently
  dead.
- `register_shutdown_hook` ?emits `gepaw.shutdown`, terminates the
  pet desktop process that this gepaw process has adopted (either
  autostarted by the plugin or already healthy on startup /
  `desktop/start`; controlled by `gepaw_PET_STOP_ON_SHUTDOWN`), and
  restores the patched class methods. So when the user exits gepaw
  the floating pet exits with it, including the case where the pet was
  a leftover from a previous gepaw run.
- `register_http_router` ?mounts `router.py` under `/gepaw-pet`.

## Environment variables

| Var | Purpose | Default |
| --- | --- | --- |
| `gepaw_PET_DESKTOP_URL` | Full base URL for the plugin ?desktop HTTP bridge. If set, port auto-fallback is **disabled** (URL and listener must agree). | unset ?derive from host + port below |
| `gepaw_PET_DESKTOP_HOST` | Bind address when spawning (`--host`) | `127.0.0.1` |
| `gepaw_PET_DESKTOP_PORT` | Preferred port when spawning; if busy, the plugin scans upward (unless `DESKTOP_URL` is set or `STRICT` is on) | `8765` |
| `gepaw_PET_DESKTOP_STRICT_PORT` | `1` ?never auto-pick another port (fail with EADDRINUSE if taken) | `0` |
| `gepaw_PET_DESKTOP_SCALE` | Spawn-time scale (e.g. `0.58`) | unset |
| `gepaw_PET_DESKTOP_PET_DIR` | Spawn-time pet folder override | unset |
| `gepaw_PET_TOKEN_PATH` | Path to the local update token | `~/.gepaw-pet/runtime/update-token` |
| `gepaw_PET_REQUIRE_TOKEN` | `0` ?desktop *skips* the token check on mutating endpoints (anything else, including unset, enforces it) | `1` |
| `gepaw_PET_AUTOSTART` | `0` ?plugin will not spawn the desktop | `1` |
| `gepaw_PET_STOP_ON_SHUTDOWN` | `0` ?leave the pet desktop running after gepaw exits. Default: terminate any pet desktop gepaw has adopted (either by autostarting it or by seeing it healthy at startup / explicit `desktop/start`). | `1` |
| `gepaw_PET_HOME` | Runtime dir (PID file, log, **cache**, token) | `~/.gepaw-pet/` |
| `gepaw_PET_SNOWPAW_URL` | CDN URL for snowpaw's `spritesheet.webp` (downloaded once on first install) | Alicdn-hosted default |
| `gepaw_WORKING_DIR` / `COPAW_WORKING_DIR` | Where `pets/` lives | falls back to `~/.copaw` then `~/.gepaw` |

### Pet desktop log / common errors

`~/.gepaw-pet/runtime/pet-desktop.log` captures stderr from the spawned
desktop process.

- **`ModuleNotFoundError: No module named 'PySide6'`** ?install Qt into the
  **same** Python as gepaw:
...
- **`[Errno 48] address already in use` (uvicorn)** ?something else is bound
  ...
  that process, set `gepaw_PET_DESKTOP_PORT` to a free port, or unset
  `gepaw_PET_DESKTOP_URL` and let the plugin auto-pick the next free port
  after the preferred one.

The effective listen URL is mirrored under
`~/.gepaw-pet/runtime/desktop-bridge.json` (`url` field) so the plugin can
