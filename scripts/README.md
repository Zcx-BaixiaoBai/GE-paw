...
- Builds the console frontend (`console/`), copies `console/dist` to `src/gepaw/console/dist`, then builds the wheel. Output: `dist/*.whl`.
...
- Default tag: `gepaw:latest`. Uses `deploy/Dockerfile` (multi-stage: builds console then Python app).
- Example: `bash scripts/docker_build.sh myreg/gepaw:v1 --no-cache`.
