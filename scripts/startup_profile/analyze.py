...
# -*- coding: utf-8 -*-
"""
gepaw Startup Performance Analyzer
...
def parse_import_log(log_file):
    ...
    # Filter gepaw and third-party
    gepaw = [i for i in imports if "gepaw" in i["package"]]
    third_party = [
        i
        for i in imports
    ...
    gepaw.sort(key=lambda x: x["cumulative_ms"], reverse=True)
    third_party.sort(key=lambda x: x["cumulative_ms"], reverse=True)

    total_gepaw = sum(
        i["cumulative_ms"] for i in gepaw if i["package"].count(".") == 1
    )
    total_third_party = sum(i["cumulative_ms"] for i in third_party[:10])

    print(
        f"   ?{len(imports)} imports, {len(gepaw)} gepaw, {len(third_party)} third-party",
    )
    ...
    return {
        ...
        "summary": {
            "total_ms": total_gepaw + total_third_party,
            "total_gepaw_ms": total_gepaw,
...
def parse_execution_trace(trace_file):
    ...
    try:
        ...
        gepaw_funcs = [f for f in functions if "gepaw" in f["function"]]

        print(f"   ?{len(functions)} functions, {len(gepaw_funcs)} gepaw")

        return {
            "gepaw_functions": gepaw_funcs[:50],
...
def main():  # pylint: disable=too-many-statements
    """Main function."""
    print("=" * 60)
    print(" gepaw Startup Performance Analyzer")
    print("=" * 60)
    print()
