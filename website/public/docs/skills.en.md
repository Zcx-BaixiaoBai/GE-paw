...
- **Working directory:** Edit skill files directly under `$gepaw_WORKING_DIR`
  (default `~/.gepaw`), including `$gepaw_WORKING_DIR/skill_pool/` and each
  workspace's `$gepaw_WORKING_DIR/workspaces/{agent_id}/skills/`.
...
gepaw skills are organized in two layers:

- **Skill Pool:** Shared local repository at `$gepaw_WORKING_DIR/skill_pool/`
  (default `~/.gepaw/skill_pool/`).
- **Workspace Skills:** The local runtime copy at
  `$gepaw_WORKING_DIR/workspaces/{agent_id}/skills/`
  (default `~/.gepaw/workspaces/{agent_id}/skills/`).
...
- **Conflict handling:** If save, import, upload, or broadcast would land on a
  name that already exists, gepaw returns a conflict instead of silently
...
1. **Import built-ins**.
   ...
   | **QA_source_index**           | Internal gepaw source/doc index skill for quickly mapping keywords to source paths and local docs.                            | Built-in                                                       |
   | **cron**                      | Scheduled jobs. Create, list, pause, resume, or delete jobs via `gepaw cron` or Console **Control ?Cron Jobs**.              | Built-in                                                       |
   ...
   | **guidance**                  | Answer gepaw installation and configuration questions by consulting local docs first.                                         | Built-in                                                       |
   ...
   **out-of-date**. Use **Update Built-in Skills** to add missing built-ins
   ...
   [CLI](./cli) (`gepaw cron`) or Console **Control ?Cron Jobs**:

   - Create: `gepaw cron create --type agent --name "xxx" --cron "0 9 * * *" ...`
   - List: `gepaw cron list`
   - Check state: `gepaw cron state <job_id>`
...
6. **Manual filesystem changes**.
   You can place folders directly under `$gepaw_WORKING_DIR/skill_pool/`, but this is not
...
`$gepaw_WORKING_DIR/workspaces/{agent_id}/skills/`. Those copies are what the agent
...
`$gepaw_WORKING_DIR/workspaces/{agent_id}/skills/`, including using gepaw itself to help
...
Create a directory under `$gepaw_WORKING_DIR/workspaces/{agent_id}/skills/`, add a
...
variables, declare them in `metadata.requires`; gepaw exposes them as
...
workspace and channel, gepaw injects that config into the runtime environment
...
The full config is always available as `gepaw_SKILL_CONFIG_<SKILL_NAME>`
...
- `timeout` is not in `requires.env`, so it is only available via the full
  JSON below.
- `gepaw_SKILL_CONFIG_MY_SKILL` always contains the full JSON config.
...
For `requires` metadata, the parser checks keys in order: `metadata.openclaw.requires` ?`metadata.gepaw.requires` ?`metadata.requires`. The first one found is used.
