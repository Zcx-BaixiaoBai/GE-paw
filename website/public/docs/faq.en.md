...
### gepaw vs OpenClaw: Feature Comparison

Please check the [Comparison](/docs/comparison) page for detailed feature comparison.

### How to install gepaw

gepaw supports multiple installation methods. See
[Quick Start](https://gepaw.agentscope.io/docs/quickstart) for details:
...
To update gepaw, use the method matching your installation type:
...
5. If using the Desktop app (Windows `.exe` or macOS `.zip`), you currently need to uninstall and reinstall:
   - Uninstall gepaw on your machine
   - Download the latest build: https://gepaw.agentscope.io/downloads
   - Reinstall

After upgrading, restart the service with `gepaw app`.

If you previously used CoPaw, upgrading to gepaw only requires downloading the latest gepaw. No extra migration is needed; your configuration, memory, skills, and other data from the CoPaw era continue to work.

### How to initialize and start gepaw service
...
[Quick Start](https://gepaw.agentscope.io/docs/quickstart).

### Port 8088 conflict on Windows

On Windows, Hyper-V and WSL2 may reserve certain port ranges, which can conflict
with gepaw's default port **8088**. This affects all installation methods
...
- gepaw fails to start, or browser cannot connect to `http://127.0.0.1:8088/`
...
1. Run `gepaw app --port 8090` from a terminal instead
...
>  **Warning**: This changes system-wide port configuration. Only do this if
> you understand the implications.

### APITimeoutError when running gepaw in WSL2 (NAT mode)

When running gepaw inside WSL2 with NAT networking (especially when a VPN is
...
After this, gepaw should be able to communicate with model providers
normally.

### Open-source repository

gepaw is open source. Official repository:
`https://github.com/agentscope-ai/gepaw`

### Where to check latest version upgrade details

See the site [Release notes](https://gepaw.agentscope.io/release-notes/?lang=en)
or gepaw GitHub [Releases](https://github.com/agentscope-ai/gepaw/releases).

### How to configure models

In Console, go to **Settings ?Models** to configure. See the
[Models](https://gepaw.agentscope.io/docs/models) doc for details:
...
You can also use `gepaw models` for setup, downloads, and switching. See
[CLI ?Models and environment variables ?gepaw models](https://gepaw.agentscope.io/docs/cli#gepaw-models).

### How to use gepaw-Flash series models

gepaw-Flash is a family of models tuned by the gepaw team for gepaw's core
...
gepaw-Flash models are currently open-sourced on
...
All built-in local providers in gepaw can be used with gepaw-Flash models:

#### gepaw Local (llama.cpp)

In the gepaw Local model interface, simply choose a gepaw-Flash model to
...
> gepaw Local is still in beta. Compatibility across different devices and
> runtime stability are still being improved. If you run into issues while
> using it, please open an issue on GitHub.
> If gepaw Local does not work properly in your environment, we recommend
> deploying gepaw-Flash with Ollama or LM Studio first.

#### Ollama

1. Download a quantized gepaw-Flash model from
   ...
   [gepaw-Flash-4B-Q4_K_M](https://www.modelscope.cn/models/AgentScope/gepaw-Flash-4B-Q4_K_M).
...
Create a text file named `gepaw-flash.txt` with the following contents. Replace
`/path/to/your/gepaw-xxx.gguf` with the absolute path to the `.gguf` file in
the gepaw-Flash model repository you downloaded:
...
4. In gepaw model settings, choose the Ollama provider, then automatically load
   the model on the Models page.

#### LM Studio

1. Follow step 1 in the Ollama section above to download an appropriate
   quantized gepaw-Flash model.
...
4. In gepaw model settings, choose the LM Studio provider, then automatically
   load the model on the Models page.

### When using models deployed with Ollama / LM Studio, why can't gepaw complete multi-turn interactions, complex tool calls, or remember earlier instructions?

In most cases, this is not a gepaw bug. The root cause is usually that the
model's context length is configured too small.

When you deploy a local model with Ollama or LM Studio, if the model's
`context length` is too low, gepaw may show problems such as:
...
- Before running gepaw, set the model's `context length` to **at least 32K**
- For more complex tasks, frequent tool calls, or longer conversations, you
  may need a value **higher than 32K**

>  **Before running gepaw, you must set the context length to 32K or higher**
>
> For local models deployed with Ollama or LM Studio, gepaw typically needs a
...
The easiest way to create a cron job is to talk to gepaw in the channel where you want the results. For example, say: Create a scheduled task that reminds me to drink water every five minutes.?You can then see the enabled job in Console.

If a scheduled task does not run as expected, try the following:

1. Confirm that the gepaw service is running.
...
6. To verify that the task was created and can run, click **Execute Now**. If it works, you should see the reply in the target channel. You can also ask gepaw: Trigger the drink water reminder?task I just created.?
...
[Skills](https://gepaw.agentscope.io/docs/skills).

### How to configure MCP

Go to **Agent -> MCP** in Console. You can enable/disable/delete/create MCP
clients there. See [MCP](https://gepaw.agentscope.io/docs/mcp).
...
gepaw supports API keys obtained via DashScope Coding Plan. If it still fails,
please check:
...
[issue](https://github.com/agentscope-ai/gepaw/issues) in the gepaw GitHub
...
Please upload that file (e.g. `/var/folders/.../gepaw_query_error_qzbx1mv1.json`)
and also provide your current model provider, model name, and gepaw version.
