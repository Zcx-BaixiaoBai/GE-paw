...
This document will guide you through using the RESTful API to interact with gepaw Agents.

> **Protocol Details**: gepaw's API is based on an extension of the AgentScope Runtime protocol. For more details, see:
> [AgentScope Runtime Protocol Documentation (English)](https://runtime.agentscope.io/en/protocol.html)

>  **Security Warning**:
> If your gepaw instance is **exposed to the public internet**, strongly recommend enabling [Web Login Authentication](./security#web-authentication)!
...
gepaw provides a RESTful API interface that allows you to interact with Agents via HTTP requests. Through the API, you can:
...
- This is designed for local development and CLI tools (`gepaw`) convenience
...
data: {"sequence_number":2,"object":"response","status":"in_progress","output":[{"role":"assistant","content":[{"type":"text","text":"Hello! I'm gepaw..."}]}],...}
...
gepaw automatically manages conversation context through `session_id` and `user_id`. Simply use the same `session_id` across different requests, and the system will automatically save and load conversation history:
...
If [Web Login Authentication](./security#web-authentication) is enabled (`gepaw_AUTH_ENABLED=true`), all API requests require an authentication token.

#### Register Account

**First-time setup requires registering an admin account** (gepaw uses single-user mode):
...
# Or use gepaw_SECRET_DIR environment variable
rm "${gepaw_SECRET_DIR}/auth.json"

# Restart gepaw and re-register
gepaw app
```
...
You can also auto-create an account via environment variables when starting gepaw:
...
- **Multiple Tokens**:
  -  Each login creates a new token; old tokens are NOT automatically revoked
...
Remove the `-e gepaw_AUTH_ENABLED=true` parameter:
...
Verify gepaw service is running:
...
3. Submit an [Issue](https://github.com/agentscope-ai/gepaw/issues) on GitHub
