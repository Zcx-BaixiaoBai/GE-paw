...
gepaw provides a plugin system that allows users to extend gepaw's functionality.
...
**Note**: Plugin operations can only be performed when gepaw is offline.
...
| `min_version`    | `string`           | no       | Minimum gepaw version required. Defaults to `0.1.0`.                                                                                                                     |
...
**Notes**: `window.gepaw.host` provides the following shared libraries ?plugins do not need to bundle them:
...
**`window.gepaw.modules`**: At startup, the application auto-registers all modules under `src/pages/` into this object. Plugins can access and replace internal exports by module.

>  **Warning**: The module structure inside `modules` is not maintained as a public API and may change across versions. Always verify compatibility before use.
...
Let's say you want to initialize a monitoring service when gepaw starts.
...
class CustomGreetingPlugin {
  ...
  setup(): void {
    ...
    mod.configProvider.getGreeting = () => "Hello! I'm customized gepaw ";
...
and is served by the same FastAPI app as gepaw's core API, so it
...
1. Confirm plugin is installed and restart gepaw
...
1. **Only install trusted plugins**: Plugin code executes in the gepaw process
...
4. **Offline operations**: Plugin install/uninstall requires gepaw to be offline
...
For plugins that need to modify gepaw behavior (like custom commands), you can use monkey patching:
...
### Q: What gepaw APIs can plugins access?
...
### Q: Can plugins modify gepaw's core behavior?
...
A tool plugin that adds OpenAI's GPT Image 2 image generation capability to gepaw agents.

**Requirements:**

- Minimum gepaw version: `1.1.5`

**Installation:**

```bash
# Clone the gepaw repository (if not already cloned)
...
1. After installation, restart gepaw
