...
gepaw supports **ACP (Agent Client Protocol)** in two complementary ways:

1. **gepaw using ACP as a Tool**: gepaw connects to external ACP runners and uses them as delegated collaborators
2. **gepaw as an ACP Server**: external clients connect to gepaw over ACP

This page explains both modes and the scenarios each one fits best.

---

## gepaw using ACP as a Tool

In this mode, gepaw acts as an **ACP client / orchestrator**, connecting to **configured and enabled external ACP runners** and bringing them into the current session as delegated collaboration capabilities.

The actual entry point for this mode is the built-in tool `delegate_external_agent`. It is intended for scenarios where gepaw needs to collaborate with other ACP-capable external agent runtimes, such as the built-in examples `opencode`, `qwen_code`, `claude_code`, and `codex`. For more ACP-compatible agents, see the official ACP agent list and integration guide: <https://agentclientprotocol.com/get-started/agents>. In other words, gepaw does not directly talk to arbitrary external agents. It talks to runners that have been registered in ACP configuration, and it starts, continues, responds to, and closes delegated collaboration sessions through them.

### What this mode does

In this mode, gepaw uses the built-in `delegate_external_agent` tool to:
...
Conceptually, this lets gepaw treat an external agent as a collaborative, tool-like capability while keeping gepaw as the primary orchestrator of the main conversation.
...
Once the command-line side is ready, you can configure a custom runner in gepaw or collaborate with one of the built-in runners directly.
...
When an external ACP runner asks for permission, gepaw does **not** decide on the user's behalf.
...
This keeps delegated ACP execution aligned with the same user-controlled safety model used elsewhere in gepaw.

### When to use ACP as a Tool

Use this mode when:

- you want gepaw to collaborate with another agent runtime
- you have a specialized ACP-compatible external runner for a certain class of tasks
- you want gepaw to remain the primary orchestrator while delegating part of the work outward

### ACP Tool vs MCP

ACP as a Tool and MCP solve different problems:

- **MCP** connects gepaw to external services and tool servers
- **ACP as a Tool** connects gepaw to an external **agent** runtime
...
## gepaw as an ACP Server

In this mode, gepaw exposes itself as an [Agent Client Protocol (ACP)](https://github.com/agentclientprotocol/python-sdk) compliant agent service over stdio JSON-RPC. External clients, such as [Zed](https://zed.dev), [OpenCode](https://github.com/nicholasgasior/opencode), or any ACP-compatible editor, can connect to gepaw via the `gepaw acp` command and interact with it programmatically.

### Quick Start

```bash
# Start gepaw as an ACP agent
gepaw acp
...
2. **WORKING_DIR config**: read `agents.active_agent` from `config.json` inside `WORKING_DIR` (default `~/.gepaw`, or `~/.copaw` for legacy installations; overridable via the `gepaw_WORKING_DIR` environment variable)
...
| Aspect               | gepaw as an ACP Server                      | gepaw using ACP as a Tool                            |
| -------------------- | --------------------------------------------- | ------------------------------------------------------ |
| gepaw's role       | Server / target agent                         | Client / orchestrator                                  |
| Connection direction | External client connects to gepaw           | gepaw connects to an external runner                 |
| Main purpose         | Let editors or external clients drive gepaw | Let gepaw delegate work to another agent             |
| Typical entry point  | `gepaw acp`                                 | Delegation tool + ACP runner configuration             |
...
ACP in gepaw is not just one feature. It supports both directions:

- **Expose gepaw outward** as an ACP server
- **Reach outward from gepaw** to external ACP agents as delegated tools

If you are integrating gepaw into another client, start with **ACP Server**.
If you want gepaw to coordinate with another agent runtime, use **ACP as a Tool**.
