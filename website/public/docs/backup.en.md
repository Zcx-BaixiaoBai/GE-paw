...
**Backup & Restore** lets you create snapshots of your gepaw instance: visually create, export, import, and restore your entire agent environment. It is designed for scenarios such as **rolling back before a version upgrade, migrating between machines, or keeping a safety net before risky changes**.
...
A backup is a single zip file (stored at `~/.gepaw.backups/<backup_id>.zip`) that may contain up to four kinds of content:

| Module               | Path                                | Actual content                                                                                                                                                                        |
| -------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Agent workspaces** | `~/.gepaw/workspaces/<agent_id>/` | Every file inside each agent's workspace, e.g. persona files, memory, skills, chat history, and channel configs (including channel credentials such as `bot_token` and `app_secret`). |
| **Global settings**  | `~/.gepaw/config.json`            | Runtime parameters, security rules, and other global settings.                                                                                                                        |
| **Skill pool**       | `~/.gepaw/skill_pool/`            | The globally shared skill repository.                                                                                                                                                 |
| **Secrets**          | `~/.gepaw.secret/`                | **LLM provider configuration (including API keys)**, plus environment variables used by tools and skills.                                                                             |
...
A backup ID has the format `gepaw-<version>-<timestamp>-<short8>`, which makes it easy to identify the source version and creation time across machines.
...
2. Tick what you need:
   ...
   - **Secrets**: whether to include the secrets (i.e. the `~/.gepaw.secret/` directory). Off by default; turning it on shows the same red sensitive-content warning.
...
>  Restore is **irreversible**. Read the "Pre-restore backup" section before you proceed.

### Pre-restore backup

![Create a pre-restore backup](https://img.alicdn.com/imgextra/i3/O1CN01MwWjoy24qx3N2wfoR_!!6000000007443-2-tps-861-314.png)

When you click the **Restore** button on any backup row, gepaw first opens the **Pre-restore backup** dialog:
...
- **Pick agents one by one**: restore only the agents you tick; agents outside the restore scope are kept as they are. You can also specify the default storage location for newly added agents at restore time; if unspecified, they go to `~/.gepaw/workspaces/<agent_id>/`.
...
| Item             | Path / Default                 |
| ---------------- | ------------------------------ |
| Backup directory | `~/.gepaw.backups/`          |
...
Add `-v gepaw-backups:/app/working.backups` to your `docker run` command:
...
A: gepaw shows an overwrite confirmation; confirm it to continue importing and overwrite the existing backup.
