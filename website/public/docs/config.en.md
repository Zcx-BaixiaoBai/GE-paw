...
From **v0.1.0**, gepaw supports **multi-agent**. Configuration is split into two layers:
...
The default working directory is `~/.gepaw`. After running `gepaw init`, the complete structure looks like:

```
$gepaw_WORKING_DIR/                      # Default ~/.gepaw
...
$gepaw_SECRET_DIR/                       # Default ~/.gepaw.secret
 providers.json                       # Model provider config and API keys
 envs.json                            # Environment variables
```

> **Path explanation:** `$gepaw_WORKING_DIR` and `$gepaw_SECRET_DIR` are environment variables, with default values of `~/.gepaw` and `~/.gepaw.secret` respectively. They can be customized via environment variables, see "Environment Variables" section below.

### Directory Explanation

**Global Directory (`~/.gepaw/`)**
...
**Agent Workspace (`~/.gepaw/workspaces/{agent_id}/`)**
...
> **Persona files:** Agent behavior and personality are defined by persona files. Running `gepaw init` automatically creates template files based on your chosen language (`zh` / `en` / `ru`). For detailed explanation and management, see [Agent Persona](./persona).
...
| -------------------------- | ------------------- | ----------------------------------------------------------------------------------------------------------- |
| `gepaw_WORKING_DIR`      | `~/.gepaw`        | Working directory root path                                                                                 |
| `gepaw_SECRET_DIR`       | `~/.gepaw.secret` | Sensitive data directory (stores `providers.json` and `envs.json`). Docker default is `/app/working.secret` |
| `gepaw_CONFIG_FILE`      | `config.json`       | Config file name (relative to `gepaw_WORKING_DIR`)                                                        |
| `gepaw_HEARTBEAT_FILE`   | `HEARTBEAT.md`      | Heartbeat file name (relative to agent workspace)                                                           |
| `gepaw_JOBS_FILE`        | `jobs.json`         | Cron jobs file name (relative to agent workspace)                                                           |
| `gepaw_CHATS_FILE`       | `chats.json`        | Conversation history file name (relative to agent workspace)                                                |
| `gepaw_TOKEN_USAGE_FILE` | `token_usage.json`  | Token usage record file name (relative to agent workspace)                                                  |

**Other configuration:**

| Variable                             | Default         | Description                                                                 |
| ------------------------------------ | --------------- | --------------------------------------------------------------------------- |
| `gepaw_LOG_LEVEL`                  | `info`          | Log level (`debug` / `info` / `warning` / `error` / `critical`)             |
| `gepaw_MEMORY_COMPACT_THRESHOLD`   | `100000`        | Character threshold to trigger memory compaction                            |
| `gepaw_MEMORY_COMPACT_KEEP_RECENT` | `3`             | Number of recent messages to keep after compaction                          |
| `gepaw_MEMORY_COMPACT_RATIO`       | `0.7`           | Threshold ratio for triggering compaction (relative to context window size) |
| `gepaw_CONSOLE_STATIC_DIR`         | _(auto-detect)_ | Console frontend static files path                                          |

**Security & Authentication:**

| Variable                     | Default | Description                                        |
| ---------------------------- | ------- | -------------------------------------------------- |
| `gepaw_AUTH_ENABLED`       | `false` | Whether to enable Web console login authentication |
| `gepaw_AUTH_USERNAME`      | -       | Admin username for auto-registration (optional)    |
| `gepaw_AUTH_PASSWORD`      | -       | Admin password for auto-registration (optional)    |
| `gepaw_TOOL_GUARD_ENABLED` | `true`  | Whether to enable tool guard                       |
| `gepaw_SKILL_SCAN_MODE`    | `warn`  | Skill scanning mode (`block` / `warn` / `off`)     |
...
```bash
export gepaw_WORKING_DIR=/home/me/my_gepaw
gepaw app
```
...
1. **Global config** - `~/.gepaw/config.json` (providers, environment variables, agent list)
2. **Agent config** - `~/.gepaw/workspaces/{agent_id}/agent.json` (per-agent settings)
...
| `last_api.host`       | string \| null | `null`              | Host address from last `gepaw app` start                        |
| `last_api.port`       | int \| null    | `null`              | Port from last `gepaw app` start                                |
...
| `workspace_dir` | string | No       | Workspace path (optional, defaults to `$gepaw_WORKING_DIR/workspaces/{id}`) |
...
> **Model provider configuration** is stored in `$gepaw_SECRET_DIR/providers.json` (default `~/.gepaw.secret/providers.json`).
> **Environment variables** are stored in `$gepaw_SECRET_DIR/envs.json` (default `~/.gepaw.secret/envs.json`).

### Agent config (agent.json)

Each agent has an independent `agent.json` in its workspace directory (`~/.gepaw/workspaces/{agent_id}/`) that stores all of its configuration (channels, tools, heartbeat, MCP, security, etc.). This allows different agents to have completely different configurations without interfering with each other.
...
gepaw needs an LLM provider to work. You can set it up in three ways:

- **`gepaw init`** ?interactive wizard, the easiest way
...
| ---------------------------------- | ------------------------ | --------------------------------------------------- | -------------- |
| gepaw Local                      | `gepaw-local`          | _(local)_                                           | _(none)_       |
...
> **Tip:** Run `gepaw init` and follow the prompts ?it will list available
...
> gepaw does not verify whether the key is correct or has sufficient quota ?
...
- **`gepaw init`** ?prompts "Configure environment variables?" during setup
...
> keys) are valid. gepaw only stores and injects them ?it does not verify
> correctness.
...
| Directory                                  | Purpose                                           |
| ------------------------------------------ | ------------------------------------------------- |
| `~/.gepaw/skill_pool/`                   | Local shared pool for built-ins and shared skills |
| `~/.gepaw/workspaces/{agent_id}/skills/` | Skills present in a specific agent's workspace    |
...
Skill enabled state and configuration are controlled by `~/.gepaw/workspaces/{agent_id}/skill.json`.

**Manage skills via:**

- Console (Agent ?Skills) ?Visual management, import, create, enable/disable
- `gepaw init` (choose all / none / custom during setup)
- `gepaw skills config` (interactive toggle)
...
gepaw has persistent cross-conversation memory: it automatically compresses context and saves key information to Markdown files for long-term retention.

Memory files are stored in the agent workspace:

| File / Directory                                        | Purpose                                                               |
| ------------------------------------------------------- | --------------------------------------------------------------------- |
| `~/.gepaw/workspaces/{agent_id}/MEMORY.md`            | Long-lived key information (decisions, preferences, persistent facts) |
| `~/.gepaw/workspaces/{agent_id}/memory/YYYY-MM-DD.md` | Daily logs (notes, runtime context, auto-generated summaries)         |
...
- Everything lives under **`~/.gepaw`** by default; override with `gepaw_WORKING_DIR` (and related env vars) if needed.
- From **v0.1.0**, configuration is split into:
  - **Global config** (`~/.gepaw/config.json`) ?providers, environment variables, agent list
  - **Agent config** (`~/.gepaw/workspaces/{agent_id}/agent.json`) ?per-agent settings
...
- LLM providers are globally configured via `gepaw init` or the Console.
