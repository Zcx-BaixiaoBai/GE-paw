...
| `/new`     | ?No  | ?Clear           | ?Background save | ?New conversation prompt    |
| `/clear`   | ?No  | ?Clear           | ?No save         | ?History cleared prompt     |
...
>  **Warning**: `/clear` is **irreversible**! Unlike `/new`, cleared content will not be saved.
...
>  **Warning**: `/load_history` clears current memory before loading. Existing conversation will be lost!
...
- ?- Supports image input
-  - Supports video input
- _(user-added)_ - User-added model (via `gepaw models add-model` command)
...
Commands for controlling and monitoring gepaw's runtime status. These commands execute directly without going through the Agent.

Send `/daemon <subcommand>` or short names (e.g., `/status`) in chat, or run `gepaw daemon <subcommand>` from the terminal.
...
| `/daemon logs` or `/daemon logs 50` | View last N lines of log (default 100, max 2000, from `gepaw.log` in working directory) | ?  | ?      |
...
>  **Warning**: `/stop` immediately terminates the task, which may result in partial data loss.
...
```
/status                    # In chat
gepaw daemon status        # From terminal
```
...
```
/restart                   # In chat
gepaw daemon restart       # From terminal (prints instructions only)
```
...
gepaw daemon reload-config      # From terminal
```
...
Display gepaw version number, working directory path, and log file path.

**Usage:**

```
/daemon version            # In chat
gepaw daemon version       # From terminal
```
...
View the last N lines of `gepaw.log` in the working directory. Default 100 lines, maximum 2000 lines.
...
gepaw daemon logs -n 200   # From terminal, specify 200 lines
```
...
```
 **Cannot enter Phase 2**: prd.json format errors:
