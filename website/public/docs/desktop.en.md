# gepaw Desktop Application Guide
...
This guide explains how to install and use the gepaw Desktop application on Windows and macOS.
...
1. **Download the installer**
   Download `gepaw-Setup-<version>.exe` from the [Release page][releases]

2. **Run the installer**
   Double-click the `.exe` file and follow the installation wizard
   - Default installation location: `C:\Users\<your-username>\AppData\Local\gepaw`
...
#### **gepaw Desktop** (Recommended for daily use)
...
- **How to Launch**: Double-click the "gepaw Desktop" icon on desktop or Start Menu
- **Technical Note**: Uses VBScript launcher, runs Python process in background

#### **gepaw Desktop (Debug)** (Debug Mode)
...
- **How to Launch**: Double-click "gepaw Desktop (Debug)" icon in Start Menu
- **Log Contents**:
...
A: Use "gepaw Desktop (Debug)" mode to view terminal output for error messages

**Q: How to uninstall?**

A: Go to Windows Settings ?Apps ?Installed apps ?Find "gepaw Desktop" ?Uninstall
...
- **Architecture**:
  ...
  -  Intel chips - May works, but may not be able to use built-in local model services

### Installation Steps

1. **Download the archive**
   Download `gepaw-<version>-macOS.zip` from the [Release page][releases]

2. **Extract**
   Double-click the `.zip` file to extract and get `gepaw.app`

3. **Move to Applications folder (Optional)**
   Drag `gepaw.app` to the `/Applications` folder

### First Launch: Bypassing System Security Restrictions

#### Why manual trust is needed?

gepaw is **not Apple Developer-signed or notarized**, so macOS Gatekeeper will block it by default.
...
-  **First-time manual trust required**: One-time operation, permanently effective
...
1. **Right-click** (or Control + click) on `gepaw.app`
...
2. Scroll down to find a message like:
   _"'gepaw' was blocked from use because it cannot verify the developer"_
 ...
 **Warning**: This method completely removes security checks; only use if you fully trust the application source.
...
- Double-click `gepaw.app` to launch
- The app runs in the background and opens a browser window
- Logs are written to: `~/.gepaw/desktop.log`
...
cd /Applications  # or wherever your gepaw.app is located
...
1. Check the `~/.gepaw/desktop.log` file for errors
...
A: Drag `gepaw.app` to the Trash, then delete the `~/.gepaw` configuration folder
...
- **GitHub Issues**: [Submit an issue](https://github.com/agentscope-ai/gepaw/issues)
- **Packaging documentation**: `scripts/pack/README.md` - Technical details and local build guide
- **Log locations**:
  - Windows: View in Debug mode terminal, or `%USERPROFILE%\.gepaw\` directory
