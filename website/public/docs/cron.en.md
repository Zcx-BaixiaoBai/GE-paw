...
In gepaw, scheduled tasks (cron jobs) let the system run actions
...
> If creation fails, check [FAQ](https://gepaw.agentscope.io/docs/faq) under
...
2. Fill in fields:
   ...
   - **Task type and content**:
     ...
     - **agent**: send `content.text` to gepaw and deliver its response.
...
You can also tell gepaw directly in your target channel:
...
See [CLI `gepaw cron` section](./cli#gepaw-cron). Common commands:
...
Example (ask gepaw every 2 hours and deliver the response):
...
gepaw uses five-field cron: **minute hour day month weekday** (no seconds).
...
- [CLI `gepaw cron` section](./cli#gepaw-cron) ?command reference
