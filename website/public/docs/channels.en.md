...
A **channel** is where you talk to gepaw: connect DingTalk and it replies
...
- **Edit `agent.json` directly** ?Agent workspace config at `~/.gepaw/workspaces/{agent_id}/agent.json`, set `enabled: true` and fill in that platform's credentials. Saving triggers a reload without restarting the app.
...
Go to "ControlChannels", find **DingTalk**, click it, and enter the **Client ID** and **Client Secret** you just obtained.
...
In your agent's `agent.json` (e.g., `~/.gepaw/workspaces/default/agent.json`), find `channels.dingtalk` and fill in the corresponding information, for example:
...
> - `robot_code` is recommended in group scenarios; if empty, gepaw falls back to `client_id`.

Save the file; if the app is already running, the channel will reload. Otherwise run `gepaw app`.
...
> You can add the bot to a group chat via **Group Settings ?Bots ?Add a robot in DingTalk**. If you create a group chat from your one-on-one chat with the bot, the bots replies will not be triggered.
...
4. Run **`gepaw app`** to start gepaw
...
> **Note:** Follow this order: Configure App ID/Secret ?start `gepaw app` ?then configure the long connection in the Feishu console. If errors persist, try stopping the gepaw service and restarting `gepaw app`.
...
Find `channels.feishu` in your agent's `agent.json` (e.g., `~/.gepaw/workspaces/default/agent.json`). Only **App ID** and **App Secret** are required (copy from the Feishu console under Credentials & basic info):
...
> **Note:** You can also fill in **App ID** and **App Secret** in the Console UI, but you must restart the gepaw service before continuing with the long-connection configuration.
...
> **User display name (recommended):** To show **user nicknames** in sessions and logs (e.g. "#1d1a" instead of "unknown#1d1a"), enable the contact read permission **Read contact as app** (`contact:user.base:readonly`). Without it, Feishu only returns identity fields (e.g. open_id) and not the user's name, so gepaw cannot resolve nicknames. After enabling, publish or update the app version so the permission takes effect.
...
>  The iMessage channel is **macOS only**. It relies on the local Messages app and the iMessage database, so it cannot run on Linux or Windows.
...
3. For gepaw to read iMessage data, **Terminal** (or the app you use to run `gepaw app`) and **Messages** need **Full Disk Access** (System Settings ?Privacy & Security ?Full Disk Access).
...
4. Set the iMessage database path. The default is `~/Library/Messages/chat.db`; use this unless you've moved the database. You can configure it in either of these ways:
   ...
   - Or edit the agent workspace `agent.json` (usually at `~/.gepaw/workspaces/default/agent.json`):
...
Find `channels.discord` in your agent's `agent.json` (e.g., `~/.gepaw/workspaces/default/agent.json`) and fill in the fields, for example:
...
5. In **Developer settings**, get **AppID** and **AppSecret** (ClientSecret) and fill them into config (see below). Add your servers **IP to the whitelist** ?only whitelisted IPs can call the Open API outside sandbox.

   > **Tip:** If you are using ModelScope Creative Space to deploy gepaw, the IP whitelist for QQ channel should be: `47.92.200.108`
...
In your agent's `agent.json` (e.g., `~/.gepaw/workspaces/default/agent.json`), find `channels.qq` and set `app_id` and `client_secret` to the values above:
...
The **OneBot** channel connects gepaw to [NapCat](https://github.com/NapNeko/NapCatQQ), [go-cqhttp](https://github.com/Mrs4s/go-cqhttp), [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), or any other [OneBot v11](https://github.com/botuniverse/onebot-11) compatible implementation via **reverse WebSocket**.
...
gepaw starts a WebSocket server; the OneBot implementation (e.g. NapCat) connects to it as a client:

```
NapCat  reverse WS? gepaw (:6199/ws)
```
...
3. Go to **Network Config** ?**New** ?**WebSocket Client** (reverse WS):
   - URL: `ws://<gepaw_host>:6199/ws`
   - Access Token: same as `access_token` in gepaw config (optional)
...
> **Docker Compose tip:** When running gepaw and NapCat in Docker Compose, set the NapCat reverse WS URL to `ws://gepaw:6199/ws` (using the service name).
...
> **Note:** Audio and video are received at the channel level, but require gepaw's transcription provider (`transcription_provider_type`) to be configured for the LLM to process them. Without transcription, voice messages are shown as placeholders.
...
**Method 2:** Fill in `agent.json` (e.g., `~/.gepaw/workspaces/default/agent.json`)
...
| `media_dir`              | string | `~/.gepaw/media` | Media files (images, files, etc.) download directory |
| `max_reconnect_attempts` | int    | `-1`               | WebSocket max reconnect attempts (`-1` = unlimited)  |
...
- **Authentication**: On first use, scan a QR code to authorize. The token is automatically persisted to a local file (default `~/.gepaw/wechat_bot_token`), so you won't need to scan again on subsequent starts.
...
1. Open the gepaw Web Console and go to **Settings ?Channels ?WeChat Personal (iLink)**.
...
You can also configure directly in the agent workspace `agent.json` (e.g., `~/.gepaw/workspaces/default/agent.json`):
...
| `bot_token_file` | string | `~/.gepaw/wechat_bot_token` | Path to persist the token for future runs                                             |
| `base_url`       | string | official default              | iLink API base URL; leave empty to use the official default                           |
| `media_dir`      | string | `~/.gepaw/media`            | Directory to save received images and files                                           |
...
Find `channels.telegram` in your agent's `agent.json` (e.g., `~/.gepaw/workspaces/default/agent.json`) and fill in the fields, for example:
...
2. Fuzzy match subscription and automatic push
   ...
   Subscribe to the wildcard topic `/server/+/up`. Messages will be automatically pushed to the corresponding topic based on the client's `client_id`. For example, after a client pushes a message to `/server/client_a/up`, gepaw will push the message to `/client/client_b/down` after processing.
...
3. Redirected topic push
   ...
   Messages will be pushed to `client/client_b/down` based on the `redirect_client_id` attribute, enabling cross-topic push. In IoT scenarios, with gepaw as the core, autonomous message pushing between multiple devices can be achieved according to individual requirements.

---

## Matrix

The Matrix channel connects gepaw to any Matrix homeserver using the [matrix-nio](https://github.com/poljar/matrix-nio) library. It supports text messaging in both direct messages and group rooms.
...
Find `channels.matrix` in your agent's `agent.json` (e.g., `~/.gepaw/workspaces/default/agent.json`):
...
Save the file; the channel will reload automatically if gepaw is already running.
...
The Yuanbao channel connects gepaw to Tencent's Yuanbao AI assistant platform via protobuf WebSocket, supporting C2C (direct) and group chat with image/file sending.
...
2. In the bot settings, find **Method 2** to get the **App ID** and **App Secret**, then fill them into gepaw's channel settings and click **Done**.
...
The XiaoYi channel connects gepaw via **A2A (Agent-to-Agent) protocol** over WebSocket to Huawei's AI assistant platform.
...
The Voice channel enables phone call interactions with gepaw via Twilio ConversationRelay, supporting Speech-to-Text (STT) and Text-to-Speech (TTS) for voice-based conversations.

### Prerequisites

1. **Twilio Account**: Register at [Twilio](https://www.twilio.com/) and obtain credentials
2. **Cloudflare Tunnel** (or similar): Expose your local gepaw service to the public internet for Twilio webhook callbacks
...
Twilio needs to reach gepaw's webhook endpoint via the public internet, so you need to expose your local service.
...
After configuration, simply call your Twilio phone number to have a voice conversation with gepaw:

1. Dial the phone number
2. After hearing the welcome greeting, start speaking
3. gepaw converts speech to text and processes it through the Agent
...
| `welcome_greeting`   | string | `"Hi! This is gepaw. How can I help you?"` | Welcome message when call connects           |
...
The SIP channel enables voice conversations with gepaw via standard SIP phones and softphones (e.g., Linphone, MicroSIP, IP desk phones). It works entirely on your local network or private infrastructure ?no cloud account or public URL required.
...
The fastest way to try SIP. gepaw starts a built-in SIP registrar automatically ?no Asterisk, FreeSWITCH, or any external server needed.
...
2. Start gepaw and configure in Console:
...
3. Open [Linphone](https://www.linphone.org/linphone) (or any SIP softphone) and configure:
   ...
   You should hear the welcome greeting, then speak ?gepaw will reply!
...
2. Install, start gepaw, and configure in Console:
...
3. Generate a token and join via [LiveKit Meet](https://meet.livekit.io/):
   ...
   - Allow microphone access, then speak ?gepaw responds!
...
Use Asterisk, FreeSWITCH, or any SIP PBX as the registrar. Set `sip_server` to your PBX address. gepaw registers as a SIP extension and receives calls routed by the PBX.
...
When `sip_server` is empty, gepaw starts a built-in SIP registrar on port 5060 and the agent registers to it automatically. When `sip_server` is set (e.g., `"192.168.1.100:5060"`), gepaw registers to that external server instead.

### LiveKit mode configuration

Production mode delegates SIP/RTP to LiveKit SIP Server ?a Go binary that handles NAT traversal, jitter buffering, and codec negotiation. gepaw joins LiveKit rooms as an AI participant.
...
3. Start gepaw. For SIP phone calls, also set up LiveKit infrastructure with a SIP Trunk and Dispatch Rule (see [LiveKit SIP docs](https://docs.livekit.io/sip/)). For browser-based testing, see the [Quick try](#quick-try-livekit-mode-via-browser-3-minutes-no-sip-phone-needed) section above.
...
2. Start speaking ?gepaw converts speech to text via streaming STT
...
- **ChannelManager** keeps one queue per channel that uses it. When a message arrives, the channel calls **`self._enqueue(payload)`** (injected by the manager at startup); the managers consumer loop then calls **`channel.consume_one(payload)`**.
...
| Method                                                  | Purpose                                                                                                                                                            |
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `build_agent_request_from_native(self, native_payload)` | Convert the channels native message to `AgentRequest` (using runtime `Message` / `TextContent` / `ImageContent` etc.) and set `request.channel_meta` for sending. |
...
- **Directory**: Channels under the working dir at `custom_channels/` (default `~/.gepaw/custom_channels/`) are loaded at runtime. The manager scans `.py` files and packages (subdirs with `__init__.py`), loads `BaseChannel` subclasses, and registers them by the classs `channel` attribute.
- **Install**: `gepaw channels install <key>` creates a template `<key>.py` in `custom_channels/` for you to edit, or use `--path <local path>` / `--url <URL>` to copy a channel module from disk or the web. `gepaw channels add <key>` does the same and also adds a default entry to config (with optional `--path`/`--url`).
- **Remove**: `gepaw channels remove <key>` deletes that channels module from `custom_channels/` (custom channels only; built-ins cannot be removed). By default it also removes the key from `channels` in `config.json`; use `--keep-config` to leave config unchanged.
- **Config**: `ChannelConfig` uses `extra="allow"`, so any channel key can appear under `channels` in `config.json`. Use `gepaw channels config` for interactive setup or edit config by hand.

### HTTP route registration

For channels that require webhook callbacks (e.g., WeChat, Slack, LINE), you can register custom HTTP routes by exporting a `register_app_routes` callable in your module ?no changes to gepaw's core source required.

At startup, gepaw scans modules in `custom_channels/` for a `register_app_routes` export. If found, it is called with the FastAPI `app` instance, allowing the channel to register any routes it needs.
...
**Real-world example**: WeChat ClawBot integration ([PR #2140](https://github.com/agentscope-ai/gepaw/pull/2140), [Issue #2043](https://github.com/agentscope-ai/gepaw/issues/2043)) uses this mechanism to register the `/api/wechat/callback` route with Tencent's official SDK for message delivery.
