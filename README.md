...
[![GitHub Repo](https://img.shields.io/badge/GitHub-Repo-black.svg?logo=github)](https://github.com/agentscope-ai/gepaw)
[![PyPI](https://img.shields.io/pypi/v/gepaw?color=3775A9&label=PyPI&logo=pypi)](https://pypi.org/project/gepaw/)
[![Documentation](https://img.shields.io/badge/Docs-Website-green.svg?logo=readthedocs&label=Docs)](https://gepaw.agentscope.io/)
...
[![Last Commit](https://img.shields.io/github/last-commit/agentscope-ai/gepaw)](https://github.com/agentscope-ai/gepaw)
[![License](https://img.shields.io/badge/license-Apache%202.0-red.svg?logo=apache&label=License)](LICENSE)
[![Code Style](https://img.shields.io/badge/code%20style-black-black.svg?logo=python&label=CodeStyle)](https://github.com/psf/black)
[![GitHub Stars](https://img.shields.io/github/stars/agentscope-ai/gepaw?style=flat&logo=github&color=yellow&label=Stars)](https://github.com/agentscope-ai/gepaw/stargazers)
[![GitHub Forks](https://img.shields.io/github/forks/agentscope-ai/gepaw?style=flat&logo=github&color=purple&label=Forks)](https://github.com/agentscope-ai/gepaw/network)
...
[[Documentation](https://gepaw.agentscope.io/)] [[](README_zh.md)] [[](README_ja.md)] [[](README_ru.md)]

<p align="center">
  <img src="https://gw.alicdn.com/imgextra/i1/O1CN01sens5C1TuwioeGexL_!!6000000002443-55-tps-771-132.svg" alt="gepaw Logo" width="120">
</p>
...
>
> **Skills extension** ?Built-in scheduling, PDF/Office processing, news digest, and more; custom skills auto-loaded, no lock-in. Skills determine what gepaw can do.
>
...
>
> **Every channel** ?DingTalk, Feishu, WeChat, Discord, Telegram, and more. One gepaw, connect as needed.
>
...
>
> <details>
> <summary><b>What you can do with gepaw</b></summary>
>
> <br>
>
...
- [2026-06-01] **v1.1.10 ?Spawn Subagent & More Channels** | Agents can now spawn sub-agents; Tencent Yuanbao joins the channel family.
  ...
  Also: OpenRouter attribution headers, bundled desktop CLI, skill update stability fixes. [v1.1.10 Release Notes ](https://gepaw.agentscope.io/release-notes#v1.1.10)

- [2026-05-27] **v1.1.9** ?Coding Mode (three-panel Web IDE), Tauri desktop app, unified access control. [v1.1.9 Release Notes ](https://gepaw.agentscope.io/release-notes#v1.1.9)

- [2026-05-19] **v1.1.8** ?Official plugin distribution, gepaw Pet, streaming cards for DingTalk / Feishu / Telegram. [v1.1.8 Release Notes ](https://gepaw.agentscope.io/release-notes#v1.1.8)

- [2026-05-14] **v1.1.7** ?Browser batch actions, OAuth 2.1 MCP, Cron calendar view, multiple file attachments. [v1.1.7 Release Notes ](https://gepaw.agentscope.io/release-notes#v1.1.7)

- [2026-05-09] **v1.1.6** ?Whisper voice input, GPT Image 2 plugin, Volcano Engine provider, Mermaid diagrams. [v1.1.6 Release Notes ](https://gepaw.agentscope.io/release-notes#v1.1.6)

- [2026-04-12] **CoPaw rebrands to gepaw** ?deeper Qwen ecosystem integration, same open-source mission. [v1.0.0 Release Notes ](https://gepaw.agentscope.io/release-notes#v1.0.0)
...
> - ** I want to use in DingTalk/Feishu/WeChat**: Complete Quick Start ?[Configure model](#api-key) ?[Channel setup](https://gepaw.agentscope.io/docs/channels)
> - ** I don't want to install Python**: [Desktop app](#option-6-desktop-application-beta) or [Script install](#option-2-script-install) or [ModelScope Studio](https://modelscope.cn/studios/fork?target=AgentScope/gepaw)
> - ** I want to use local models (no API key needed)**: [Local Models](#local-models) ?download model ?start using
> - **?I want to contribute code or develop new features**: [Install from source](#install-from-source) ?[Contributing](#contributing)
- [News](#news)
...
Then open the Console in your browser at **http://127.0.0.1:8088/** to configure your model. To chat in DingTalk, Feishu, WeChat, etc., see the [Channel setup](https://gepaw.agentscope.io/docs/channels) documentation.
...
No Python setup required, one command installs everything. The script will automatically download uv (Python package manager), create a virtual environment, and install gepaw with all dependencies (including Node.js and frontend assets). Note: May not work in restricted network environments or corporate firewalls.
...
>      - Check if `uv` is available: Enter `uv --version` in CMD. If a version number appears, **only configure the gepaw path**. If you receive the prompt `'uv' is not recognized as an internal or external command, operable program or batch file,` configure both paths.
...
>      - gepaw path: Typically located at `%USERPROFILE%\.gepaw\bin`.
...
>   - **Configure `uv` environment variables**: Add the `uv` directory and `%USERPROFILE%\.gepaw\bin` to your system's `Path` variable.
>   - **Re-run the installation**: Open a new terminal and execute the installation script again to complete the `gepaw` installation.
>   - **Configure the `gepaw` environment variable**: Add `%USERPROFILE%\.gepaw\bin` to your system's `Path` variable.

Once installed, open a new terminal and run:

```bash
gepaw init --defaults   # or: gepaw init (interactive)
gepaw app
```
...
# Uninstall
gepaw uninstall          # keeps config and data
gepaw uninstall --purge  # removes everything
```
...
Images are on **Docker Hub** (`agentscope/gepaw`). Image tags: `latest` (stable); `pre` (PyPI pre-release).
...
Also available on Alibaba Cloud Container Registry (ACR) for users in China: `agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/gepaw` (same tags).

Then open **http://127.0.0.1:8088/** for the Console. Config, memory, and skills are stored in the `gepaw-data` volume; model provider settings and API keys are in the `gepaw-secrets` volume; backup archives are stored in the `gepaw-backups` volume. To pass API keys (e.g. `DASHSCOPE_API_KEY`), add `-e VAR=value` or `--env-file .env` to `docker run`.

> **Connecting to Ollama or other services on the host machine**
>
> Inside a Docker container, `localhost` refers to the container itself, not your host machine. If you run Ollama (or other model services) on the host and want gepaw in Docker to reach them, use one of these approaches:
>
...
>   agentscope/gepaw:latest
> ```
> Then in gepaw **Settings ?Models**, change the Base URL to `http://host.docker.internal:<port>` ?for example, `http://host.docker.internal:11434` for Ollama, or `http://host.docker.internal:1234/v1` for LM Studio.
>
...
To run gepaw on Alibaba Cloud (ECS), use the one-click deployment: open the [gepaw on Alibaba Cloud (ECS) deployment link](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) and follow the prompts. For step-by-step instructions, see [Alibaba Cloud Developer: Deploy your AI assistant in 3 minutes](https://developer.aliyun.com/article/1713682).

---

### Option 5: Using ModelScope

**No local install?** [ModelScope Studio](https://modelscope.cn/studios/fork?target=AgentScope/gepaw) one-click cloud setup. Set your Studio to **non-public** so others cannot control your gepaw.
...
If you're not comfortable with command-line tools, you can download and use gepaw's desktop application without manually configuring Python environments or running commands.

#### Download

Download the desktop app from [GitHub Releases](https://github.com/agentscope-ai/gepaw/releases):
- **Windows**: `gepaw-Setup-<version>.exe`
- **macOS**: `gepaw-<version>-macOS.zip` (Apple Silicon recommended)
...
When you download the gepaw macOS app from Releases, macOS may show: *"Apple cannot verify that 'gepaw' contains no malicious software"*. This happens because the app is not notarized. You can still open it as follows:

- **Right-click to open (recommended)**
  Right-click (or Control+click) the gepaw app ?**Open** ?in the dialog click **Open** again. This tells Gatekeeper you trust the app; after that you can double-click to launch as usual.

- **Allow in System Settings**
  If it is still blocked, go to **System Settings ?Privacy & Security**, scroll to the message like *"gepaw was blocked because it is from an unidentified developer"*, and click **Open Anyway** or **Allow**.
...
For detailed usage instructions, troubleshooting, and common issues, see the [Desktop Application Guide](https://gepaw.agentscope.io/docs/desktop).

---

## API Key

If you use a **cloud LLM API** (e.g., Qianwen, Gemini, OpenAI), you must configure an API key before chatting. gepaw will not work until a valid key is set. See the [official docs](https://gepaw.agentscope.io/docs/models) for details.

**How to configure:**

1. **Console (recommended)** ?After running `gepaw app`, open **http://127.0.0.1:8088/** ?**Settings** ?**Models**. Choose a provider, enter the **API Key**, and enable that provider and model.
2. **`gepaw init`** ?When you run `gepaw init`, it will guide you through configuring the LLM provider and API key. Follow the prompts to choose a provider and enter your key.
...
Tools that need extra keys (e.g. `TAVILY_API_KEY` for web search) can be set in Console **Settings ?Environment variables**, see [Config](https://gepaw.agentscope.io/docs/config) for details.

> **Using local models only?** If you use [Local Models](#local-models) (llama.cpp / Ollama / LM Studio), you do **not** need any API key.

## Local Models

gepaw can run LLMs entirely on your machine ?no API keys or cloud services required. See the [official docs](https://gepaw.agentscope.io/docs/models) for details.
...
| Topic                                                                 | Description                                      |
| --------------------------------------------------------------------- | ------------------------------------------------ |
| [Introduction](https://gepaw.agentscope.io/docs/intro)                | What gepaw is and how to use it                  |
| [Quick start](https://gepaw.agentscope.io/docs/quickstart)            | Install and run (local or ModelScope Studio)    |
| [Console](https://gepaw.agentscope.io/docs/console)                   | Web UI: chat and agent configuration            |
| [Models](https://gepaw.agentscope.io/docs/models)                     | Configure cloud, local, and custom providers    |
| [Channels](https://gepaw.agentscope.io/docs/channels)                  | DingTalk, Feishu, QQ, Discord, iMessage, and more |
| [Skills](https://gepaw.agentscope.io/docs/skills)                      | Extend and customize capabilities               |
| [Plugins](https://gepaw.agentscope.io/docs/plugins)                    | Plugin system                                    |
| [MCP](https://gepaw.agentscope.io/docs/mcp)                            | Manage MCP clients                               |
| [Memory](https://gepaw.agentscope.io/docs/memory)                     | Long-term memory                     |
| [Memory-Evolving & Proactive](https://gepaw.agentscope.io/docs/memory-evolving-and-proactive) | Agent memory evolution and proactive interaction |
| [Context](https://gepaw.agentscope.io/docs/context)                   | Context management mechanism                     |
| [Magic commands](https://gepaw.agentscope.io/docs/commands)           | Control conversation state without waiting for the AI |
| [Heartbeat](https://gepaw.agentscope.io/docs/heartbeat)                | Scheduled check-in and digest                    |
| [Multi-Agent](https://gepaw.agentscope.io/docs/multi-agent)           | Create multiple agents and enable collaboration  |
| [Config & working dir](https://gepaw.agentscope.io/docs/config) | Working directory and config file                |
| [CLI](https://gepaw.agentscope.io/docs/cli)                            | Init, cron jobs, skills, clean                   |
| [FAQ](https://gepaw.agentscope.io/docs/faq)                           | Common questions and troubleshooting             |
...
gepaw includes multi-layer security mechanisms to protect your data and system:
...
- **Web Authentication** ?Optional login protection for the Console. Disabled by default; set `gepaw_AUTH_ENABLED=true` to enable. See [Web Authentication](https://gepaw.agentscope.io/docs/security#Web-Authentication) for details.

See [Security documentation](https://gepaw.agentscope.io/docs/security) for details.

---

## FAQ

For common questions, troubleshooting tips, and known issues, please visit the **[FAQ page](https://gepaw.agentscope.io/docs/faq)**.

---

## Staying ahead

<a href="https://github.com/agentscope-ai/gepaw">
  <img src="https://img.alicdn.com/imgextra/i1/O1CN01V8HYv61By0HYcIDaq_!!6000000000013-1-tps-1698-954.gif" width="600" alt="Star gepaw" />
</a>

Star gepaw on GitHub and be instantly notified of new releases.
...
| **gepaw Application**   | gepaw Creator                                                             | In Progress          |
|                          | gepaw Insight                                           | In Progress              |
...
- **Then**: Run `gepaw init --defaults`, then `gepaw app`.

> **Note for updates:** When updating to a new major version after `git pull`, please also rebuild the frontend, reinstall the package (`pip install -e .`), restart `gepaw app`, and clear your browser cache with `Ctrl+Shift+R` (or `Cmd+Shift+R` on macOS).

---

## Contributing

gepaw evolves through open collaboration, and we welcome all forms of contribution! Check the [Roadmap](#roadmap) above (especially items marked **Seeking Contributors**) to find areas that interest you, and read [CONTRIBUTING](https://github.com/agentscope-ai/gepaw/blob/main/CONTRIBUTING.md) to get started. We particularly welcome:
...
Join [GitHub Discussions](https://github.com/agentscope-ai/gepaw/discussions) to discuss ideas or pick up tasks.

---

## Why gepaw?

gepaw stands for Qwen Personal Agent Workstation, and also embodies the wisdom of Qwen and the warmth of a Paw.
We hope it is not a cold tool, but an intelligent and warm "little paw" always ready to helpa most intuitive partner in your digital life.
...
gepaw collects **anonymous** usage data during `gepaw init` to help us understand our user base and prioritize improvements. Data is sent **once per version** ?when you upgrade gepaw, telemetry is re-collected so we can track version adoption.

**What we collect:**

- gepaw version (e.g., 0.0.7)
...
When running `gepaw init` interactively, you will be asked whether to opt in. If you choose `--defaults`, telemetry is accepted automatically. The prompt appears once per version and never affects gepaw's functionality.

---

## License

gepaw is released under the [Apache License 2.0](LICENSE).
...
<a href="https://github.com/agentscope-ai/gepaw/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=agentscope-ai/gepaw" alt="Contributors" />
</a>
